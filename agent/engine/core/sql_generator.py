...
class StaticOptimizer:
    ...
    @classmethod
    def _optimize_zero_movement(cls, nodes: List[Dict], edges: List[Dict], connections: Dict):
        node_map = {n['node_id']: n for n in nodes}
        ...
        for node in list(nodes):
            if node.get('operator_type') == "load":
                ...
                # Get connection IDs
                sink_conn_id = str(node.get("connection_id") or (node.get("destination_asset") or {}).get("connection_id"))
                ...
                if source_conn_id and source_conn_id == sink_conn_id:
                    ...
                    if not conn_data or conn_data.get('type') not in ["postgresql", "mysql", "mariadb", "mssql", "snowflake", "bigquery"]:
                        continue
                    ...
                    target_table = config.get("asset") or config.get("table") or config.get("target_table")
                    if not target_table:
                        continue
    ...
    @classmethod
    def _get_node_connection_id(cls, node: Dict) -> Optional[str]:
        if node.get('operator_type') == "extract":
            return str(node.get("connection_id") or (node.get("source_asset") or {}).get("connection_id"))
        return node.get('config', {}).get("_source_connection_id")

    @classmethod
    def _get_node_sql(cls, node: Dict) -> Optional[str]:
        if node.get('operator_type') == "extract":
            config = node.get('config', {})
            ...
            if config.get("_pushdown_operators"):
                return SQLPushdownGenerator.generate_sql(base, config["_pushdown_operators"])
    ...
    @classmethod
    def _collapse_join(cls, join_node: Dict, left: Dict, right: Dict, edges: List[Dict]):
    ...
    @classmethod
    def _attempt_linear_pushdown(cls, start_node: Dict, node_map: Dict[str, Dict], edges: List[Dict]):
