...
import psutil
import os
from typing import Dict, Any, List, Set, Optional, Union, Iterator, Tuple
...
class DataCache:
    ...
    def store(self, node_id: str, chunks: List[pd.DataFrame]):
        with self._lock:
            ...
            if node_id in self._access_order: self._access_order.remove(node_id)
        ...
        with self._lock:
            for nid in node_ids:
                if nid in self._cache:
                    ...
                    if nid in self._access_order: self._access_order.remove(nid)
...
class NodeExecutor:
    ...
    def execute(self, node: Dict[str, Any], inputs: Dict[str, List[pd.DataFrame]], status_cb: Any = None) -> List[pd.DataFrame]:
        ...
        logger.info(f"→ Executing {node_id} ({op_type}/{op_class})")
        ...
        def on_chunk(chunk: pd.DataFrame, direction: str = "out", error_count: int = 0, filtered_count: int = 0):
            if chunk.empty and error_count == 0: return
        ...
        try:
            ...
            if op_type == "extract":
                ...
                if not conn_data: raise ValueError(f"Connection {conn_id} missing")
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as pool:
                    ...
                    for future in concurrent.futures.as_completed(futures):
                        ...
                        try:
                            ...
                            log_cb(f"✅ Node {nid} finalized. [{total_rows:,} rows]", nid)
                            if status_cb:
                            ...
                            log_cb(f"❌ Node {nid} aborted: {str(e)}", nid)
                            if status_cb: status_cb(nid, "failed", {"error_message": str(e)})
                            raise e
