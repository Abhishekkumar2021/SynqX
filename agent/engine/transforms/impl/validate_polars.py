...
class ValidatePolarsTransform(PolarsTransform):
    """
    High-performance Data Quality & Validation using Polars on Remote Agent.
    """
        ...
        for df in data:
            ...
            if not invalid_df.is_empty():
                ...
                if strict:
                    ...
                    raise TransformationError(f"Strict validation failed: {first_error}")

                if on_chunk_cb:
                    on_chunk_cb(invalid_df.to_pandas(), direction="quarantine", error_count=len(invalid_df))
