...
def run_step(msg: str, cmd: list, cwd: Path | None = None):
    ...
    with console.status(f"[cyan]{msg}...[/cyan]"):
        try:
            ...
            console.print(f"[bold red]Exception during {msg}:[/bold red] {e!s}")
            raise
...
def install(  # noqa: PLR0912
    ...
    client_id: str | None = Option(None, help="Agent Identity ID"),
    api_key: str | None = Option(None, help="Agent Security Key"),
    ...
    console.print(
        Panel.fit("[bold blue]SynqX Agent Deployment[/bold blue]", border_style="blue")
    )
    ...
    if not is_venv() and not force:
        ...
        if not Confirm.ask(
            "Do you want to continue with global installation? (Not recommended)"
        ):
            raise typer.Abort()
        ...
        try:
            if (Path.cwd() / "requirements.txt").exists():
                run_step(
                    ...
                    pip_cmd + ["-r", "requirements.txt"],  # noqa: RUF005
                )
            ...
            if libs_dir.exists():
                for lib in ["synqx-core", "synqx-engine"]:
                    ...
                    if lib_path.exists():
                        run_step(f"Installing {lib}", pip_cmd + ["-e", str(lib_path)])  # noqa: RUF005

            if (Path.cwd() / "pyproject.toml").exists():
                run_step("Installing Agent package", pip_cmd + ["-e", "."])  # noqa: RUF005
        except Exception as e:
    ...
    if not client_id or not api_key:
        ...
        if not client_id:
            client_id = (
                Prompt.ask("Enter Client ID", default=current_id)
                if not current_id
                else current_id
            )
        if not api_key:
            api_key = (
                Prompt.ask("Enter API Key", password=True, default=current_key)
                if not current_key
                else current_key
            )
    ...
    try:
        ...
        console.print(
            Panel.fit(
                "[bold green]Success![/bold green] Agent is ready for service.",
                border_style="green",
            )
        )
        ...
        console.print(
            "[red]Verification failed. Please check your dependencies and environment.[/red]"  # noqa: E501
        )
