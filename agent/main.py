...
class SynqxAgent:
    def __init__(self, headless: bool = False):
        ...
        if not self.client_id or not self.api_key:
            if not self.headless:
                console.print(
                    Panel(
                        "[bold red]Configuration Missing[/bold red]\nRun [cyan]python install.py[/cyan] or [cyan]configure[/cyan] command.",  # noqa: E501
                        border_style="red",
                    )
                )
            sys.exit(1)
    ...
    def _handle_exit(self, signum, frame):
        ...
        try:
            ...
            self.session.post(
                f"{self.api_url}/agents/heartbeat", json=payload, timeout=2
            )
        except Exception:
            pass
    ...
    def report_step_status(
        self, job_id: int, node_id: str, status: str, data: dict[str, Any] = None  # noqa: RUF013
    ):
        now = time.time()
        ...
        if not is_terminal and (now - last_report < 2):  # noqa: PLR2004
            return
    ...
    def send_logs(self, job_id: int, level: str, message: str, node_id: str = None):  # noqa: RUF013
        payload = [
            {
                "level": level,
                "message": message,
    ...
    def process_ephemeral_job(self, data: dict[str, Any]):  # noqa: PLR0912, PLR0915
        job_id = data["id"]
        job_type = data["type"]
        ...
        try:
            ...
            if job_type == "explorer":
                ...
                if action == "list":
                    # For listing, we might want to list relative to root, but here we list the sandboxed path  # noqa: E501
            ...
            self.session.post(
                ...
                json=sanitize_for_json(result_update),
                timeout=10,
            )
    ...
    def run(self):
        ...
        while self.running:
            try:
                # Heartbeat every 30s
                if time.time() - self.last_heartbeat > 30:  # noqa: PLR2004
                    self.heartbeat()
                    self.last_heartbeat = time.time()

                # Poll for jobs
                resp = self.session.post(
                    f"{self.api_url}/agents/poll", json=self.tags, timeout=15
                )
                ...
                if resp.status_code == 200:  # noqa: PLR2004
                    ...
                    self.running = False
                elif resp.status_code >= 500:  # noqa: PLR2004
...
def ping():
    ...
    headers = {"X-SynqX-Client-ID": client_id, "X-SynqX-API-Key": api_key}

    with console.status("[cyan]Pinging gateway...[/cyan]"):
        try:
            resp = requests.post(
                f"{api_url}/agents/poll", json=[], headers=headers, timeout=5
            )
    ...
    if PID_FILE.exists():
        try:
            ...
            if psutil.pid_exists(pid):
                ...
                console.print(
                    "[bold yellow]STALE[/bold yellow] (Process not found but PID file exists)"  # noqa: E501
                )
        except Exception as e:
