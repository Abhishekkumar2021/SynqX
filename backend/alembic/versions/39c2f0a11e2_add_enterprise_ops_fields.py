...
down_revision: str | Sequence[str] | None = "28b3f07a11e1"
...
def upgrade() -> None:
    ...
    op.add_column(
        "pipelines",
        sa.Column(
            "upstream_pipeline_ids", sa.JSON(), nullable=True, server_default="[]"
        ),
    )
    ...
    op.add_column(
        "jobs",
        sa.Column("backfill_config", sa.JSON(), nullable=True, server_default="{}"),
    )
