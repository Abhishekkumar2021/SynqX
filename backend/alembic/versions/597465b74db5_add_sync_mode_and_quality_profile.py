...
down_revision: Union[str, Sequence[str], None] = 'ada1ce8ad284'
...
def upgrade() -> None:
    """Upgrade schema."""
    # 0. Clean up existing incorrect Enum types if they exist (handling potential case mismatch)
    ...
    write_strategy_enum = sa.Enum('append', 'overwrite', 'upsert', 'scd2', name='writestrategy')
    schema_evolution_policy_enum = sa.Enum('strict', 'evolve', 'ignore', name='schemaevolutionpolicy')
    ...
    # 1. Add sync_mode
    op.add_column('pipeline_nodes', sa.Column('sync_mode', sync_mode_enum, nullable=False, server_default='full_load'))
    
    # 2. Alter write_strategy
    op.execute('ALTER TABLE pipeline_nodes ALTER COLUMN write_strategy DROP DEFAULT')
    op.alter_column('pipeline_nodes', 'write_strategy',
               existing_type=sa.VARCHAR(length=50),
               type_=write_strategy_enum,
               existing_nullable=False,
               postgresql_using='write_strategy::writestrategy')
    op.execute("ALTER TABLE pipeline_nodes ALTER COLUMN write_strategy SET DEFAULT 'append'")
    
    # 3. Alter schema_evolution_policy
    op.execute('ALTER TABLE pipeline_nodes ALTER COLUMN schema_evolution_policy DROP DEFAULT')
    op.alter_column('pipeline_nodes', 'schema_evolution_policy',
               existing_type=sa.VARCHAR(length=50),
               type_=schema_evolution_policy_enum,
               existing_nullable=False,
               postgresql_using='schema_evolution_policy::schemaevolutionpolicy')
    op.execute("ALTER TABLE pipeline_nodes ALTER COLUMN schema_evolution_policy SET DEFAULT 'strict'")
...
def downgrade() -> None:
    ...
    op.alter_column('pipeline_nodes', 'schema_evolution_policy',
               existing_type=sa.Enum('strict', 'evolve', 'ignore', name='schemaevolutionpolicy'),
               type_=sa.VARCHAR(length=50),
               existing_nullable=False,
               existing_server_default=sa.text("'strict'::character varying"))
    op.alter_column('pipeline_nodes', 'write_strategy',
               existing_type=sa.Enum('append', 'overwrite', 'upsert', 'scd2', name='writestrategy'),
               type_=sa.VARCHAR(length=50),
               existing_nullable=False,
