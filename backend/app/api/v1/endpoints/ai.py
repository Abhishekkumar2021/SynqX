...
async def convert_prompt(
    ...
    try:
        ...
        if "429" in error_msg:
            ...
            raise HTTPException(
                status_code=429, 
                detail="AI Quota Exceeded. The Gemini API is currently limiting requests for this key."
            )
        if "404" in error_msg:
            raise HTTPException(
                status_code=404,
                detail=f"Model '{settings.GOOGLE_AI_MODEL}' not found. Please check your .env configuration."
            )
