...
@router.post(
    "/",
    response_model=alert_schema.AlertConfigRead,
...
def list_alert_configs(
    ...
    return (
        ...
        .filter(models.AlertConfig.workspace_id == current_user.active_workspace_id)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def list_alerts(
    ...
    total = query.count()
    items = (
        query.order_by(models.Alert.created_at.desc()).offset(skip).limit(limit).all()
    )
