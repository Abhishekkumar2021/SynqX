...
def execute_connection_query(
    ...
    try:
        ...
        return EphemeralJobResponse(
            ...
            started_at=datetime.now(UTC),  # Approx
...
def get_query_history(
    ...
    legacy_history = (
        db.query(QueryHistory, models.Connection.name.label("connection_name"))
...
def clear_query_history(
    ...
    db.query(QueryHistory).filter(
        QueryHistory.workspace_id == current_user.active_workspace_id
    ).delete()
...
def get_connection_schema_metadata(
    ...
    if is_remote_group(agent_group):
        ...
        for asset in connection.assets:
            if asset.schema_metadata:
                metadata[asset.name] = [
                    col["name"] for col in asset.schema_metadata.get("columns", [])
                ]
            else:
                metadata[asset.name] = []
        return {
    ...
    try:
        ...
        connector = ConnectorFactory.get_connector(
            connector_type=connection.connector_type.value, config=config
        )
