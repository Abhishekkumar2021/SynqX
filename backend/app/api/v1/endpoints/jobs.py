...
def get_job(
    ...
    service = JobService(db)
    job = service.get_job(
        job_id, user_id=current_user.id, workspace_id=current_user.active_workspace_id
    )
...
def get_job_run(
    ...
    from synqx_core.models.execution import Job, PipelineRun  # noqa: PLC0415
    from synqx_core.schemas.pipeline import PipelineVersionRead  # noqa: PLC0415

    # Check job ownership
    job_query = db.query(Job).filter(Job.id == job_id)
    if current_user.active_workspace_id:
        job_query = job_query.filter(
            Job.workspace_id == current_user.active_workspace_id
        )
    else:
    ...
    if not run:
        # If no run record yet, return a synthetic one based on the Job to allow progress display  # noqa: E501
    ...
    step_runs = service.get_run_steps(
        run.id, user_id=current_user.id, workspace_id=current_user.active_workspace_id
    )
...
def cancel_job(
    job_id: int,
    cancel_request: JobCancelRequest = Body(default=JobCancelRequest()),  # noqa: B008
    db: Session = Depends(deps.get_db),  # noqa: B008
    current_user: models.User = Depends(deps.get_current_user),  # noqa: B008
...
def retry_job(
    job_id: int,
    retry_request: JobRetryRequest = Body(default=JobRetryRequest()),  # noqa: B008
    db: Session = Depends(deps.get_db),  # noqa: B008
    current_user: models.User = Depends(deps.get_current_user),  # noqa: B008
    _: models.WorkspaceMember = Depends(deps.require_editor),  # noqa: B008
):
...
def get_job_logs(
    ...
    all_attempts: bool = Query(
        False, description="Include logs from all retry attempts"
    ),
    db: Session = Depends(deps.get_db),  # noqa: B008
    current_user: models.User = Depends(deps.get_current_user),  # noqa: B008
    _: models.WorkspaceMember = Depends(deps.require_viewer),  # noqa: B008
...
def list_runs(  # noqa: PLR0913
    pipeline_id: int | None = Query(None, description="Filter by pipeline"),
    status: PipelineRunStatus | None = Query(None, description="Filter by status"),  # noqa: B008
    limit: int = Query(100, ge=1, le=1000),
    offset: int = Query(0, ge=0),
    db: Session = Depends(deps.get_db),  # noqa: B008
    current_user: models.User = Depends(deps.get_current_user),  # noqa: B008
...
@router.get(
    ...
    description="Get detailed information about a specific pipeline run including step runs",  # noqa: E501
)
def get_run(
    run_id: int,
    db: Session = Depends(deps.get_db),  # noqa: B008
    ...
    service = PipelineRunService(db)
    run = service.get_run(
        run_id, user_id=current_user.id, workspace_id=current_user.active_workspace_id
    )
...
@router.get(
    ...
    description="Download complete pipeline run details including context and steps as JSON",  # noqa: E501
)
def export_run(
    run_id: int,
    db: Session = Depends(deps.get_db),  # noqa: B008
...
def get_run_steps(
    ...
    try:
        ...
        raise HTTPException(  # noqa: B904
            ...
            detail={
                "error": "Internal server error",
                "message": "Failed to get step runs",
            },
        )
...
def get_step_logs(
    ...
    try:
        ...
        raise HTTPException(  # noqa: B904
            ...
            detail={
                "error": "Internal server error",
                "message": "Failed to get step logs",
            },
        )
...
def get_step_data(  # noqa: PLR0913
    run_id: int,
    step_id: int,
    ...
    try:
        from synqx_core.models.execution import PipelineRun, StepRun  # noqa: PLC0415

        from app.engine.agent_core.forensics import ForensicSniffer  # noqa: PLC0415
        ...
        sniffer.base_dir = os.path.join(
            project_root, ".synqx", "forensics", f"run_{run_id}"
        )

        # We use node.id (the integer from pipeline_nodes) which is stored in step.node_id  # noqa: E501
        logger.debug(
            f"Fetching forensic data for node {step.node_id}, run {run_id}, direction {direction}"  # noqa: E501
        )
        data_slice = sniffer.fetch_slice(
            step.node_id, direction=direction, limit=limit, offset=offset
        )
        ...
        logger.error(
            f"Error fetching step data for run {run_id}, step {step_id}: {e}",
            exc_info=True,
        )
        raise HTTPException(  # noqa: B904
            status_code=500, detail=f"Failed to fetch forensic data: {e!s}"
        )
...
def clear_forensic_cache(
    ...
    if not current_user.is_superuser:
        raise HTTPException(
            status_code=403, detail="Only superusers can clear forensic cache"
        )

    try:
        from app.engine.runner_core.forensics import ForensicSniffer  # noqa: PLC0415
        ...
        raise HTTPException(status_code=500, detail="Failed to clear forensic cache")  # noqa: B904
...
def list_quarantine(  # noqa: PLR0912
    ...
    try:
                ...
                if filename.endswith("_quarantine.parquet"):
                    try:
                        ...
                        quarantine_candidates.append(
                            (run_id, node_id, os.path.join(run_path, filename))
                        )
        ...
        for run_id, node_id, file_path in quarantine_candidates:
            ...
            if step:
                # Prioritize database records_error count for consistency with aggregate stats  # noqa: E501
                ...
                results.append(
                    {
                        ...
                        "pipeline_name": step.pipeline_run.pipeline.name
                        if step.pipeline_run.pipeline
                        else "Unknown",
                        "node_id": node_id,
