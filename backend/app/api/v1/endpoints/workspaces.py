...
class WorkspaceCreate(BaseModel):
    ...
    default_agent_group: str | None = None
    git_config: dict[str, Any] | None = None
...
class WorkspaceUpdate(BaseModel):
    ...
    default_agent_group: str | None = None
    git_config: dict[str, Any] | None = None
    clear_all_pipelines: bool | None = False
...
def update_workspace(
    ...
    db: Session = Depends(deps.get_db),  # noqa: B008
    current_user: User = Depends(deps.get_current_user),  # noqa: B008
    _: WorkspaceMember = Depends(deps.require_admin),  # noqa: B008
):
    ...
    if request.default_agent_group is not None:
        ...
        "default_agent_group" in request.model_fields_set
        and request.default_agent_group is None
    ):
    ...
    if request.clear_all_pipelines or not is_remote_group(request.default_agent_group):
        ...
        db.query(Pipeline).filter(Pipeline.workspace_id == workspace_id).update(
            {Pipeline.agent_group: "internal"}, synchronize_session=False
        )
...
def create_workspace(
    ...
    member = WorkspaceMember(
        workspace_id=ws.id, user_id=current_user.id, role=WorkspaceRole.ADMIN
    )
    db.add(member)
...
def list_workspace_members(
    ...
    db: Session = Depends(deps.get_db),  # noqa: B008
    current_user: User = Depends(deps.get_current_user),  # noqa: B008
    _: WorkspaceMember = Depends(deps.require_viewer),  # noqa: B008
):
...
def invite_member(
    ...
    member = WorkspaceMember(
        workspace_id=workspace_id, user_id=user.id, role=request.role
    )
    db.add(member)
    db.commit()
    db.refresh(member)
    ...
    AlertService.create_system_alert(
        db,
        workspace_id=workspace_id,
        message=f"User {user.email} was invited to the workspace by {current_user.email}.",  # noqa: E501
...
def remove_workspace_member(
    ...
    if user_id == current_user.id:
        raise HTTPException(
            status_code=400,
            detail="You cannot remove yourself from the workspace. Delete the workspace or have another admin remove you.",  # noqa: E501
        )
...
def export_workspace_context(
    ...
    filename = (
        f"synqx_workspace_{ws.slug}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
