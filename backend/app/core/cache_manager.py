...
class ResultCacheManager:
    ...
    @staticmethod
    def _get_cache_key(
        connection_id: int, query: str, limit: int, offset: int, params: dict | None
    ) -> str:
    ...
    @classmethod
    def set_cached_result(  # noqa: PLR0913
        cls,
        connection_id: int,
        query: str,
        limit: int,
        ...
        try:
            ...
            # 2. Store in Redis
            cache_payload = {"metadata": {"summary": summary}, "arrow_data": arrow_b64}

            cache.set(key, cache_payload, ttl=ttl)
            logger.debug(
                f"Cached result for connection {connection_id} ({len(results)} rows)"
            )
        except Exception as e:
