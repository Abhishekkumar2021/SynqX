...
class ParallelExecutionLayer:
    ...
    def __init__(self, max_workers: int | None = None):
        ...
        if not max_workers or max_workers == 0:
            ...
            logger.info(
                f"ParallelExecutionLayer initialized with auto-scaled {self.max_workers} workers (CPUs: {cpu_count})"  # noqa: E501
            )
        else:
            self.max_workers = max_workers
            logger.info(
                f"ParallelExecutionLayer initialized with {self.max_workers} workers"
            )

    def execute_layer(  # noqa: PLR0912, PLR0913, PLR0915
        self,
        ...
        for node in nodes:
            ...
            for edge in incoming_edges:
                ...
                if condition:
                    try:
                        # Simple evaluator for conditions like "inputs['prev'].count > 0"  # noqa: E501
                        if not self._evaluate_condition(condition, data_cache):
                            is_branch_active = False
                            break
                    except Exception as e:
                        logger.warning(
                            f"Failed to evaluate edge condition for node '{node.name}': {e}"  # noqa: E501
                        )
            ...
            if is_branch_active:
                ...
                DBLogger.log_job(
                    ...
                    job_id,
                    "INFO",
                    f"Branching: Skipping node '{node.name}' due to False edge condition.",  # noqa: E501
                )
                ...
                state_manager.update_step_status(
                    step_run,
                    OperatorRunStatus.SKIPPED,
                    message="Branch condition evaluated to False",
                )
        ...
        if len(executable_nodes) == 1:
            ...
            DBLogger.log_job(
                ...
                job_id,
                "DEBUG",
                f"Executing node '{node.name}' sequentially (no parallel siblings).",
            )
            try:
            ...
            DBLogger.log_job(
                ...
                job_id,
                "INFO",
                f"Initiating parallel execution for {len(nodes)} independent nodes using {actual_workers} worker threads.",  # noqa: E501
            )
            logger.info(
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                for future in concurrent.futures.as_completed(future_to_node):
                    ...
                    try:
                        ...
                        results[node.node_id] = result
                        logger.info(
                            f"[SUCCESS] Node '{node.name}' completed ({len(result)} chunks)"  # noqa: E501
                        )
                    except concurrent.futures.TimeoutError:
                        errors.append(
                            (
                                node.name,
                                Exception(f"Execution timeout reached ({timeout}s)"),
                            )
                        )
                        logger.error(
                            f"[FAILED] Node '{node.name}' timed out after {timeout}s"
                        )
                    except Exception as e:
                        errors.append((node.name, e))
                        logger.error(
        ...
        if errors:
            error_summary = "; ".join([f"Node '{name}': {e!s}" for name, e in errors])
            raise PipelineExecutionError(
                f"Stage execution failed. Component errors: {error_summary}"
            )
        ...
        try:
            ...
            if "count" in condition:
                ...
                if match:
                    ...
                    eval_expr = condition.replace(
                        f"inputs['{node_id}'].count", str(count)
                    )
    ...
    def _execute_single_node(  # noqa: PLR0913
        self,
        node: PipelineNode,
        pipeline_run: PipelineRun,
        data_cache: DataCache,
        ...
        if getattr(node, "is_dynamic", False) and node.mapping_expr:
            return self._execute_dynamic_node(
                node, pipeline_run, data_cache, dag, state_manager, job_id
            )
    ...
    def _execute_dynamic_node(  # noqa: PLR0913
        self,
        node: PipelineNode,
        pipeline_run: PipelineRun,
        data_cache: DataCache,
        ...
        try:
            ...
            if expr.startswith("[") and expr.endswith("]"):
                ...
                if match:
                    ...
                    if up_data:
                        ...
                        mapping_items = (
                            up_data[0].to_dict("records")
                            if not is_df_empty(up_data[0])
                            else []
                        )
            ...
            if not mapping_items:
                DBLogger.log_job(
                    ...
                    job_id,
                    "WARNING",
                    f"Dynamic node '{node.name}' mapping expression evaluated to an empty list. Skipping.",  # noqa: E501
                )
                return []
            ...
            DBLogger.log_job(
                ...
                job_id,
                "INFO",
                f"Dynamic Fan-out: Node '{node.name}' spawning {len(mapping_items)} parallel instances.",  # noqa: E501
            )
        except Exception as e:
            raise PipelineExecutionError(  # noqa: B904
                f"Failed to evaluate mapping expression for node '{node.name}': {e}"
            )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            def run_instance(item):
                # For prototype, we'll assume the executor knows how to handle 'dynamic_item' in config  # noqa: E501
                instance_config = {**(node.config or {}), "_dynamic_item": item}
                # We need a way to pass this config...
                # Better: modify the _execute_node_with_retry to take an optional config override  # noqa: E501
                return self._execute_node_with_retry(
                    node,
                    pipeline_run,
                    data_cache,
            ...
            futures = [
                executor.submit(run_instance, it) for i, it in enumerate(mapping_items)
            ]
    ...
    def _execute_node_with_retry(  # noqa: PLR0913
        self,
        node: PipelineNode,
        pipeline_run: PipelineRun,
        data_cache: DataCache,
            ...
            try:
                with SessionLocal() as session:
                    ...
                    t_node = (
                        ...
                        .filter(PipelineNode.id == node.id)
                        .first()
                    )

                    if not t_pipeline_run or not t_node:
                        raise PipelineExecutionError(
                            f"Failed to load pipeline run or node '{node.name}' in thread"  # noqa: E501
                        )
                    ...
                    for uid in upstream_ids:
                        ...
                        logger.debug(
                            f"Node '{node.name}' loaded {len(chunks)} chunks from '{uid}'"  # noqa: E501
                        )

                    # Execute node
                    logger.info(
                        f"→ Executing node '{node.name}' (type: {node.operator_type.value}, attempt: {attempt + 1}/{max_retries + 1})"  # noqa: E501
                    )
                    ...
                    if attempt > 0:
                        DBLogger.log_job(
                            ...
                            job_id,
                            "WARNING",
                            f"Retrying node '{node.name}' (Attempt {attempt + 1}/{max_retries + 1})",  # noqa: E501
                        )

                    results, quality_profile = t_node_executor.execute(  # noqa: RUF059
                        t_pipeline_run, t_node, input_data
                    )
                ...
                if isinstance(
                    e, (ConfigurationError, ValueError, PipelineExecutionError)
                ):
                    logger.error(
                        f"Node '{node.name}' failed with non-retryable error: {e}"
                    )
                    raise e

                if attempt > max_retries:
                    logger.error(
                        f"Node '{node.name}' failed after {attempt} attempts: {e}"
                    )
                    raise e
...
class PipelineAgent:
    ...
    def __init__(
        ...
        max_cache_memory_mb: int | None = None,
    ):
    ...
    def _build_dag(self, pipeline_version: PipelineVersion) -> DAG:
        ...
        logger.info(
            f"Building DAG: {len(pipeline_version.nodes)} nodes, {len(pipeline_version.edges)} edges"  # noqa: E501
        )
        ...
        for edge in pipeline_version.edges:
            ...
            if from_node.node_id == to_node.node_id:
                raise ConfigurationError(
                    f"Self-loop detected: node '{from_node.node_id}' cannot connect to itself"  # noqa: E501
                )
        ...
        try:
            ...
            logger.info(
                f"[SUCCESS] DAG validation successful (topological order: {len(order)} nodes)"  # noqa: E501
            )
    ...
    def _cleanup_cache(
        ...
        if cache_stats["utilization_pct"] > 75:  # noqa: PLR2004
            logger.warning(
                f"High cache utilization: {cache_stats['utilization_pct']:.1f}% "
                f"({cache_stats['memory_mb']:.2f}MB / {cache_stats['memory_limit_mb']}MB)"  # noqa: E501
            )
    ...
    def run(self, pipeline_version: PipelineVersion, db: Session, job_id: int) -> None:  # noqa: PLR0915
        ...
        logger.info(
            f"Nodes: {len(pipeline_version.nodes)}, Max Parallel: {self.max_parallel_nodes}"  # noqa: E501
        )
        logger.info("=" * 80)
        ...
        DBLogger.log_job(
            ...
            job_id,
            "INFO",
            f"Execution lifecycle initiated. Orchestrating {len(pipeline_version.nodes)} logical processing nodes. "  # noqa: E501
        ...
        try:
            ...
            DBLogger.log_job(
                ...
                job_id,
                "DEBUG",
                "Analyzing pipeline topology and constructing Directed Acyclic Graph (DAG)...",  # noqa: E501
            )
            ...
            DBLogger.log_job(
                ...
                job_id,
                "INFO",
                "Performing static analysis and ELT pushdown optimization...",
            )
            ...
            DBLogger.log_job(
                ...
                job_id,
                "DEBUG",
                "Resolving stream dependencies and calculating optimal execution stages...",  # noqa: E501
            )
            ...
            DBLogger.log_job(
                ...
                job_id,
                "INFO",
                f"Static analysis finalized. Orchestration plan contains {len(layers)} sequential stages. "  # noqa: E501
            ...
            for layer_idx, layer_nodes in enumerate(layers, 1):
                ...
                if (
                    ...
                    and pipeline_version.pipeline.execution_timeout_seconds
                ):
                    elapsed = (
                        datetime.now(UTC) - self.metrics.execution_start
                    ).total_seconds()
                    if elapsed > pipeline_version.pipeline.execution_timeout_seconds:
                        timeout_msg = f"Orchestration aborted: Pipeline execution exceeded global timeout limit of {pipeline_version.pipeline.execution_timeout_seconds}s."  # noqa: E501
                ...
                DBLogger.log_job(
                    ...
                    job_id,
                    "INFO",
                    f"Stage {layer_idx}/{len(layers)}: Initiating synchronized processing for nodes: [{', '.join(node_ids)}].",  # noqa: E501
                )
                ...
                for node_id, results in layer_results.items():
                    ...
                    DBLogger.log_job(
                        ...
                        job_id,
                        "INFO",
                        f"Node '{node_id}' finalized. [Processed {records:,} records in {len(results)} chunks]",  # noqa: E501
                        source="engine",
                    )
                ...
                DBLogger.log_job(
                    ...
                    job_id,
                    "INFO",
                    f"Stage {layer_idx} finalized successfully. Processed {total_layer_records:,} records in {layer_duration:.2f}s. "  # noqa: E501
                    f"Total records in-flight: {self.metrics.total_records_processed:,} | Memory footprint: {cache_stats['utilization_pct']:.1f}%.",  # noqa: E501
                )
            ...
            summary = (
                ...
                f"Processed {self.metrics.total_records_processed:,} records across {self.metrics.completed_nodes} nodes with an average throughput of {self.metrics.throughput_records_per_sec:.2f} rec/s."  # noqa: E501
            )
            ...
            error_msg = (
                f"Orchestration halted due to a terminal error after {self.metrics.duration_seconds:.2f}s "  # noqa: E501
                f"({self.metrics.completed_nodes}/{self.metrics.total_nodes} nodes finalized). "  # noqa: E501
                f"Terminal fault: {e!s}"
            )
