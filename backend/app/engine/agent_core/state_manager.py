...
from synqx_core.models.enums import OperatorRunStatus, OperatorType, PipelineRunStatus
...
class StateManager:
    ...
    def initialize_run(
        self, pipeline_id: int, version_id: int, total_nodes: int = 0
    ) -> PipelineRun:
        """
        ...
        """
        pipeline_run = (
            self.db.query(PipelineRun).filter(PipelineRun.job_id == self.job_id).first()
        )
        ...
        if pipeline_run:
            ...
            if not job:
                raise Exception(
                    f"Job {self.job_id} not found during run initialization"
                )
        ...
        DBLogger.log_job(
            ...
            self.job_id,
            "INFO",
            f"Pipeline run initialized. Run number: {pipeline_run.run_number} | Stage count: {total_nodes}.",  # noqa: E501
        )
    ...
    def create_step_run(
        self, pipeline_run_id: int, node_id: int, operator_type: str, order_index: int
    ) -> StepRun:
    ...
    def update_step_status(  # noqa: PLR0912, PLR0913, PLR0915
        self,
        step_run: StepRun,
        ...
        try:
            logger.debug(
                f"Updating step {step_run.id}: IN={records_in}, OUT={records_out}, ERR={records_error}"  # noqa: E501
            )
            # Re-fetch to ensure we are in the current session
            t_step_run = (
                self.db.query(StepRun).filter(StepRun.id == step_run.id).first()
            )
            ...
            if status == OperatorRunStatus.RUNNING and not t_step_run.started_at:
                t_step_run.started_at = datetime.now(UTC)
                DBLogger.log_step(
                    self.db,
                    t_step_run.id,
                    "INFO",
                    f"Step '{t_step_run.node.name if t_step_run.node else t_step_run.node_id}' execution started",  # noqa: E501
                    job_id=self.job_id,
                )

            if status in [OperatorRunStatus.SUCCESS, OperatorRunStatus.FAILED]:
                t_step_run.completed_at = datetime.now(UTC)
                ...
                if status == OperatorRunStatus.SUCCESS:
                    DBLogger.log_step(
                        ...
                        t_step_run.id,
                        "INFO",
                        f"Step '{t_step_run.node.name if t_step_run.node else t_step_run.node_id}' completed successfully",  # noqa: E501
                        job_id=self.job_id,
                    )
            ...
            if quality_profile is not None:
                t_step_run.quality_profile = sanitize_for_json(quality_profile)
            ...
            if error:
                ...
                DBLogger.log_step(
                    ...
                    t_step_run.id,
                    "ERROR",
                    f"Step '{t_step_run.node.name if t_step_run.node else t_step_run.node_id}' failed: {error!s}",  # noqa: E501
                    job_id=self.job_id,
                )
                ...
                DBLogger.log_step(
                    ...
                    t_step_run.id,
                    "ERROR",
                    f"Step '{t_step_run.node.name if t_step_run.node else t_step_run.node_id}' failed with {error_msg}",  # noqa: E501
                    job_id=self.job_id,
                )
            ...
            if is_terminal:
                ...
                t_pipeline_run = (
                    ...
                    .filter(PipelineRun.id == t_step_run.pipeline_run_id)
                    .first()
                )
                ...
                if t_pipeline_run:
                    ...
                    fresh_steps = (
                        ...
                        .filter(StepRun.pipeline_run_id == t_pipeline_run.id)
                        .all()
                    )
                    ...
                    failed_steps = len(
                        [s for s in fresh_steps if s.status == OperatorRunStatus.FAILED]
                    )
            ...
            telemetry_payload = {
                ...
                "completed_at": t_step_run.completed_at.isoformat()
                if t_step_run.completed_at
                else None,
                ...
                telemetry_payload.update(
                    {
                        ...
                        if t_pipeline_run.total_nodes > 0
                        else 0,
                    }
                )
    ...
    def complete_run(self, pipeline_run: PipelineRun):
        ...
        try:
            ...
            t_run.completed_at = datetime.now(UTC)

            if t_run.started_at:
                t_run.duration_seconds = (
                    t_run.completed_at - t_run.started_at
                ).total_seconds()

            # Recalculate aggregates
            fresh_steps = (
                self.db.query(StepRun).filter(StepRun.pipeline_run_id == t_run.id).all()
            )
            t_run.total_extracted = sum(
                s.records_out
                for s in fresh_steps
                if s.operator_type == OperatorType.EXTRACT
            )
            t_run.total_loaded = sum(
                s.records_out
                for s in fresh_steps
                if s.operator_type == OperatorType.LOAD
            )
            ...
            completed_steps = len(
                [s for s in fresh_steps if s.status == OperatorRunStatus.SUCCESS]
            )
            ...
            DBLogger.log_job(
                ...
                self.job_id,
                "SUCCESS",
                f"Pipeline orchestration completed. Run #{t_run.run_number} finalized successfully.",  # noqa: E501
            )
            ...
            self._broadcast_telemetry(
                {
                    ...
                    "completed_steps": completed_steps,
                    "progress_pct": (completed_steps / t_run.total_nodes * 100)
                    if t_run.total_nodes > 0
                    else 0,
                }
            )
    ...
    def fail_run(self, pipeline_run: PipelineRun, error: Exception):
        ...
        try:
            ...
            DBLogger.log_job(
                ...
                self.job_id,
                "ERROR",
                f"Pipeline orchestration aborted. Run #{t_run.run_number} failed: {error!s}",  # noqa: E501
            )
    ...
    def retry_step(self, step_run: StepRun):
        ...
        with SessionLocal() as db:
            ...
            DBLogger.log_step(
                ...
                step_run.id,
                "WARNING",
                f"Retrying task (Attempt #{t_step.retry_count}). Previous execution attempt failed.",  # noqa: E501
            )
