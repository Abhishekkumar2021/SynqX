...
class EphemeralJobService:
    @staticmethod
    def create_job(
        db: Session, workspace_id: int, user_id: int, data: EphemeralJobCreate
    ) -> EphemeralJob:
        ...
        if is_remote:
            if not AgentService.is_group_active(db, workspace_id, data.agent_group):
                raise AppError(
                    f"No active agents found in group '{data.agent_group}'. Please ensure your remote agent is running."  # noqa: E501
                )
        ...
        if not is_remote:
            from app.worker.tasks import process_internal_ephemeral_job  # noqa: PLC0415
    ...
    def update_job(db: Session, job_id: int, data: EphemeralJobUpdate) -> EphemeralJob:
        ...
        if data.result_sample_arrow:
            try:
                ...
                logger.error(
                    f"Failed to deserialize Arrow payload for job {job_id}: {e}"
                )
        ...
        # PERFORMANCE: Sanitize JSON fields to handle Timestamps/datetimes from Arrow or other sources  # noqa: E501
        # This prevents 'Object of type datetime is not JSON serializable' errors during db.commit()  # noqa: E501
        if "result_sample" in update_data:
            update_data["result_sample"] = sanitize_for_json(
                update_data["result_sample"]
            )
        if "result_summary" in update_data:
            update_data["result_summary"] = sanitize_for_json(
                update_data["result_summary"]
            )

        for field, value in update_data.items():
            setattr(job, field, value)

        if data.status == JobStatus.SUCCESS or data.status == JobStatus.FAILED:  # noqa: PLR1714
    ...
    @staticmethod
    def list_jobs(
        db: Session, workspace_id: int, job_type: JobType | None = None, limit: int = 50
    ) -> list[EphemeralJob]:
