        ...
        <div className="flex flex-col gap-6 p-1 relative text-foreground">
            ...
            <div className="flex items-center justify-between px-2 mb-2">
                <div className="flex flex-col">
                    <span className="font-black text-xs tracking-[0.2em] leading-none uppercase text-muted-foreground/60 flex items-center gap-2">
                ...
                <div className="flex p-1 bg-muted/20 backdrop-blur-md rounded-xl border border-border/20 shadow-inner">
                    {(['preset', 'custom'] as const).map((m) => (
                        <button
                            ...
                            className={cn(
                                "px-4 py-1.5 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all duration-500",
                                mode === m
                ...
                {mode === 'preset' && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 animate-in fade-in slide-in-from-left-4 duration-500">
                        {PRESETS.map((preset) => (
                            ...
                            >
                                <div className="flex justify-between w-full mb-1 relative z-10">
                                    <span className={cn("text-[10px] font-black uppercase tracking-widest", value === preset.value ? "text-primary" : "text-foreground/70")}>
                                        {preset.label}
                                    </span>
                                    {value === preset.value && (
                                ...
                                <span className="text-[9px] text-muted-foreground/60 font-bold tracking-tight mb-3 line-clamp-1 relative z-10 uppercase">{preset.desc}</span>
                                <code className={cn(
                                    "text-[9px] px-2 py-0.5 rounded-md font-mono font-black transition-all relative z-10 tracking-widest",
                                    value === preset.value ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground/60"
                                )}>
                                    {preset.value}
                                </code>
                            </button>
                        ...
                        >
                            ...
                            <span className="text-[9px] font-black uppercase tracking-[0.2em] opacity-60">Manual Config</span>
                        </button>
                    </div>
                )}
                ...
                {mode === 'custom' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                        <div className="space-y-2">
                            <Label className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground/40 ml-1">Precision Expression</Label>
                        ...
                        <div className="rounded-2xl bg-muted/5 border border-border/20 p-4 shadow-sm relative overflow-hidden">
                            <div className="flex items-center gap-2 mb-3 text-muted-foreground/40 relative z-10">
                                ...
                                <span className="text-[8px] font-black uppercase tracking-[0.3em]">Quick Reference</span>
                            </div>
                            <div className="grid grid-cols-4 gap-4 text-[9px] font-bold text-muted-foreground/60 relative z-10">
                                {[['*', 'All'], [',', 'List'], ['-', 'Range'], ['/', 'Step']].map(([s, l]) => (
                                    <div key={s} className="flex flex-col items-center gap-1">
                                        <code className="text-primary font-black bg-primary/10 px-1.5 py-0.5 rounded text-[10px]">{s}</code>
                            ...
                            <div className="flex items-center gap-3 overflow-hidden">
                                ...
                                <span className="text-[8px] font-black uppercase tracking-[0.3em] text-muted-foreground/40 shrink-0">Status</span>
                                ...
                                <span className={cn(
                                    ...
                                    isValid ? "text-foreground/80" : "text-red-500/60 italic"
                            ...
                            {isValid && (
                                ...
                                <Badge variant="outline" className="h-5 text-[7px] font-black border-emerald-500/20 text-emerald-500 bg-emerald-500/10 tracking-widest uppercase rounded-md px-1.5">Active</Badge>
            ...
            const CronSlot = ({ label, value }: { label: string, value: string }) => (
                ...
                <div className="flex flex-col items-center gap-1.5 group">
                    ...
                    <div className="w-full aspect-square flex items-center justify-center rounded-xl bg-background/40 border border-border/20 shadow-sm font-mono text-xs font-black text-foreground/80 transition-all group-hover:border-primary/40 group-hover:shadow-md ring-1 ring-border/5 backdrop-blur-sm group-hover:scale-105 duration-300">

                        {value}

                    </div>

                    <span className="text-[7px] uppercase font-black text-muted-foreground/30 tracking-[0.2em]">{label}</span>
