        ...
        <div className="flex flex-col gap-6 p-1 relative text-foreground">
                ...
                {mode === 'preset' && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 animate-in fade-in slide-in-from-left-4 duration-500">
                        {PRESETS.map((preset) => (
                            ...
                            >
                                <div className="flex justify-between w-full mb-1 relative z-10">
                                    <span className={cn("text-[10px] font-bold uppercase tracking-widest", value === preset.value ? "text-primary" : "text-foreground/70")}>
                                        {preset.label}
                                    </span>
                                    {value === preset.value && (
                        ...
                        >
                            ...
                            <span className="text-[9px] font-bold uppercase tracking-[0.2em] opacity-60">Manual Config</span>
                        </button>
                    </div>
                )}
                ...
                {mode === 'custom' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                        <div className="space-y-2">
                            <Label className="text-[9px] font-bold uppercase tracking-[0.2em] text-muted-foreground/40 ml-1">Precision Expression</Label>
                        ...
                        <div className="rounded-2xl bg-muted/5 border border-border/20 p-4 shadow-sm relative overflow-hidden">
                            <div className="flex items-center gap-2 mb-3 text-muted-foreground/40 relative z-10">
                                ...
                                <span className="text-[8px] font-bold uppercase tracking-[0.3em]">Quick Reference</span>
                            </div>
                            <div className="grid grid-cols-4 gap-4 text-[9px] font-bold text-muted-foreground/60 relative z-10">
                                {[['*', 'All'], [',', 'List'], ['-', 'Range'], ['/', 'Step']].map(([s, l]) => (
                                    <div key={s} className="flex flex-col items-center gap-1">
                                        <code className="text-primary font-bold bg-primary/10 px-1.5 py-0.5 rounded text-[10px]">{s}</code>
                            ...
                            <div className="flex items-center gap-3 overflow-hidden">
                                ...
                                <span className="text-[8px] font-bold uppercase tracking-[0.3em] text-muted-foreground/40 shrink-0">Status</span>
                            ...
                            {isValid && (
                                ...
                                <Badge variant="outline" className="h-5 text-[7px] font-bold border-emerald-500/20 text-emerald-500 bg-emerald-500/10 tracking-widest uppercase rounded-md px-1.5">Active</Badge>
            ...
            const CronSlot = ({ label, value }: { label: string, value: string }) => (
                ...
                <div className="flex flex-col items-center gap-1.5 group">
                    ...
                    <span className="text-[7px] uppercase font-bold text-muted-foreground/30 tracking-[0.2em]">{label}</span>
