...
export const SpatialMap: React.FC<SpatialMapProps> = ({ 
    ...
    return (
        <div className="w-full rounded-2xl overflow-hidden border border-border/40 shadow-xl relative group" style={{ height }}>
            <Map
                initialViewState={{
                    longitude: longitude,
                    latitude: latitude,
                ...
                {showPopup && (
                    ...
                    >
                        <div className="p-3 min-w-[150px]">
                            <h4 className="text-xs font-black uppercase tracking-tight text-foreground">{title || 'Data Asset'}</h4>
                            <p className="text-[10px] text-muted-foreground mt-1 leading-tight">{description || 'Spatial coordinate context'}</p>
                            <div className="mt-2 pt-2 border-t border-border/10 flex items-center gap-2">
                                <code className="text-[9px] font-mono opacity-40">{latitude.toFixed(4)}, {longitude.toFixed(4)}</code>
                            </div>
                        </div>
                    </Popup>
                )}
