          ...
          <div className="flex flex-wrap gap-1.5 min-h-[1.5rem]">
            {agent.tags?.groups?.map((g: string) => (
              <Badge
                key={g}
                variant="outline"
                className="text-[9px] font-bold uppercase tracking-widest bg-primary/5 border-primary/10 text-primary/70 px-2 py-0.5 rounded-md"
              >
                {g}
              </Badge>
            ))}
        ...
        <div className="col-span-12 md:col-span-4 flex items-center gap-4 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2">
              <Badge
                variant="outline"
                className="text-[9px] font-bold uppercase tracking-widest bg-muted/30 border-border/40 px-1.5 py-0 rounded-md"
              >
                {agent.client_id.substring(0, 8)}
              </Badge>
            </div>
        ...
        <div className="col-span-6 md:col-span-3 flex flex-wrap gap-1 border-l border-border/20 pl-4 items-center">
          {agent.tags?.groups?.slice(0, 2).map((g: string) => (
            <Badge
              key={g}
              variant="outline"
              className="text-[8px] font-bold uppercase bg-primary/5 border-primary/10 text-primary/70 px-1.5 py-0 rounded-md"
            >
              {g}
            </Badge>
          ))}
      ...
      <AnimatePresence mode="popLayout">
        {viewMode === 'grid' ? (
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
            {agents.map((agent) => (
              <AgentCard key={agent.id} agent={agent} onInspect={onInspect} onDelete={onDelete} />
            ))}
          </div>
        ) : (
          <div className="divide-y divide-border/30">
            {agents.map((agent) => (
              <AgentRow key={agent.id} agent={agent} onInspect={onInspect} />
            ))}
          </div>
        )}
      </AnimatePresence>
