...
// --- API Helpers ---
const createAgent = async (data: { name: string; tags: any }) =>
                ...
                <CommandEmpty className="p-0">
                  <div className="p-8 flex flex-col items-center gap-4 text-center">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <p className="text-[10px] font-medium text-muted-foreground/60 leading-relaxed max-w-[180px] mx-auto">
                        Create a unique processing label for this agent.
                      </p>
                    </div>
                    {inputValue.trim() && (
                      <Button
    ...
    <div className="flex flex-col h-full bg-background/95 backdrop-blur-3xl overflow-hidden text-foreground">
      <div
        className={cn(
          ...
          isNew ? 'bg-emerald-500/5' : 'bg-primary/5'
        )}
      >
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground font-medium mt-1.5 opacity-80">
                {isNew
                  ? 'Credentials generated. Save them before closing.'
                  : `Re-install agent for ${agentName}`}
              </p>
            </div>
          </div>
          <Badge
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-8 pt-6 space-y-8">
          ...
          <div className="flex items-center justify-between bg-emerald-500/5 border border-emerald-500/20 p-4 rounded-xl shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div className="space-y-0.5">
                <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-900/60 dark:text-emerald-400/60 leading-none">
                  Backup Identity
                </p>
                <p className="text-[11px] font-bold text-emerald-800/80 dark:text-emerald-500/80 leading-tight">
                  Save key for re-registration.
                </p>
              </div>
            </div>
            <Button
              ...
              variant="outline"
              className="h-9 rounded-lg text-[10px] font-bold uppercase border-emerald-500/20 bg-background hover:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 gap-2 shadow-sm shrink-0"
            >
          ...
          <div className="flex items-center justify-between bg-muted/20 p-4 rounded-xl border border-border/40 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-[11px] font-bold text-foreground leading-tight truncate max-w-[120px]">
                  OS: <span className="text-primary capitalize">{platform}</span>
                </p>
              </div>
            </div>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  className="h-9 rounded-lg font-bold text-[10px] uppercase gap-2 shadow-sm bg-background shrink-0 text-foreground border-border/40"
                >
              ...
              <DropdownMenuContent
                align="end"
                className="w-40 rounded-xl border-border/40 backdrop-blur-xl bg-background/95"
              >
                <DropdownMenuItem
                  onClick={() => setPlatform('macos')}
                  className="rounded-lg font-bold py-2 cursor-pointer text-xs"
                >
                  macOS
                </DropdownMenuItem>
                <DropdownMenuItem
        ...
        <div className="space-y-5">
          <div className="flex items-center gap-3">
            <div className="h-6 w-6 rounded-lg bg-primary/10 text-primary flex items-center justify-center text-[10px] font-bold shadow-sm border border-primary/20">
              2
            </div>
            <h3 className="font-bold text-sm tracking-widest uppercase opacity-80 text-foreground leading-none">
              Installation Path
            </h3>
          ...
          <Tabs value={installMethod} onValueChange={setInstallTab} className="w-full">
            <TabsList className="grid w-full grid-cols-2 bg-muted/30 border border-border/40 rounded-xl p-1 h-11">
              <TabsTrigger
                value="manual"
                className="gap-2 text-[11px] font-bold uppercase tracking-tight rounded-lg"
              >
            ...
            <div className="mt-6">
              ...
              >
                ...
                <div className="p-6 rounded-2xl border border-primary/20 bg-primary/5 dark:bg-primary/10 flex flex-col md:flex-row items-center justify-between gap-6 group hover:border-primary/40 transition-all shadow-sm">
                  <div className="flex items-center gap-5">
                    <div className="p-4 rounded-xl bg-background border border-primary/10 shadow-md group-hover:scale-110 transition-transform">
                      <ArrowDownToLine
                        className={cn('h-7 w-7 text-primary', isDownloading && 'animate-bounce')}
                      />
                    </div>
                    <div className="space-y-1">
                      ...
                      <p className="text-[11px] text-muted-foreground font-medium leading-relaxed max-w-70">
                        Self-contained installer with internal dependencies.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    <Select value={selectedVersion} onValueChange={setSelectedVersion}>
                      ...
                      <SelectContent
                        align="end"
                        className="w-48 rounded-xl border-border/40 backdrop-blur-xl bg-background/95"
                      >
                        <SelectItem
                          value="latest"
                        ...
                        {releases?.map((rel: any) => (
                          ...
                          >
                            v{rel.version}
                            {rel.version === '1.0.2' && (
                              <span className="ml-2 text-[8px] bg-primary/10 text-primary px-1.5 py-0.5 rounded-md inline-block">
                                New
                              </span>
                            )}
                          </SelectItem>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3 px-1">
                    ...
                    <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60 leading-none">
                      Setup Instructions
                    </h4>
                  </div>
                  <CodeBlock
                  ...
                  />
                  <p className="text-[10px] text-center text-muted-foreground opacity-60 font-medium uppercase tracking-tight">
              ...
              >
                <p className="text-[11px] font-medium text-muted-foreground px-1 leading-relaxed">
                  Isolated container execution. The preferred method for production servers.
                </p>
                <CodeBlock
                  language="bash"
                  code={dockerCommand}
                  ...
                  wrap
                  maxHeight="120px"
                  className="border-border/40 bg-muted/10 shadow-sm"
                />
              </TabsContent>
            </div>
          </Tabs>
        ...
        {isNew && (
          <Alert className="rounded-xl bg-amber-500/10 border-amber-500/20 py-4 px-6 border-dashed dark:bg-amber-500/5">
            ...
            <div className="text-[11px] text-amber-900 dark:text-amber-400 font-bold ml-2 leading-none">
              Awaiting initial heartbeat. Status will turn{' '}
              <span className="underline decoration-amber-500/40 underline-offset-4">online</span>{' '}
              automatically.
            </div>
          </Alert>
        )}
      ...
      <div className="p-6 border-t border-border/40 bg-muted/30 dark:bg-muted/10 flex items-center justify-between shrink-0 backdrop-blur-xl">
        ...
        <Button
          onClick={onClose}
          variant="ghost"
          className="rounded-xl font-bold h-10 px-6 hover:bg-background border border-border/40 transition-all text-foreground shadow-sm"
        >
      ...
      <DialogContent className="sm:max-w-4xl rounded-2xl p-0 overflow-hidden border-border/40 bg-background/95 shadow-2xl backdrop-blur-3xl ring-1 ring-white/10 dark:ring-white/5 max-h-[85vh] flex flex-col gap-0">
        ...
        {!generatedCreds ? (
          <motion.div
            ...
            className="p-10 space-y-10 overflow-y-auto custom-scrollbar bg-background text-foreground flex-1"
          >
              ...
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-muted-foreground mt-2 opacity-80">
                    Provision a new execution slot for your private environment.
                  </p>
                </div>
              </div>
            </DialogHeader>
            ...
            <div className="space-y-8 max-w-3xl">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                <div className="space-y-3">
                  <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground ml-1 leading-none">
                    Friendly Name
                  </label>
                  <Input
                ...
                <div className="p-6 rounded-2xl bg-muted/20 border border-border/40 space-y-3 group hover:bg-muted/30 transition-all shadow-sm">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <p className="text-[11px] text-muted-foreground font-medium leading-relaxed opacity-80">
                      Outbound-only connectivity via HTTPS. Zero firewall setup.
                    </p>
                  </div>
                </div>
                <div className="p-6 rounded-2xl bg-muted/20 border border-border/40 space-y-3 group hover:bg-muted/30 transition-all shadow-sm">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <p className="text-[11px] text-muted-foreground font-medium leading-relaxed opacity-80">
                      Deploy on Docker, Windows, or Linux. Joins global pool.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="pt-8 border-t border-border/40 flex justify-end">
              ...
              >
                {createMutation.isPending ? (
                  ...
                  <>
                    <Plus className="h-5 w-5" /> Generate Identity
                  </>
                )}
              </Button>
            </div>
