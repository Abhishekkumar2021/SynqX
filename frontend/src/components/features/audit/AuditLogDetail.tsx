                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <div className="flex items-center gap-2.5 mb-0.5">
                                    <DialogTitle className="text-lg font-bold tracking-tight uppercase ">{formatEventName(log.event_type)}</DialogTitle>
                                    <Badge className={cn("font-bold uppercase text-[8px] tracking-[0.15em] px-2 py-0.5 rounded-md shadow-sm", 
                ...
                <div className="p-6 space-y-6 max-h-[60vh] overflow-y-auto scrollbar-thin">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        <div className="p-3 rounded-2xl border border-border/40 bg-muted/5 flex flex-col gap-0.5">
                            <span className="text-[7px] font-bold uppercase tracking-widest text-muted-foreground/50">Actor ID</span>
                            <span className="text-[10px] font-mono font-bold text-foreground">USR-{log.user_id}</span>
                        </div>
                        <div className="p-3 rounded-2xl border border-border/40 bg-muted/5 flex flex-col gap-0.5">
                            <span className="text-[7px] font-bold uppercase tracking-widest text-muted-foreground/50">Trace ID</span>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="p-4 rounded-3xl border border-border/40 bg-muted/20 backdrop-blur-sm flex items-center gap-3 group/actor hover:border-primary/30 transition-colors">
                                ...
                                <div className="flex flex-col min-w-0">
                                    <span className="text-xs font-bold text-foreground truncate">{user?.full_name || 'System Principal'}</span>
                                    <span className="text-[10px] text-muted-foreground/60 font-medium truncate ">{user?.email || 'synqx-automated-service'}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="p-4 rounded-3xl border border-border/40 bg-muted/20 backdrop-blur-sm space-y-2 hover:border-primary/30 transition-colors">
                                <div className="flex items-center justify-between">
                                    ...
                                    <Badge variant="outline" className="text-[8px] font-bold bg-background/50 h-4 px-1.5">{log.target_type || 'Platform'}</Badge>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="text-[9px] font-bold text-muted-foreground/60 uppercase">Resource Pointer</span>
                                    <span className="text-[10px] font-mono font-bold text-foreground">{log.target_id ? `#${log.target_id}` : 'ROOT'}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between px-1">
                            ...
                            <Badge variant="outline" className="text-[7px] font-bold opacity-40 uppercase tracking-widest bg-muted/20 border-border/40 px-1.5 py-0">JSON Payload</Badge>
                        </div>
                        <CodeBlock 
