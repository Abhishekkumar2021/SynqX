    ...
    <>
      ...
      >
        ...
        <div className="col-span-12 md:col-span-5 flex items-center gap-4">
          <div
            className={cn(
              'h-10 w-10 rounded-xl flex items-center justify-center shrink-0 shadow-sm ring-1 ring-white/5 transition-transform group-hover:scale-110',
              eventInfo.bgColor,
              eventInfo.color
            )}
          >
          ...
          <div className="flex flex-col min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              ...
              {log.status !== 'success' && (
                <Badge
                  variant="destructive"
                  className="h-4 px-1.5 text-[8px] font-bold uppercase tracking-widest ring-1 ring-destructive/20"
                >
                  {log.status}
                </Badge>
              )}
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex items-center gap-3 max-w-full">
                  ...
                  <div className="flex flex-col min-w-0">
                    <span className="text-[10px] font-bold text-foreground truncate leading-none mb-0.5">
                      {user?.full_name || 'System'}
                    </span>
                    <span className="text-[8px] text-muted-foreground font-medium truncate opacity-50  uppercase tracking-wider">
                      {user?.email || 'automated'}
                    </span>
                  </div>
                </div>
              </TooltipTrigger>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    <p className="text-xs font-bold text-foreground">
                      {user?.full_name || 'System Service'}
                    </p>
                    <p className="text-[10px] text-muted-foreground font-medium">
                      {user?.email || 'synqx-internal'}
                    </p>
                  </div>
                </div>
              </TooltipContent>
          ...
          <div className="flex flex-col gap-1">
            {log.target_type ? (
              <div className="flex items-center gap-2">
                <Badge
                  variant="outline"
                  className="text-[8px] font-bold uppercase tracking-[0.15em] bg-muted/20 border-border/40 text-muted-foreground/70 px-2 py-0"
                >
                  {log.target_type}
                </Badge>
                <span className="text-[10px] font-mono font-bold text-primary/60 tracking-tighter">
                  ID: {log.target_id}
                </span>
              </div>
            ) : (
