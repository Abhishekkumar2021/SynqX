...
export const AssetFileExplorer: React.FC<AssetFileExplorerProps> = ({
  ...
  const getIcon = (item: FileSystemNode) => {
    if (item.type === 'directory')
      return <Folder className="h-4 w-4 text-blue-500 fill-blue-500/20" />
  ...
  const isAllSelected =
    allFilesUnderCurrent.length > 0 &&
    selectedFilesUnderCurrent.length === allFilesUnderCurrent.length
  const isIndeterminate =
    selectedFilesUnderCurrent.length > 0 &&
    selectedFilesUnderCurrent.length < allFilesUnderCurrent.length
      ...
      <div className="flex flex-col h-full border-none rounded-none overflow-hidden bg-background/30 backdrop-blur-xl">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 p-4 bg-muted/20 border-b border-border/40 shrink-0">
          ...
          <div className="flex items-center gap-0.5 overflow-x-auto no-scrollbar flex-1 px-2 py-1 bg-background/50 rounded-lg border border-border/40 min-h-9">
            <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/40 px-2 shrink-0">
              Path:
            </span>
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              >
                <DropdownMenuLabel className="text-[10px] font-bold uppercase tracking-widest px-2 py-1.5 opacity-50">
                  Sort By
                </DropdownMenuLabel>
                <DropdownMenuItem
                  onClick={() => setSortField('name')}
                  className="rounded-lg text-xs font-bold gap-2.5"
                >
                  <FileText
                    className={cn('h-4 w-4', sortField === 'name' ? 'text-primary' : 'opacity-40')}
                  />
                  Name
                  {sortField === 'name' &&
                    (sortOrder === 'asc' ? (
                      <SortAsc className="h-3 w-3 ml-auto opacity-40" />
                    ) : (
                      <SortDesc className="h-3 w-3 ml-auto opacity-40" />
                    ))}
                </DropdownMenuItem>
                <DropdownMenuItem
                ...
                >
                  <Hash
                    className={cn('h-4 w-4', sortField === 'size' ? 'text-primary' : 'opacity-40')}
                  />
                  Size
                  {sortField === 'size' &&
                    (sortOrder === 'asc' ? (
                ...
                <DropdownMenuItem
                  onClick={() => setSortField('format')}
                  className="rounded-lg text-xs font-bold gap-2.5"
                >
                  <Database
                    className={cn(
                      'h-4 w-4',
                      sortField === 'format' ? 'text-primary' : 'opacity-40'
                    )}
                  />
                  Format
                  {sortField === 'format' &&
                ...
                >
                  <SortAsc
                    className={cn('h-4 w-4', sortOrder === 'asc' ? 'text-primary' : 'opacity-40')}
                  />
                  Ascending
                </DropdownMenuItem>
                <DropdownMenuItem
                ...
                >
                  <SortDesc
                    className={cn('h-4 w-4', sortOrder === 'desc' ? 'text-primary' : 'opacity-40')}
                  />
                  Descending
                </DropdownMenuItem>
                ...
                >
                  <Layers
                    className={cn('h-4 w-4', groupBy === 'type' ? 'text-primary' : 'opacity-40')}
                  />
                  File Type
                </DropdownMenuItem>
                <DropdownMenuItem
                ...
                >
                  <X
                    className={cn('h-4 w-4', groupBy === 'none' ? 'text-primary' : 'opacity-40')}
                  />
                  None
                </DropdownMenuItem>
              </DropdownMenuContent>
              ...
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-center opacity-50">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    This directory is empty or doesn't match filters.
                  </p>
                </div>
              ) : viewMode === 'list' ? (
                <Table wrapperClassName="rounded-none border-none">
                  <TableHeader className="bg-muted/10 sticky top-0 z-30 backdrop-blur-md border-b border-border/40">
                    <TableRow className="hover:bg-transparent border-none">
                      ...
                      <TableHead className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60 text-right pr-6">
                        Size
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                    ...
                    {items.map((item) => (
                      <TableRow
                        ...
                        className={cn(
                          ...
                          item.type === 'directory'
                            ? 'cursor-pointer hover:bg-primary/3'
                            : 'hover:bg-muted/30'
                        )}
                      ...
                      >
                                  ...
                                  >
                                    <div
                                      className={cn(
                                        ...
                                        getAllFilesUnderNode(item).length > 0 &&
                                          getAllFilesUnderNode(item).every((f) =>
                                            selectedAssets.has(f)
                                          ) &&
                                    ...
                                    >
                                      <div
                                        className={cn(
                                          ...
                                          getAllFilesUnderNode(item).some((f) =>
                                            selectedAssets.has(f)
                        ...
                        <TableCell className="text-center py-2">
                          {item.type === 'directory' ? (
                            <Badge
                              variant="outline"
                              className="h-5 text-[8px] font-bold uppercase tracking-widest border-blue-500/20 text-blue-600 dark:text-blue-400 bg-blue-500/5"
                            >
                              FOLDER
                            </Badge>
                          ) : (
                            <Badge
                              variant="outline"
                              className="h-5 text-[8px] font-bold uppercase tracking-widest bg-muted/50 border-border/40 text-muted-foreground/70"
                            >
                ...
                <div className="p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                  {items.map((item) => {
                    const isSelected =
                      item.type === 'file'
                        ? selectedAssets.has(item.asset.name)
                        : getAllFilesUnderNode(item).every((f) => selectedAssets.has(f))
                    const isPartial =
                      item.type === 'directory' &&
                      !isSelected &&
                      getAllFilesUnderNode(item).some((f) => selectedAssets.has(f))
                      ...
                      <motion.div
                        ...
                        onClick={() =>
                          item.type === 'directory'
                            ...
                            : onToggleAsset?.(item.asset.name, !isSelected)
                        }
                      >
                        {!readOnly && (
                          <div
                        ...
                        >
                          {React.cloneElement(
                            getIcon(item) as React.ReactElement<{ className?: string }>,
                            { className: 'h-8 w-8' }
                          )}
                        </div>
        ...
        <div className="bg-muted/10 border-t border-border/40 p-2 px-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-background/50 border border-border/40">
              ...
              <span className="text-[9px] font-bold uppercase tracking-tighter text-foreground/70">
                {items.length} Items
              </span>
            </div>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-7 w-7 rounded-lg hover:bg-background shadow-none opacity-40 hover:opacity-100 transition-opacity"
                >
              ...
              <TooltipContent
                side="top"
                className="text-[9px] font-bold uppercase tracking-widest p-2 bg-background border-border/60 shadow-xl"
              >
