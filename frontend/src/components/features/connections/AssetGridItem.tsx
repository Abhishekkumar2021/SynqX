...
export const AssetGridItem: React.FC<AssetGridItemProps> = ({ asset, connectionId }) => {
    ...
    >
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="ghost"
              size="icon"
              className="h-8 w-8 rounded-lg hover:bg-muted opacity-0 group-hover:opacity-100 transition-opacity"
            >
          ...
          <DropdownMenuContent align="end" className="w-48 rounded-xl border-border/60 shadow-xl">
            ...
            <DropdownMenuItem onClick={() => inferMutation.mutate()} className="gap-2 text-xs">
              <RefreshCw className={cn('h-3.5 w-3.5', inferMutation.isPending && 'animate-spin')} />{' '}
              Refresh Schema
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem
      ...
      <div className="grid grid-cols-3 gap-2 mt-auto">
        ...
        <div className="flex flex-col gap-0.5 p-2 rounded-lg bg-muted/30 border border-border/20">
          ...
          <span className="text-[10px] font-bold text-foreground tabular-nums truncate">
            {asset.size_bytes_estimate
              ? (asset.size_bytes_estimate / 1024).toFixed(1) + ' KB'
              : '—'}
          </span>
        </div>
      </div>
      ...
      <AlertDialog open={isDeleteAlertOpen} onOpenChange={setIsDeleteAlertOpen}>
        <AlertDialogContent className="rounded-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete the asset "{asset.name}"? This action cannot be
              undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <DialogDescription className="text-xs font-medium text-muted-foreground">
                    Exploration & Export Suite for {asset.name}
                  </DialogDescription>
                </div>
              </div>
              <Button
