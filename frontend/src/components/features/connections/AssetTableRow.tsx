...
export const AssetActions = ({ asset, connectionId }: { asset: Asset, connectionId: number }) => {
    ...
    const inferMutation = useMutation({
        ...
        onSuccess: (data) => {
            if (data.success) {
                toast.success("Schema Refreshed", { description: data.message || "Latest structure captured." });
            } else {
        ...
        onError: (err: any) => toast.error("Refresh Failed", { description: err.response?.data?.detail?.message || "Internal error." })
    });
        ...
        <>
            <div className="flex items-center justify-end gap-2">
                <Button
                    ...
                    onClick={(e) => { e.stopPropagation(); setIsSampleDialogOpen(true); }}
                ...
                <Button
                    ...
                    size="sm"
                    className="h-8 w-8 p-0 hover:bg-background hover:text-primary hover:border hover:border-border"
                    onClick={(e) => { e.stopPropagation(); setIsSchemaDialogOpen(true); }}
                    title="View Schema"
                >
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-background hover:border hover:border-border" onClick={(e) => e.stopPropagation()}>
                    ...
                    <DropdownMenuContent align="end" className="w-48 rounded-xl border-border/60 shadow-xl backdrop-blur-md">
                        ...
                        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); setIsSampleDialogOpen(true); }}>
                        ...
                        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); inferMutation.mutate(); }} disabled={inferMutation.isPending}>
                            <RefreshCw className={cn("mr-2 h-3.5 w-3.5", inferMutation.isPending && "animate-spin")} />
                            Refresh Schema
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); setIsSchemaDialogOpen(true); }}>
...
export const AssetTableRow: React.FC<AssetTableRowProps & { minimal?: boolean }> = ({ asset, connectionId, minimal = false }) => {
    if (minimal) {
