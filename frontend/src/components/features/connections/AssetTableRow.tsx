...
export const AssetActions = ({ asset, connectionId }: { asset: Asset; connectionId: number }) => {
  ...
  const inferMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.success) {
        toast.success('Schema Refreshed', {
          description: data.message || 'Latest structure captured.',
        })
      } else {
        toast.warning('Refresh Incomplete', { description: data.message })
      }
    ...
    onError: (err: any) =>
      toast.error('Refresh Failed', {
        description: err.response?.data?.detail?.message || 'Internal error.',
      }),
  })
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="ghost"
              size="icon"
              className="h-8 w-8 hover:bg-background hover:border hover:border-border"
          ...
          <DropdownMenuContent
            align="end"
            className="w-48 rounded-xl border-border/60 shadow-xl backdrop-blur-md"
          >
            ...
            >
              <RefreshCw
                className={cn('mr-2 h-3.5 w-3.5', inferMutation.isPending && 'animate-spin')}
              />
              Refresh Schema
            </DropdownMenuItem>
            <DropdownMenuItem
        ...
        <AlertDialog open={isDeleteAlertOpen} onOpenChange={setIsDeleteAlertOpen}>
          <AlertDialogContent onClick={(e) => e.stopPropagation()}>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Are you sure you want to delete the asset "{asset.name}"? This will remove its
        ...
        <DialogContent
          className={cn(
            ...
            isMaximized
              ...
              : 'max-w-7xl h-[85vh] sm:rounded-[2rem]'
          )}
          onClick={(e) => e.stopPropagation()}
        >
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <DialogDescription className="text-xs font-medium text-muted-foreground flex items-center gap-2">
                    ...
                    <Badge
                      variant="outline"
                      className="h-5 text-[9px] font-bold bg-background/50 border-emerald-500/20 text-emerald-600 uppercase tracking-widest px-2"
                    >
                      {selectedRows.size > 0
                        ? `${selectedRows.size} SELECTED`
                        : `${sampleData?.count || 0} TOTAL`}
                    </Badge>
                  </DialogDescription>
                </div>
              </div>
...
export const AssetTableRow: React.FC<AssetTableRowProps & { minimal?: boolean }> = ({
  asset,
  connectionId,
  minimal = false,
}) => {
  if (minimal) {
    return <AssetActions asset={asset} connectionId={connectionId} />
  }
  ...
  return (
    <TableRow className="group transition-colors border-b border-border/40 hover:bg-muted/30">
      ...
      <TableCell className="px-6 py-2.5 capitalize text-muted-foreground text-xs font-medium">
        {asset.asset_type}
      </TableCell>
      <TableCell className="px-6 py-2.5">
        {asset.current_schema_version ? (
          <Badge
            variant="secondary"
            className="font-mono text-[10px] bg-muted text-muted-foreground border-border/50"
          >
      ...
      <TableCell className="text-right pr-6 py-2.5">
        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <AssetActions asset={asset} connectionId={connectionId} />
        </div>
      </TableCell>
    </TableRow>
  )
