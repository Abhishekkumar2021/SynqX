...
export const ConfigurationTabContent = ({
  ...
  const otherEntries = Object.entries(config).filter(
    ([key]) =>
      !connectivityFields.includes(key.toLowerCase()) &&
    ...
    <div className="h-full flex flex-col rounded-3xl border border-border/40 bg-background/40 backdrop-blur-xl shadow-xl overflow-hidden relative">
      ...
      <div className="p-5 md:p-6 border-b border-border/40 bg-muted/10 flex flex-col md:flex-row items-center justify-between shrink-0 gap-4">
        ...
        <div className="flex items-center gap-3">
          <Badge
            variant="outline"
            className="bg-primary/5 text-primary border-primary/20 tracking-widest px-3 py-1 font-bold rounded-lg text-[9px]"
          >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div className="lg:col-span-3 space-y-10">
              ...
              <div className="space-y-6">
                ...
                <div className="grid gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                  ...
                  {config.account && (
                    <ConfigField label="Account ID" value={config.account} copyable />
                  )}
              ...
              <div className="space-y-6">
                ...
                <div className="grid gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                  {config.database && (
                    <ConfigField label="Database" value={config.database} copyable />
                  )}
                  {config.database_path && (
                    <ConfigField label="File System Path" value={config.database_path} copyable />
                  )}
                  ...
                  {config.bucket && (
                    <ConfigField label="S3/GCS Bucket" value={config.bucket} copyable />
                  )}
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground/60 px-1">
                      Access Credentials
                    </label>
                    <div className="h-11 flex items-center justify-between px-4 rounded-xl border border-border/40 bg-muted/5 backdrop-blur-sm">
                      <div className="font-mono text-xs tracking-[0.4em] text-foreground/20">
                        ••••••••
                      </div>
                      <Badge
                        variant="outline"
                        className="text-[8px] bg-emerald-500/5 text-emerald-600 dark:text-emerald-500 border-emerald-500/20 font-bold px-1.5 py-0 uppercase"
                      >
                        Encrypted
                      </Badge>
                    </div>
                  ...
                  {config.username && (
                    <ConfigField label="Identity" value={config.username} copyable />
                  )}
                  <ConfigField
              ...
              <div className="space-y-6 pt-4">
                ...
                <div className="grid gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                  <ConfigField
                    label="Created On"
                    value={format(new Date(connection.created_at || ''), 'PPP')}
                  />
                  <ConfigField
                    label="Last Updated"
                    value={format(new Date(connection.updated_at || ''), 'PPP')}
                  />
            ...
            <div className="space-y-6 lg:border-l lg:border-border/20 lg:pl-8">
              <div className="space-y-2 mb-4">
                <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60 px-1">
                  Health & Activity
                </h4>
              </div>
              {/* Usage Stats Section */}
                ...
                <div className="p-5 space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="p-3 rounded-xl bg-muted/20 border border-border/30 text-center">
                        <div className="text-xl font-bold text-emerald-500">
                          {usageStats?.sync_success_rate
                            ? `${usageStats.sync_success_rate.toFixed(0)}%`
                            : '100%'}
                        </div>
                        <div className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider mt-0.5 opacity-60">
                          Success
                  ...
                  <div className="space-y-3 pt-1">
                    ...
                    <div className="flex items-center justify-between text-[10px] font-bold px-1">
                      <span className="text-muted-foreground/60 uppercase tracking-widest">
                        Avg Latency
                      </span>
                      <span className="text-foreground font-mono">
                        {usageStats?.average_latency_ms
                          ? `${usageStats.average_latency_ms.toFixed(0)}ms`
                          : 'N/A'}
                      </span>
                    </div>
                  </div>
