    ...
    const onSubmit = (data: FormValues) => {
        const payload: AssetBulkCreate = {
            assets: data.assets.filter(a => a.name.trim() !== '').map(asset => {
                ...
                if ((connectorType === ConnectorType.PROSOURCE || connectorType === ConnectorType.OSDU) && asset.raw_schema) {
                    try {
                        schemaMetadata = JSON.parse(asset.raw_schema);
                    } catch (e) {
                        toast.error("Schema Error", { description: `Invalid JSON schema for asset ${asset.name}.` });
                        return null; 
                    }
                }
        ...
        if (payload.assets.length === 0) {
            toast.error("Validation Error", { description: "Please provide at least one asset with valid configuration." });
            return;
        }
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <AnimatePresence>
                                                ...
                                                {(connectorType === ConnectorType.PROSOURCE && (assetType === AssetType.OSDU_KIND || assetType === AssetType.DOMAIN_ENTITY)) && (
                                                    <motion.div
                                                        initial={{ opacity: 0, height: 0 }}
                                                        animate={{ opacity: 1, height: 'auto' }}
                                                        exit={{ opacity: 0, height: 0 }}
                                                        ...
                                                        <div className="p-4">
                                                            <div className="bg-primary/5 border border-primary/10 rounded-[1.5rem] p-5 space-y-5 shadow-inner">
                                                                <div className="space-y-3">
                                                                    <div className="flex items-center justify-between px-1">
                                                                        <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground flex items-center gap-2">
                                                                            <Database className="h-3 w-3 text-primary" /> Raw Schema (JSON)
                                                                        </Label>
                                                                        <span className="text-[9px] font-bold text-primary/60 bg-primary/5 px-2 py-0.5 rounded-full border border-primary/10">
