...
import { createConnection, updateConnection, type ConnectionCreate, getConnections } from '@/lib/api';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Badge } from '@/components/ui/badge';
import {
    ShieldCheck, Lock, CheckCircle2, Server, ArrowLeft, Search, 
    Plus, Loader2, FileCode
} from 'lucide-react';
...
export const CreateConnectionDialog: React.FC<CreateConnectionDialogProps> = ({ 
    ...
    const [selectedType, setSelectedType] = useState<ConnectorType | null>(connectionToEdit?.connector_type || null);
    ...
    const stagingConnections = useMemo(() => {
        ...
        return allConnections.filter(c => ['s3', 'gcs', 'azure_blob'].includes(c.connector_type.toLowerCase()));
    }, [allConnections]);
    ...
    const mutation = useMutation({
        mutationFn: (data: ConnectionCreate) => 
            connectionToEdit 
                ? updateConnection(connectionToEdit.id, data) 
        ...
        onError: (err: any) => {
            toast.error("Operation Failed", { description: err.response?.data?.detail?.message || "An unexpected error occurred." });
        }
    });
    ...
    const filteredConnectors = Object.entries(CONNECTOR_TYPE_INFO).filter(([_, info]: [any, any]) => 
                        ...
                        <div className="space-y-1">
                            ...
                            <DialogDescription className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                                {step === 'type' ? (
                                    ...
                                    <>
                                        <Badge variant="outline" className="h-5 px-1.5 text-[8px] font-bold uppercase tracking-widest bg-background/50 border-primary/20 text-primary">{schema?.group}</Badge>
                            ...
                            <div className="px-10 py-6 border-b border-border/20 bg-muted/5 flex items-center justify-between gap-6 shrink-0">
                                <div className="relative flex-1 max-w-xl group">
                                    ...
                                    <div className="absolute right-5 top-4 flex items-center gap-1.5 opacity-40 group-focus-within:opacity-0 transition-opacity">
                                        <Badge variant="outline" className="h-5 px-1 rounded font-mono text-[9px]">⌘</Badge>
                                        <Badge variant="outline" className="h-5 px-1 rounded font-mono text-[9px]">K</Badge>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3">
                                    <Badge variant="outline" className="h-8 px-3 rounded-lg border-border/40 bg-background/50 font-bold uppercase tracking-widest text-[9px]">
                                ...
                                <div className="p-10 space-y-12">
                                    ...
                                    {!searchQuery && (
                                        <div className="space-y-6">
                                            <div className="flex items-center gap-4">
                                                <h3 className="text-xs font-black uppercase tracking-[0.3em] text-primary/80 whitespace-nowrap">Recommended</h3>
                                                    ...
                                                    .map(([type, info]) => (
                                                        ...
                                                        >
                                                            <div className={cn(
                                                                ...
                                                                <SafeIcon icon={React.createElement(info.icon)} className="h-5 w-5" />
                                                            </div>
                                                            <div className="min-w-0">
                                                                <h4 className="font-bold text-sm text-foreground truncate">{info.label}</h4>
                                                                <p className="text-[9px] text-muted-foreground font-medium uppercase tracking-wider">{info.group}</p>
                                                            </div>
                                                        </motion.button>
                                                    ))}
                                            </div>
                                    ...
                                    {Object.entries(groupedConnectors).map(([groupName, connectors]) => (
                                        <div key={groupName} className="space-y-6">
                                            <div className="flex items-center gap-4">
                                                <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground/70 whitespace-nowrap">{groupName}</h3>
                                                ...
                                                {connectors.map((c) => (
                                                    ...
                                                    >
                                                        <div className={cn(
                                                            ...
                                                            c.color || "bg-background border-border/40 group-hover:border-primary/20 group-hover:shadow-primary/5"
                                                        )}>
                                                        ...
                                                        <div className="space-y-1">
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                {c.popular && (
                                                                    <Badge className="h-4 px-1.5 text-[7px] font-black uppercase tracking-tighter bg-amber-500/10 text-amber-600 border-amber-500/20 hover:bg-amber-500/20">Popular</Badge>
                                                                )}
                                                            </div>
                        ...
                        <Form {...form}>
                            <form onSubmit={form.handleSubmit(onSubmit)} className="flex-1 flex flex-col min-h-0">
                                <ScrollArea className="flex-1">
                                    <div className="p-10 space-y-10">
                                        ...
                                        <div className="space-y-8">
                                            <div className="flex items-center gap-4">
                                                <div className="flex items-center justify-center w-8 h-8 rounded-xl bg-primary/10 text-primary text-xs font-black border border-primary/20 shadow-sm">1</div>
                                                <h4 className="text-[11px] font-black uppercase tracking-[0.3em] text-foreground/70">General Settings</h4>
                                        ...
                                        <div className="space-y-8">
                                            <div className="flex items-center justify-between gap-4">
                                                <div className="flex items-center gap-4 flex-1">
                                                    <div className="flex items-center justify-center w-8 h-8 rounded-xl bg-primary/10 text-primary text-xs font-black border border-primary/20 shadow-sm">2</div>
                                                    <h4 className="text-[11px] font-black uppercase tracking-[0.3em] text-foreground/70">Connection parameters</h4>
                                                    <div className="h-px flex-1 bg-linear-to-r from-border/60 to-transparent" />
                                                </div>
                                                <Badge variant="secondary" className="rounded-xl px-3 py-1 bg-primary/5 text-primary border border-primary/10 font-mono text-[10px] font-bold shadow-xs">
                                                ...
                                                {schema?.fields.map((field: any) => {
                                                    ...
                                                    return (
                                                        <FormField key={field.name} control={form.control} name={`config.${field.name}`} render={({ field: f }) => (
                                                            <FormItem className={cn("space-y-2", field.type === 'textarea' && 'col-span-full')}>
                                                                <Label className="text-xs font-bold uppercase tracking-widest text-muted-foreground ml-1 flex items-center justify-between">
                                                                    {field.label}
                                                                    {field.required && <span className="text-[8px] text-primary/60 bg-primary/5 px-1.5 rounded-sm border border-primary/10">REQUIRED</span>}
                                                                </Label>
                                                                <FormControl>
                                                                    {field.type === 'select' ? (
                                                                        <Select 
                                                                            onValueChange={(val) => {
                                                                                ...
                                                                                else if (!isNaN(Number(val)) && val.trim() !== '') f.onChange(Number(val));
                                                                                else f.onChange(val);
                                                                            }} 
                                                                        ...
                                                                        <div className="space-y-2">
                                                                            ...
                                                                            <div className="relative group min-h-[150px]">
                                                                                <CodeBlock
                                                                                    ...
                                                                                    language={field.language || (field.name.includes('json') ? 'json' : 'text')}
                                                                                    onChange={f.onChange}
                                                                                    editable
                                                                                    rounded
                                                                                    maxHeight="300px"
                                        ...
                                        {(selectedType === 'snowflake' || selectedType === 'bigquery') && (
                                            <div className="space-y-8">
                                                <div className="flex items-center gap-4">
                                                    <div className="flex items-center justify-center w-8 h-8 rounded-xl bg-amber-500/10 text-amber-500 text-xs font-black border border-amber-500/20 shadow-sm">3</div>
                                                    <h4 className="text-[11px] font-black uppercase tracking-[0.3em] text-amber-600/80">High Performance Staging</h4>
                                                        ...
                                                        <FormItem className="space-y-2">
                                                            ...
                                                            >
                                                                ...
                                                                <SelectContent className="rounded-2xl border-border/40 backdrop-blur-xl bg-background/95">
                                                                    <SelectItem value="none" className="rounded-xl">No Staging (Direct Load)</SelectItem>
                                                                    {stagingConnections?.map((conn: any) => (
                                                                        <SelectItem key={conn.id} value={String(conn.id)} className="rounded-xl">
                                                                            <div className="flex items-center gap-2">
                                                                                <Server size={14} className="text-muted-foreground" />
                                                                                <span>{conn.name}</span>
                                                                                <Badge variant="outline" className="text-[8px] h-4">{conn.connector_type}</Badge>
                                                                            </div>
                                                                        </SelectItem>
                                                                    ))}
                                                                </SelectContent>
                                ...
                                <DialogFooter className="p-10 border-t border-border/40 bg-muted/10 gap-4 shrink-0">
                                    ...
                                    >
                                        {mutation.isPending ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <CheckCircle2 className="mr-2 h-5 w-5" />}
