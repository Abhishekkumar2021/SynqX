        ...
        <div className="flex flex-col items-center justify-center h-full gap-4 text-center p-8 bg-muted/5 rounded-xl border border-dashed border-border/40">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground max-w-xs mx-auto">
              Initialize the isolated {language} environment to manage dependencies and execute
              scripts safely.
            </p>
          </div>
          <Button
          ...
          {envInfo.node_version && (
            ...
            >
              ...
              {initializedLangs.has('node') && (
                <Badge variant="secondary" className="ml-2 text-[9px] h-4 px-1">
                  {Object.keys(npmPackages).length}
                </Badge>
              )}
            </Button>
          )}
                  ...
                  {envInfo.python_version && (
                    <div className="p-4 rounded-2xl bg-muted/20 border border-border/30 flex flex-col gap-2">
                      ...
                      <span className="text-xl font-bold">
                        {envInfo.python_version.replace('Python ', '').split(' ')[0]}
                      </span>
                    </div>
                  )}
                  {envInfo.node_version && (
              ...
              renderTabContent(
                ...
                <div className="space-y-6">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-muted/10 p-4 rounded-2xl border border-border/40">
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-muted-foreground font-medium">
                        Add dependencies to your isolated Python environment.
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                    {Object.entries(installedPackages)
                      .sort((a, b) => a[0].localeCompare(b[0]))
                      .map(([pkg, ver]) => (
                        <div
                          key={pkg}
                          className="group flex items-center justify-between p-3 rounded-xl bg-background border border-border/40 hover:border-primary/40 hover:shadow-md transition-all"
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <Badge
                              variant="outline"
                              className="h-5 text-[9px] bg-muted/50 font-bold border-border/50"
                            >
                              pypi
                            </Badge>
                            <Button
                ...
                <div className="space-y-6">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-muted/10 p-4 rounded-2xl border border-border/40">
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-muted-foreground font-medium">
                        Add dependencies to your isolated Node.js environment.
                      </p>
                    </div>
                    <div className="flex items-center gap-2 w-full md:w-auto md:min-w-[300px]">
                      <PackageAutocomplete
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                    {Object.entries(npmPackages)
                      ...
                      .map(([pkg, ver]) => (
                        <div
                          key={pkg}
                          className="group flex items-center justify-between p-3 rounded-xl bg-background border border-border/40 hover:border-emerald-500/40 hover:shadow-md transition-all"
                        >
                          <div className="flex flex-col min-w-0">
                            <span className="text-xs font-bold truncate group-hover:text-emerald-600 transition-colors">
                              {pkg}
                            </span>
                            <span className="text-[9px] text-muted-foreground font-mono">
                              {String(ver)}
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(availableTools).map(([tool, path]) => (
                    <div
                      key={tool}
                      className="flex flex-col gap-3 p-4 rounded-2xl bg-background border border-border/40 hover:border-amber-500/40 transition-all shadow-sm"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <code
                          className="block p-2 rounded-lg bg-muted/30 text-[10px] font-mono truncate text-muted-foreground"
                          title={String(path)}
                        >
                          {String(path)}
                        </code>
