...
export const FilePreview: React.FC<FilePreviewProps> = ({
  ...
  const hasChanges = useMemo(
    () => isEditable && content !== modifiedContent,
    [isEditable, content, modifiedContent]
  )
    ...
    const loadFile = async () => {
      ...
      try {
        ...
        if (['jpg', 'jpeg', 'png', 'gif', 'svg', 'webp'].includes(extension)) {
          ...
          if (
            text.includes('\u0000') &&
            !['json', 'jsonl', 'xml', 'csv', 'tsv'].includes(extension)
          ) {
            setFileType('binary')
            setContent(null)
          } else {
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <h3 className="text-sm font-bold tracking-tight truncate max-w-[200px] sm:max-w-md uppercase">
              {filename}
            </h3>
            <Badge
              variant="outline"
              className="h-4 text-[8px] font-bold uppercase tracking-tighter bg-background/50 border-border/40"
            >
              {extension || 'FILE'}
            </Badge>
          </div>
    ...
    if (error) {
      return (
        <div className="flex flex-col gap-6 p-12 items-center justify-center h-full text-center">
          ...
          <Button
            variant="outline"
            className="rounded-xl px-8 h-11 font-bold gap-2 hover:bg-destructive/5 hover:text-destructive hover:border-destructive/30 transition-all"
            ...
            <div className="h-full w-full flex flex-col items-center justify-center gap-8 p-12 text-center">
              ...
              <div className="space-y-3">
                ...
                <p className="text-sm text-muted-foreground max-w-sm mx-auto leading-relaxed font-medium">
                  Rich preview is unavailable for this specific format. Download the resource to
                  inspect locally.
                </p>
              </div>
              <Button
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    <DetailItem
                      ...
                      label="Format"
                      value={extension.toUpperCase() || 'Unknown'}
                    />
                    <DetailItem
                    ...
                    <DetailItem
                      ...
                      value={
                        metadata?.modified_at
                          ? format(new Date(metadata.modified_at * 1000), 'MMM dd, HH:mm')
                          : '—'
                      }
                    />
                  </div>
  ...
  if (isMaximized) {
    return createPortal(
      <div className="fixed inset-0 z-1000 bg-background flex flex-col h-screen w-screen overflow-hidden">
        {renderHeader()}
        <div className="flex-1 min-h-0 flex flex-col overflow-hidden">{renderContentArea()}</div>
      </div>,
      document.body
    )
  }
...
const DetailItem = ({
  ...
  <div className="flex items-center justify-between group/item">
    <div className="flex items-center gap-2 text-muted-foreground">
      <div className="p-1 rounded-md bg-muted group-hover/item:text-primary transition-colors">
        {icon}
      </div>
