...
const FileIcon: React.FC<{ item: FileItem; className?: string }> = ({ item, className }) => {
    if (item.type === 'directory') return <Folder className={cn("h-4 w-4 text-blue-500 fill-blue-500/20", className)} />;
    ...
    const ActionMenu = ({ item }: { item: FileItem }) => (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg opacity-0 group-hover:opacity-100 focus:opacity-100 transition-all">
            ...
            >
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center bg-background/50 border border-border/40 rounded-lg p-0.5">
                            <Button
                                variant="ghost"
                                size="icon"
                                className={cn("h-7 w-7 rounded-md transition-all", viewMode === 'list' ? "bg-background shadow-sm text-primary" : "text-muted-foreground")}
                            ...
                            <Button
                                variant="ghost"
                                size="icon"
                                className={cn("h-7 w-7 rounded-md transition-all", viewMode === 'grid' ? "bg-background shadow-sm text-primary" : "text-muted-foreground")}
                                ...
                                {filteredItems.map((item) => (
                                        ...
                                        <TableCell className="text-center">
                                            <Badge variant="outline" className={cn(
                                                ...
                                                {item.type === 'directory' ? 'DIR' : (item.name.split('.').pop()?.toUpperCase() || 'FILE')}
                                            </Badge>
                                        </TableCell>
                ...
                <AnimatePresence>
                    {previewFile && (
                        ...
                        >
                            <FilePreview connectionId={connectionId} path={previewFile.path} filename={previewFile.name} metadata={previewFile} onClose={() => setPreviewFile(null)} />
                        </motion.div>
                    )}
                </AnimatePresence>
                    ...
                    <DialogContent className="sm:max-w-md rounded-3xl border-border/60 glass-panel shadow-2xl backdrop-blur-3xl">
                        ...
                        <form onSubmit={handleMkdir}>
                            <div className="grid gap-4 py-4">
                                <Input id="name" placeholder="Directory Name..." className="h-11 rounded-xl bg-background/50 border-border/40" value={newFolderName} onChange={(e) => setNewFolderName(e.target.value)} autoFocus />
                            </div>
                            <DialogFooter>
                                <Button type="button" variant="ghost" onClick={() => setIsMkdirOpen(false)}>Cancel</Button>
                                <Button type="submit" disabled={!newFolderName}>Create Folder</Button>
                            </DialogFooter>
                        </form>
                    </DialogContent>
                </Dialog>
                    ...
                    <AlertDialogContent className="rounded-[2.5rem] border-border/60 glass-panel shadow-2xl backdrop-blur-3xl p-0 overflow-hidden max-w-md">
                        <div className="p-8">
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>You are about to permanently delete {itemToDelete?.name}.</AlertDialogDescription>
                            </AlertDialogHeader>
                        </div>
                        <AlertDialogFooter className="bg-muted/30 p-6 flex items-center gap-3">
                            <AlertDialogCancel onClick={() => setIsDeleteAlertOpen(false)}>Cancel</AlertDialogCancel>
                            <AlertDialogAction onClick={confirmDelete} className="bg-destructive text-white">Confirm Purge</AlertDialogAction>
                        </AlertDialogFooter>
                    </AlertDialogContent>
                </AlertDialog>
