          ...
          {i === 1 && (
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {[1, 2, 3].map((j) => (
                <div
                  key={j}
                  className="h-[180px] p-6 rounded-2xl border border-border/40 bg-card space-y-4"
                >
...
const FilterToolbar = ({
  ...
  const getFilterValue = useCallback(
    (item: any, filterId: string, getValueFn: (i: any) => string | undefined) => {
    ...
    <div className="flex items-center gap-2 flex-wrap">
          ...
          <DropdownMenu key={f.id}>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                size="sm"
                className={cn(
                  'h-8 gap-2 rounded-lg border-dashed',
                  selected && 'border-primary bg-primary/5 text-primary border-solid'
                )}
              >
            ...
            <DropdownMenuContent align="start" className="w-56 p-1 max-h-80 overflow-y-auto">
              ...
              {available.map((opt) => (
                <DropdownMenuItem
                  key={opt}
                  onClick={() => onSelectFilter(f.id, opt === selected ? null : opt)}
      ...
      {Object.values(selectedFilters).some((v) => v !== null) && (
        <Button
          ...
          size="sm"
          onClick={onClearAll}
          className="h-8 gap-1.5 rounded-lg text-muted-foreground hover:text-primary px-2 transition-colors"
        >
...
const GenericEntityCard = ({
  ...
  const kind = isManaged
    ? item.asset_type
    : item.metadata?.entity_type || config.registration.assetType
  ...
  return (
    ...
    >
      ...
      {!isManaged && selected && <div className="absolute top-0 left-0 w-1 h-full bg-primary" />}
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <Badge
                variant="outline"
                className="h-4 px-1.5 text-[8px] font-black uppercase tracking-tighter bg-muted/50 border-border/40 text-muted-foreground/70"
              >
                {kind}
              </Badge>
              {isManaged && (
                <Badge className="h-4 px-1.5 text-[8px] font-black bg-emerald-500/10 text-emerald-600 border-none uppercase">
                  Registered
                </Badge>
              )}
            </div>
          ...
          <div className="shrink-0" onClick={(e) => e.stopPropagation()}>
            {isManaged ? (
              <AssetCardActions item={item} config={config} connectionId={connectionId} isManaged />
            ) : (
      ...
      <div className="flex items-center justify-between pt-4 border-t border-border/10 mt-auto">
        <div className="flex flex-wrap items-center gap-1.5 min-w-0">
          {!isManaged &&
            config.card.badges.map((badge, idx) => {
              ...
              return (
                <Badge
                  key={idx}
                  variant="secondary"
                  className={cn('text-[9px] font-bold px-1.5 py-0 rounded-md h-5', badge.color)}
                >
                  {badge.label || val}
                </Badge>
              )
        ...
        <div className="flex items-center gap-3 shrink-0 ml-4">
          {isManaged
            ? item.row_count_estimate !== undefined &&
              item.row_count_estimate !== null && (
                <div className="flex items-center gap-1.5 text-muted-foreground/50">
                  <Database className="h-3 w-3" />
                  <span className="font-mono font-bold text-[10px]">
                    {item.row_count_estimate.toLocaleString()}
                  </span>
                </div>
              )
...
export const DomainBrowser: React.FC<DomainBrowserProps> = ({
  ...
  const [activeView, setActiveView] = useState<'managed' | 'discovery'>(
    registeredAssets.length > 0 ? 'managed' : 'discovery'
  )
  ...
  const registerMutation = useMutation({
    ...
    onError: (err: any) => {
      toast.error('Registration Failed', {
        description:
          err.response?.data?.detail?.message || err.message || 'An unexpected error occurred.',
  ...
  return (
    <div className="h-full flex flex-col rounded-3xl border border-border/40 bg-background/40 backdrop-blur-xl shadow-xl overflow-hidden relative">
      ...
      <div className="p-4 md:p-5 border-b border-border/40 bg-muted/10 flex flex-col md:flex-row items-center justify-between shrink-0 gap-4 md:gap-6">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          {activeView === 'discovery' ? (
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button
                          ...
                          className="h-9 px-4 gap-2 text-xs font-bold rounded-xl shadow-lg shadow-primary/20"
                        >
                      ...
                      >
                        ...
                        <DropdownMenuItem
                          ...
                          className="rounded-lg text-xs font-medium py-2 gap-2"
                        >
                          <Save className="h-3.5 w-3.5 text-emerald-500" /> Register as
                          Destination(s)
                        </DropdownMenuItem>
                        <DropdownMenuItem
            ...
            <Button
              size="sm"
              className="h-9 px-4 gap-2 text-xs font-bold rounded-xl"
      ...
      <div className="flex-1 flex overflow-hidden relative">
        {isLoading && assets.length === 0 ? (
          ...
          <main className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-border/50 hover:scrollbar-thumb-border/80 scrollbar-track-transparent p-6 space-y-6">
            <div className="flex items-center justify-between px-1 border-b border-border/40 pb-4">
              <div className="flex items-center gap-4">
                ...
                <FilterToolbar
                  ...
                  onSelectFilter={(id, val) =>
                    setSelectedFilters((prev) => ({ ...prev, [id]: val }))
                  }
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-mono text-muted-foreground/60">
                  {activeView === 'managed' ? filteredManaged.length : filteredDiscovery.length}{' '}
                  items
                </span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              {(activeView === 'discovery' ? discoveryGroups : managedGroups).map(
                ([group, items]) => {
                  const groupFqns = items.map((i: any) => config.registration.getFqn(i))
                  const selectedInGroup = groupFqns.filter((fqn) =>
                    selectedForRegistration.has(fqn)
                  ).length
                      ...
                      <CollapsibleContent className="pt-4">
                        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                          {items.map((item: any) => {
                            ...
                            const fqn = isDiscovery
                              ? config.registration.getFqn(item)
      ...
      <Sheet open={entityStack.length > 0} onOpenChange={(open) => !open && setEntityStack([])}>
        <SheetContent
          side="right"
          className="sm:max-w-3xl p-0 flex flex-col bg-background/95 backdrop-blur-3xl border-l border-border/40 shadow-2xl"
        >
          {inspectingItem && (
            <DomainEntityDetails
              item={inspectingItem}
              ...
              onNavigate={(rel: any) => {
                ...
                const entityType =
                  typeof rel === 'string' ? rel.split(':').pop()?.split('--').pop() : rel.entityType
                ...
                const found =
                  ...
                  registeredAssets.find(
                    (a) => a.name === targetKind || (entityType && a.name.includes(entityType))
                  )
