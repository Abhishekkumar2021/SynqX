...
const RelationshipEdge = ({
  ...
  return (
    <>
      <BaseEdge path={edgePath} markerEnd={markerEnd} style={{ ...style, strokeWidth: 1.5, opacity: 0.4 }} />
      <EdgeLabelRenderer>
        <div
          style={{
            position: 'absolute',
    ...
    <div className="group relative">
      ...
      <div className={cn(
        ...
        <div className={cn(
          ...
          data.category === 'Technical' ? "bg-muted text-muted-foreground/30" : "bg-primary/10 text-primary"
        )}>
          {isRoot ? <Database size={18} /> : (data.category === 'Technical' ? <Binary size={16} /> : <Sparkles size={16} />)}
        </div>
...
const GraphView: React.FC<DomainEntityGraphProps & { 
    ...
    useEffect(() => {
        ...
        filteredRelationships.forEach((rel, idx) => {
          ...
          if (Math.abs(angle) < Math.PI / 4) { sourcePos = Position.Right; targetPos = Position.Left; }
          else if (Math.abs(angle - Math.PI) < Math.PI / 4) { sourcePos = Position.Left; targetPos = Position.Right; }
          else if (angle > 0) { sourcePos = Position.Bottom; targetPos = Position.Top; }
          else { sourcePos = Position.Top; targetPos = Position.Bottom; }

          const parts = rel.targetKind.split(':');
          const entityTypeFull = parts[2] || parts.pop() || rel.targetKind;
          const nodeLabel = entityTypeFull.includes('--') ? entityTypeFull.split('--')[1] : entityTypeFull;
    ...
    return (
        <div className={cn(
            ...
            !isMaximizedView ? "rounded-3xl border border-border/40 overflow-hidden" : "bg-background h-full w-full"
        )}>
            <ReactFlow
              nodes={nodes}
              edges={edges}
              ...
              <Panel position="top-left" className="m-4">
                <div className="flex items-center gap-3 bg-background/80 backdrop-blur-md border border-border/40 rounded-xl p-2.5 shadow-sm">
                    ...
                    <div className="flex flex-col pr-2">
                        <span className="text-[8px] font-black uppercase tracking-wider text-muted-foreground/60 leading-none">Entity Topology</span>
                        <h3 className="text-[11px] font-bold tracking-tight text-foreground mt-1">{rootLabel}</h3>
                    </div>
                </div>
              </Panel>

              {/* Robust Positioning for Control Bar to avoid overlapping with Sheet/Dialog close buttons */}
              <Panel position="top-right" className={cn("m-4 flex items-center gap-3 pointer-events-auto transition-all", isMaximizedView ? "mt-10 mr-24" : "mr-12")}>
