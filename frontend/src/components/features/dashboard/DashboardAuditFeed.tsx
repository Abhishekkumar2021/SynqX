...
export const DashboardAuditFeed: React.FC<DashboardAuditFeedProps> = ({ logs }) => {
    const getIcon = (eventType: string) => {
        if (eventType.includes('create') || eventType.includes('add')) return <Plus className="h-4 w-4 text-emerald-500" />;
        if (eventType.includes('update') || eventType.includes('edit')) return <Edit className="h-4 w-4 text-amber-500" />;
        if (eventType.includes('delete') || eventType.includes('remove')) return <Trash2 className="h-4 w-4 text-rose-500" />;
        if (eventType.includes('login') || eventType.includes('auth')) return <LogIn className="h-4 w-4 text-blue-500" />;
