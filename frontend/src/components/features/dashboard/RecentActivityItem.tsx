...
import { MoreVertical, FileText, RefreshCw, Terminal, Workflow } from 'lucide-react'
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export const RecentActivityItem: React.FC<RecentActivityItemProps> = ({ job }) => {
  ...
  return (
    ...
    >
      ...
      {/* Actions */}
      <div
        className="col-span-12 md:col-span-1 flex items-center justify-end opacity-0 group-hover:opacity-100 transition-opacity"
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="ghost"
              size="icon"
              className="h-7 w-7 rounded-xl text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
            >
          ...
          <DropdownMenuContent
            align="end"
            className="w-48 rounded-xl border-border/60 shadow-xl p-1"
          >
            <DropdownMenuItem
              className="cursor-pointer gap-2 rounded-xl font-medium text-xs py-2"
              onClick={() => navigate(`/jobs/${job.id}`)}
            >
