            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar border-t-0">
                {violations.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center p-12 text-center opacity-40 ">
                        <span className="text-xs font-bold uppercase tracking-widest">Perfect compliance. No violations.</span>
                    </div>
                ) : (
                    <div className="divide-y divide-border/10">
                        {violations.map((v, i) => (
                            <div key={`${v.rule_type}-${v.column_name}-${i}`} className="flex items-center justify-between p-5 hover:bg-amber-500/5 transition-colors group">
