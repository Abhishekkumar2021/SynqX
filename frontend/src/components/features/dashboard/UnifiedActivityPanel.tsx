...
const EphemeralJobsList: React.FC<{ jobs: EphemeralJobResponse[] }> = ({ jobs }) => {
    return (
        <div className="flex flex-col h-full overflow-hidden">
             ...
             <div className="grid grid-cols-12 gap-4 px-8 py-3 border-b border-border/20 bg-muted/30 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 shrink-0 sticky top-0 z-20 backdrop-blur-md">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                {jobs.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-muted-foreground gap-4 opacity-40 p-8">
                        ...
                        <span className="text-xs font-black uppercase tracking-widest">No remote tasks</span>
                    </div>
                ) : (
                        ...
                        {jobs.map((job) => (
                            <div key={job.id} className="grid grid-cols-12 gap-4 px-8 py-4 hover:bg-muted/20 transition-colors items-center group">
                                <div className="col-span-5 flex flex-col gap-1">
                                    <span className="text-xs font-black text-foreground truncate">{job.job_type}</span>
                                    {job.agent_group && (
                                ...
                                <div className="col-span-3">
                                    <Badge variant="outline" className={cn(
                                        "rounded-md text-[9px] font-black uppercase tracking-widest border-0 px-2 py-0.5",
        ...
        <Tabs defaultValue="runs" value={activeTab} onValueChange={setActiveTab} className="flex flex-col h-full overflow-hidden">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between py-5 px-8 shrink-0 gap-4">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <TabsList className="bg-muted/30 p-1 rounded-xl border border-border/40 h-10 w-full md:w-auto overflow-x-auto no-scrollbar justify-start">
                        <TabsTrigger 
                            value="runs" 
                            className="rounded-lg text-[10px] font-black uppercase tracking-widest px-3 data-[state=active]:bg-background data-[state=active]:text-foreground text-muted-foreground gap-2 min-w-fit transition-colors"
                        >
            ...
            <div className="flex-1 min-h-0 border-t border-border/20 bg-background/30 relative">
                <TabsContent value="runs" className="h-full m-0 data-[state=inactive]:hidden flex flex-col absolute inset-0">
                     <div className="grid grid-cols-12 gap-4 px-8 py-3 border-b border-border/20 bg-muted/30 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 shrink-0 z-20 backdrop-blur-md">
