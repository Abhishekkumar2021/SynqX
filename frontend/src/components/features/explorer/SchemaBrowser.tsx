...
export const SchemaBrowser: React.FC<SchemaBrowserProps> = ({ connectionId, onAction }) => {
  ...
  const filteredMetadata = useMemo(() => {
    ...
    Object.entries(metadata.metadata).forEach(([table, columns]) => {
      if (
        table.toLowerCase().includes(lowFilter) ||
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-center gap-6 isolate">
        ...
        <div className="space-y-1.5">
          <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-foreground/60">
            Initialize Registry
          </p>
          <p className="text-[9px] font-bold text-muted-foreground/30 uppercase tracking-widest">
            Select connection to browse
          </p>
        </div>
      </div>
    )
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-center gap-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-[9px] font-medium text-muted-foreground leading-relaxed px-4 max-w-[200px]">
            The secure endpoint rejected the discovery request.
          </p>
        </div>
        <Button
          size="sm"
  ...
  return (
    <div className="h-full flex flex-col bg-background/20 backdrop-blur-md relative overflow-hidden isolate">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              variant="ghost"
              className={cn(
                'h-9 w-9 rounded-xl transition-all border border-border/20',
      ...
      <div className="flex-1 min-h-0 overflow-y-auto custom-scroll select-none">
        <div className="p-3 space-y-1">
          <AnimatePresence mode="popLayout">
            {filteredMetadata &&
              Object.entries(filteredMetadata).map(([table, columns], idx) => (
                <motion.div
                  key={table}
                  ...
                  <div className="relative flex items-center">
                    ...
                    >
                      ...
                      <Box
                        className={cn(
                          ...
                          expandedTables.has(table)
                            ? 'opacity-100'
                            : 'opacity-30 group-hover/table:opacity-100'
                        )}
                      />
                    ...
                    <div className="absolute right-2 opacity-0 group-hover/wrapper:opacity-100 transition-opacity z-10">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent
                          className="w-48 glass-panel border-border/40 rounded-xl"
                          align="end"
                        >
                          <DropdownMenuLabel className="text-[9px] font-bold uppercase tracking-widest opacity-40">
                            Actions
                          </DropdownMenuLabel>
                          <DropdownMenuItem
                            onClick={() => onAction('run', `SELECT * FROM ${table} LIMIT 100;`)}
                          ...
                          <DropdownMenuSeparator />
                          <DropdownMenuItem
                            onClick={() => onAction('insert', `SELECT * FROM ${table}`)}
                            className="gap-2 text-[10px] font-bold uppercase tracking-wide"
                          >
      ...
      <div className="px-5 py-3 border-t border-border/40 bg-muted/5 backdrop-blur-sm shrink-0 flex items-center justify-between">
        <span className="text-[9px] font-bold uppercase text-muted-foreground/30 tracking-[0.2em]">
          Registry Schema
        </span>
        <motion.span
