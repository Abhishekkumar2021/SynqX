...
export const ExplorerSidebar: React.FC<ExplorerSidebarProps> = ({
  ...
  const searchResults = useFuzzySearch(connections || [], searchQuery, {
...
const SourceIcon = ({ type }: { type: string }) => {
  ...
  if (['postgresql', 'mysql', 'oracle', 'sqlite', 'mssql'].includes(t))
    return <Database className="h-4 w-4" />
  if (['snowflake', 'bigquery', 'redshift', 'databricks'].includes(t))
    return <Cloud className="h-4 w-4" />
  if (['local_file', 's3', 'gcs', 'azure_blob'].includes(t))
