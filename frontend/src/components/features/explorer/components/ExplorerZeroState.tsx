...
export const ExplorerZeroState: React.FC<ExplorerZeroStateProps> = ({ 
    ...
    >
        ...
        <div className="flex gap-3">
            <Button variant="outline" className="rounded-xl font-bold text-xs gap-2 border-border/40" onClick={onToggleSidebar}>
                {isSidebarCollapsed ? <PanelLeftOpen className="h-3.5 w-3.5" /> : <Plus className="h-3.5 w-3.5" />}
                Browse Sources
            </Button>
        </div>
    </motion.div>
