...
export const OSDUExplorer: React.FC<OSDUExplorerProps> = ({ connectionId }) => {
    ...
    const setService = (service: OSDUService) => updateParams({ service, recordId: null, offset: '0' });
    ...
    const partitionId = connection?.config?.data_partition_id || connection?.config?.partition || "OSDU";
    ...
    const { data: searchResponse, isLoading: isSearching, refetch: refetchSearch } = useQuery({
        ...
        queryFn: async () => {
            ...
            const params = currentCursor && pageOffset > 0 
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {activeService === 'registry' && (
                            <motion.div key="registry" className="h-full">
                                <OSDURegistryView kinds={kinds} onSelectKind={setKind} isLoading={isLoadingKinds} onRefresh={refetchKinds} />
                            </motion.div>
                        )}
                        ...
                        {(activeService === 'identity' || activeService === 'compliance') && (
                            <motion.div key="gov" className="h-full">
                                <OSDUGovernanceView connectionId={connectionId} initialMode={activeService as any} />
                            </motion.div>
                        )}
                    </AnimatePresence>
                </div>
