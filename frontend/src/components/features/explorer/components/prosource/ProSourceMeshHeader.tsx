...
import { Search, SlidersHorizontal, Layers, X, Database, Sparkles, LayoutGrid, List } from 'lucide-react'
      ...
      <div className="flex-1 max-w-2xl relative group">
        ...
        <div className="relative">
            <Input
                ...
                value={asset && pendingSql ? `SQL: ${pendingSql.length > 40 ? pendingSql.substring(0, 40) + '...' : pendingSql}` : ''}
                readOnly
            />
            {pendingSql && (
        ...
        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-2">
          ...
          <Sheet open={isBuilderOpen} onOpenChange={setIsBuilderOpen}>
            <SheetTrigger asChild>
              <Button variant="ghost" size="icon" className={cn("h-8 w-8 rounded-lg transition-all", pendingSql ? "text-primary bg-primary/5" : "hover:text-primary")}>
                <SlidersHorizontal size={16} />
              </Button>
            </SheetTrigger>
            <SheetContent side="right" className="w-[600px] sm:max-w-none p-0 border-l border-border/40 bg-background/95 backdrop-blur-2xl shadow-2xl flex flex-col">
              <SheetHeader className="p-8 border-b border-border/10 bg-muted/5">
                <div className="flex items-center gap-4 mb-2">
                    ...
                    <div>
                        <SheetTitle className="text-xl font-black uppercase tracking-widest">Visual Query Architect</SheetTitle>
                        <SheetDescription className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                            Construct precise Oracle SQL predicates for <b className="text-foreground">{asset?.name}</b>
                        </SheetDescription>
                    </div>
                </div>
              </SheetHeader>
              ...
              <div className="p-8 border-t border-border/10 bg-muted/5 flex items-center justify-between gap-4">
                ...
                <div className="flex gap-3">
                    <Button variant="ghost" onClick={() => setIsBuilderOpen(false)} className="rounded-xl font-bold uppercase text-[10px] tracking-widest px-6">
                        Cancel
                    </Button>
                    <Button 
                        onClick={() => {
