        ...
        <div className="flex items-center gap-5">
            ...
            <div>
                <h2 className="text-xl font-black uppercase tracking-tight flex items-center gap-3">
                    Identity Registry
                    <Badge variant="secondary" className="h-5 px-2 bg-cyan-500/10 text-cyan-600 border-none text-[9px] font-black uppercase">
                        {filteredAccounts.length} Verified
                    </Badge>
                </h2>
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] mt-1 opacity-60">SDS_ACCOUNT Domain & Privilege Management</p>
            </div>
        </div>
        ...
        <div className="p-8 max-w-[1600px] mx-auto w-full pb-32">
            {isLoading ? (
                <div className="flex flex-col items-center justify-center py-48 gap-6 opacity-40">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-cyan-600 animate-pulse">Materializing security context...</span>
                </div>
            ) : filteredAccounts.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-48 opacity-20 grayscale">
                    <Shield size={80} strokeWidth={1} />
                    <p className="mt-6 font-black uppercase text-[10px] tracking-[0.3em]">No security identities found</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {filteredAccounts.map((acc: any, i: number) => (
                        <div key={i} className="p-6 rounded-[2.5rem] bg-card border border-border/40 hover:border-cyan-500/30 hover:shadow-2xl transition-all group flex flex-col gap-6 relative overflow-hidden shadow-sm">
                            <div className="flex items-center justify-between relative z-10">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <Badge variant="secondary" className="bg-emerald-500/10 text-emerald-600 border-none text-[9px] font-black uppercase px-2 h-5">Verified_Active</Badge>
                                </div>
                            </div>
                            
                            <div className="space-y-1 relative z-10">
                                <h4 className="text-base font-black text-foreground uppercase tracking-tight truncate group-hover:text-cyan-600 transition-colors">{acc.ACCOUNT}</h4>
                            ...
                            <div className="grid grid-cols-2 gap-3 p-4 rounded-2xl bg-muted/20 border border-border/10 shadow-inner relative z-10">
                                <div className="flex flex-col gap-1">
                                    <span className="text-[7px] font-black uppercase text-muted-foreground/40">Domain Class</span>
                                    <span className="text-[10px] font-bold text-foreground/80">{acc.TYPE || 'STANDARD'}</span>
                                </div>
                                <div className="flex flex-col gap-1 text-right">
                                    <span className="text-[7px] font-black uppercase text-muted-foreground/40">Auth_Code</span>
                                    <span className="text-[10px] font-mono font-bold text-foreground/80 tabular-nums">{acc.STATUS || '00'}</span>
                                </div>
                            </div>

                            <div className="flex items-center justify-between pt-2 border-t border-border/5 relative z-10">
                                <div className="flex items-center gap-2">
                                    <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl hover:bg-cyan-500/10 hover:text-cyan-600 transition-all active:scale-90 border border-transparent hover:border-cyan-500/20 shadow-sm"><Key size={16} /></Button>
                                    <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl hover:bg-muted active:scale-90 shadow-sm transition-all"><Shield size={16} /></Button>
                                </div>
                                <Button variant="ghost" className="h-9 px-4 rounded-xl text-[9px] font-black uppercase tracking-[0.1em] text-cyan-600 opacity-0 group-hover:opacity-100 transition-all bg-cyan-500/5 hover:bg-cyan-500/10 border border-cyan-500/10">
                                    Edit Privilege
                                </Button>
                            </div>
