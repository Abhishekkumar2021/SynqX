...
import { Search, ChevronRight, Box, ChevronDown } from 'lucide-react'
  ...
  const groupedAssets = useMemo(() => {
    ...
    const filtered = assets.filter(
      (a) =>
        ...
        (a.metadata?.table || a.metadata?.TABLE || '')
          .toLowerCase()
          .includes(searchQuery.toLowerCase())
    )
      ...
      <div className="p-6 space-y-6 border-b border-border/10 bg-card/30">
        <div className="space-y-4">
          <div className="flex items-center justify-between px-1">
            ...
            <Badge
              variant="outline"
              className="h-4 px-1.5 rounded-sm text-[8px] font-black border-primary/20 text-primary/60 bg-primary/5 uppercase"
            >
            ...
            return (
              <div key={module} className="space-y-1">
                ...
                >
                  ...
                  {module}
                  <span className="ml-auto opacity-30 group-hover/group-head:opacity-60">
                    {groupItems.length}
                  </span>
                </button>
