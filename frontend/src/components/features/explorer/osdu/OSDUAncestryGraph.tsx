...
const AncestryNode = ({ data }: { data: any }) => {
  ...
  if (isRoot) {
    return (
      <div className="relative flex flex-col items-center justify-center min-w-[180px] px-6 py-3 rounded-full bg-primary text-primary-foreground shadow-xl shadow-primary/20 border-2 border-primary ring-4 ring-primary/10 transition-transform hover:scale-105">
        ...
        <span className="text-xs font-bold font-mono opacity-90 truncate max-w-[150px]">{data.label}</span>
  ...
  return (
    ...
    `}>
      <Handle type="target" position={Position.Top} className="!w-2 !h-2 !bg-muted-foreground/30 !-mt-[5px]" />
      ...
      <div className="p-3 flex flex-col gap-1.5">
         <div className="flex items-center justify-between">
            ...
            {isParent ? <Database size={12} className="text-sky-500" /> : <Layers size={12} className="text-indigo-500" />}
         </div>
      ...
      <Handle type="source" position={Position.Bottom} className="!w-2 !h-2 !bg-muted-foreground/30 !-mb-[5px]" />
    </div>
  )
}
...
export const OSDUAncestryGraph: React.FC<OSDUAncestryGraphProps> = ({ ancestryData, rootId, onNavigate }) => {
    ...
    <div className="h-full w-full bg-muted/5 relative">
      ...
      >
        ...
        <div className="absolute top-4 right-4 z-10 flex flex-col gap-2">
             <Badge variant="outline" className="bg-background/80 backdrop-blur border-sky-500/30 text-sky-600 gap-2">
