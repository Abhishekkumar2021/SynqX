...
const AncestryNode = ({ data }: { data: any }) => {
    ...
    return (
        <div className={`
            ...
            transition-all
        `}>
            <div className={`p-2 rounded-lg ${isRoot ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
                {data.type === 'parent' ? <Layers size={16} /> : <Database size={16} />}
            </div>
            <div className="flex flex-col overflow-hidden">
                <span className="text-[9px] font-black uppercase tracking-widest opacity-60 truncate max-w-[120px]">{data.kind}</span>
                <span className="text-xs font-bold truncate max-w-[120px]" title={data.label}>{data.label}</span>
            </div>
        </div>
    );
};
...
export const OSDUAncestryGraph: React.FC<OSDUAncestryGraphProps> = ({ ancestryData, rootId }) => {
    ...
    useEffect(() => {
        ...
        if (ancestryData.parents) {
            ancestryData.parents.forEach((parentId: string, idx: number) => {
        ...
        if (ancestryData.children) {
            ancestryData.children.forEach((childId: string, idx: number) => {
