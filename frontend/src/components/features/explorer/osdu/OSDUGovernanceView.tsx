      ...
      <div className="flex-1 flex flex-col min-h-0 relative">
        <div className="px-10 py-3 border-b border-border/10 bg-muted/5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-8">
            ...
            <Badge
              variant="outline"
              className="h-6 px-3 border-border/40 bg-background text-xs font-black text-foreground/80 shadow-sm"
            >
              {initialMode === 'identity'
                ? formatNumber(groups.length)
                : formatNumber(legalTags.length)}{' '}
              DEFINITIONS
            </Badge>
          </div>
            ...
            {selectedIds.size > 0 && (
              ...
              >
                ...
                <Button
                  variant="default"
                  size="sm"
                  className="h-9 px-5 rounded-xl gap-2 font-black uppercase text-[10px] tracking-widest shadow-lg shadow-primary/20"
                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 pb-32">
                  {items.map((i: any) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className="absolute top-5 left-5 z-30 opacity-0 group-hover:opacity-100 transition-all"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <Checkbox
                            checked={isSelected}
                            onCheckedChange={() => toggleSelection(id)}
                            className="h-5 w-5 rounded-lg border-border/40"
                          />
                        </div>
                        ...
                        <div className="flex items-start gap-5 pt-2">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="flex items-center gap-2">
                              <Badge
                                variant="outline"
                                className="text-[10px] font-black uppercase border-border/60 bg-muted/30 h-5 px-2 tracking-widest text-foreground"
                              >
                                {initialMode === 'identity' ? 'Domain' : 'Legal_Tag'}
                              </Badge>
                              <span
                                className="text-[11px] font-mono text-muted-foreground/60 truncate"
                                title={id}
                              >
                                {id}
                              </span>
                        ...
                        <div className="bg-muted/30 rounded-[1.5rem] p-5 border border-border/10 shadow-inner space-y-3">
                          <div className="flex items-center justify-between gap-4">
                            ...
                            <Badge className="bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border-none text-[9px] font-black h-5 uppercase">
                              Verified_Active
                            </Badge>
                          </div>
                          <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="pt-4 border-t border-border/10 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              onClick={() => copyToClipboard(id, 'Entry ID')}
                            >
                              <Copy size={14} />
                            </Button>
                            <Button
                          ...
                          <div className="text-[11px] font-black text-primary uppercase tracking-[0.2em] group-hover:translate-x-1 transition-all cursor-pointer">
                            Configure Hub
                          </div>
                        </div>
                      </motion.div>
        ...
        <div className="px-10 py-6 border-t border-border/10 bg-background/40 backdrop-blur-md flex items-center justify-between shrink-0 relative z-20 shadow-inner">
          <div className="flex items-center gap-10">
            <div className="flex flex-col">
              <span className="text-[10px] font-black uppercase tracking-[0.25em] text-muted-foreground/40 leading-none">
                technical Registry
              </span>
              <span className="text-base font-black text-foreground mt-1.5 tracking-tighter uppercase">
                Showing {formatNumber(items.length)}{' '}
                <span className="opacity-20 mx-2 text-xs">BUFFER TOTAL</span>{' '}
            ...
            <div className="flex items-center gap-3">
              ...
              <Badge className="bg-emerald-500/15 text-emerald-600 border-none font-black h-6 shadow-sm">
                SYSTEM_LOCKED
              </Badge>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Button
              variant="outline"
              size="sm"
              className="h-11 px-8 gap-2 text-[11px] font-black uppercase tracking-widest rounded-2xl border-border/40 hover:bg-muted shadow-sm transition-all"
