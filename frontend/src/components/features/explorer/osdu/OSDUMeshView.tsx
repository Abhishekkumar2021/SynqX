...
import { 
    Search, RefreshCw, X,
    Database, Globe, Briefcase,
    ChevronLeft, Layers, 
    Copy, Sparkles, Binary,
    MoreHorizontal, Eye,
    Trash2,
    ArrowRight,
    ArrowUpRight,
...
export const OSDUMeshView: React.FC<OSDUMeshViewProps> = ({
    searchQuery, onQueryChange, selectedKind, onKindChange,
    searchResults, isLoading, onExecute, onSelectRecord,
    pageOffset, onOffsetChange, limit, onToggleAI
}) => {
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
        if (e.key === 'Enter') { e.preventDefault(); onExecute(); }
    };
                ...
                <div className="h-20 px-8 flex items-center gap-8">
                    <div className="flex items-center gap-4 min-w-0">
                        ...
                        <div className="min-w-0 mr-4">
                            <h2 className="text-xl font-black tracking-tighter text-foreground uppercase leading-none">Data Mesh</h2>
                            <p className="text-[10px] text-muted-foreground font-black uppercase tracking-[0.2em] mt-1.5 leading-none opacity-60">Partition Discovery</p>
                        </div>
                    </div>
                        ...
                        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-2">
                            <AnimatePresence>
                                {searchQuery && searchQuery !== '*' && (
                                    <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} onClick={() => handleApplyQuery('*')} className="h-8 w-8 rounded-xl flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">
                                        <X size={16} />
                                    </motion.button>
                                )}
                            </AnimatePresence>
                        ...
                        <div className="flex items-center gap-2">
                            {selectedKind && (
                                <Badge className="bg-primary/5 text-primary border border-primary/20 rounded-xl px-3 py-1.5 font-black uppercase text-[9px] tracking-widest gap-2 h-9 shadow-sm">
                                    ...
                                    <button onClick={() => onKindChange(null)} className="hover:text-rose-500 transition-colors ml-1"><X size={12} /></button>
                                </Badge>
                            )}
                            <Button variant="ghost" size="icon" className="h-10 w-10 rounded-xl border border-border/40 hover:bg-muted" onClick={() => { setSelectedIds(new Set()); onExecute(); }}>
            ...
            <div className="flex-1 min-h-0 relative flex flex-col">
                <div className="px-8 py-3 border-b border-border/10 bg-muted/10 flex items-center justify-between shrink-0 backdrop-blur-sm">
                    <div className="flex items-center gap-8">
                        ...
                        <Badge variant="outline" className="h-6 px-3 border-border/40 bg-background text-[10px] font-black text-foreground/80 tracking-widest uppercase rounded-full shadow-sm">
                    ...
                    <AnimatePresence>
                        {selectedIds.size > 0 && (
                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex items-center gap-3">
                                ...
                                <Button 
                                    variant="default" size="sm" className="h-8 px-5 rounded-xl gap-2.5 font-black uppercase text-[10px] tracking-widest bg-primary shadow-lg shadow-primary/20 text-white transition-all active:scale-95"
                                    onClick={handleExport}
                                >
                                    <Binary size={14} /> Export Bundle
                                </Button>
                                <Button variant="ghost" size="icon" className="h-8 w-8 rounded-xl hover:bg-muted" onClick={() => setSelectedIds(new Set())}>
                                    <X size={14} />
                                </Button>
                            </motion.div>
                        )}
                    ...
                    {!isLoading && results.length === 0 && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-12 space-y-8 z-10 animate-in fade-in zoom-in duration-700">
                            ...
                            <div className="space-y-2">
                                <p className="font-black text-3xl tracking-tighter uppercase text-foreground">Discovery Idle</p>
                                <p className="text-[11px] font-bold uppercase tracking-[0.3em] max-w-sm mx-auto text-muted-foreground leading-relaxed">The mesh is empty. Execute a query or use the neural assistant to resolve partition entities.</p>
                            </div>
                            <Button variant="outline" className="rounded-2xl border-primary/20 hover:bg-primary/5 text-primary font-black uppercase text-[10px] tracking-[0.2em] h-11 px-8" onClick={onToggleAI}>
                        ...
                        <div className="p-10 max-w-[1600px] mx-auto w-full">
                            {isLoading ? (
                                <div className="flex flex-col items-center justify-center py-48 gap-8 opacity-40">
                                    ...
                                    <span className="text-[11px] font-black uppercase tracking-[0.5em] animate-pulse">Materializing partition frame...</span>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6 pb-24">
                                    {results.map((r: any, idx: number) => (
                                        <motion.div 
                                            key={r.id || `result-${idx}`} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}
                                            className={cn(
                                        ...
                                        >
                                            <div className="absolute top-5 left-5 z-30 opacity-0 group-hover:opacity-100 transition-all duration-300" onClick={(e) => e.stopPropagation()}>
                                                <Checkbox checked={selectedIds.has(r.id)} onCheckedChange={() => toggleSelection(r.id)} className="h-5 w-5 rounded-lg border-border/40 bg-background shadow-md" />
                                            </div>
                                            ...
                                            <div className="flex items-start gap-5 pt-1 min-w-0">
                                                ...
                                                <div className="min-w-0 flex-1 overflow-hidden space-y-1.5">
                                                    <h4 className="font-black text-sm truncate text-foreground/90 tracking-tight leading-none uppercase pr-8 group-hover:text-primary transition-colors" title={r.id.split(':').pop()}>{r.id.split(':').pop()}</h4>
                                                    <div className="flex items-center gap-2 mt-1.5 min-w-0">
                                                        <Badge variant="secondary" className="text-[8px] font-black uppercase bg-primary/5 text-primary/70 border-primary/10 px-2 h-4.5 tracking-widest shrink-0">
                                                            {r.kind.split(':').slice(-2, -1)[0].split('--').pop()}
                                                        </Badge>
                                                        <span className="text-[9px] font-mono text-muted-foreground/30 truncate flex-1 tracking-tight" title={r.kind}>{r.kind}</span>
                                                    </div>
                                                </div>
                                            </div>

                                            <div className="bg-muted/20 rounded-[1.5rem] p-4 border border-border/5 shadow-inner flex flex-col gap-3">
                                                {Object.entries(r).filter(([k]) => !['data', 'acl', 'legal', 'kind', 'id', 'authority', 'source', 'type', 'version'].includes(k)).slice(0, 3).map(([key, val]: [string, any], kIdx: number) => (
                                                    <div key={key || `attr-${kIdx}`} className="flex items-center justify-between gap-4 min-w-0">
                                                        <div className="flex items-center gap-2 min-w-0">
                                                            <span className="text-[8px] font-black uppercase opacity-20 truncate tracking-[0.2em] shrink-0">{key}</span>
                                                        </div>
                                                        <span className="text-[10px] font-bold truncate text-foreground/60 text-right font-mono">{String(val)}</span>
                                                    </div>
                                                ))}
                                                {Object.keys(r).length < 8 && (
                                                    <div className="flex items-center justify-between">
                                                        <span className="text-[8px] font-black uppercase opacity-20 tracking-[0.2em]">Source</span>
                                                        <span className="text-[10px] font-bold text-foreground/60 uppercase tracking-tighter">{r.source || 'Standard'}</span>
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <div className="pt-4 border-t border-border/5 flex items-center justify-between px-1">
                                                <div className="flex items-center gap-4">
                                                    <div className="flex flex-col">
                                                        <span className="text-[7px] font-black text-muted-foreground/30 uppercase tracking-widest leading-none mb-1">Partition</span>
                                                        <span className="text-[9px] font-bold text-muted-foreground/60 flex items-center gap-1.5 uppercase">
                                                            <Globe size={10} className="text-primary/40" /> {r.authority || 'OSDU'}
                                                        </span>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl hover:bg-primary/5 hover:text-primary transition-all duration-300 active:scale-90" onClick={() => onSelectRecord(r.id)}>
                                                        <Eye size={18} />
                                                    </Button>
                                                    <DropdownMenu>
                                                        <DropdownMenuTrigger asChild>
                                                            <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl hover:bg-muted text-muted-foreground/40 transition-all"><MoreHorizontal size={18} /></Button>
                                                        </DropdownMenuTrigger>
                                                        <DropdownMenuContent align="end" className="w-56 rounded-[1.5rem] border-white/5 bg-neutral-900/90 backdrop-blur-2xl shadow-2xl p-2.5">
                                                            <DropdownMenuItem className="text-[10px] font-black uppercase tracking-widest gap-3 py-3 rounded-xl focus:bg-primary/10 focus:text-primary transition-all cursor-pointer" onClick={() => copyToClipboard(r.id, "Entity ID")}>
                ...
                <div className="px-10 py-4 border-t border-border/10 bg-muted/20 backdrop-blur-md flex items-center justify-between shrink-0 relative z-20 shadow-inner">
                    <div className="flex items-center gap-10">
                        <div className="flex flex-col">
                            <span className="text-[8px] font-black uppercase tracking-[0.5em] text-muted-foreground/30 leading-none">technical_partition_frame</span>
                            <div className="flex items-center gap-3 mt-2.5">
                                ...
                                <span className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-widest">total: {formatNumber(totalAvailable)}</span>
                            </div>
                        </div>
                        <div className="h-10 w-px bg-border/10" />
                        <div className="flex flex-col">
                            <span className="text-[8px] font-black uppercase tracking-[0.5em] text-muted-foreground/30 leading-none">Discovery_Density</span>
                            <div className="flex items-center gap-2 mt-2.5">
                                <Badge className="bg-primary/5 text-primary border-primary/20 font-black h-5 text-[9px] rounded-sm tracking-widest">{limit} BUFFER</Badge>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <Button variant="outline" size="sm" className="h-10 px-6 gap-3 text-[10px] font-black uppercase tracking-widest rounded-2xl border-border/40 hover:bg-background hover:border-primary/40 transition-all shadow-sm group" onClick={() => onOffsetChange(Math.max(0, pageOffset - limit))} disabled={pageOffset === 0 || isLoading}>
                            <ChevronLeft size={16} className="group-hover:-translate-x-0.5 transition-transform" /> Previous
                        </Button>
                        <Button variant="outline" size="sm" className="h-10 px-6 gap-3 text-[10px] font-black uppercase tracking-widest rounded-2xl border-border/40 hover:bg-background hover:border-primary/40 transition-all shadow-sm group" onClick={() => onOffsetChange(pageOffset + limit)} disabled={results.length < limit || pageOffset + results.length >= totalAvailable || isLoading}>
                            Next Discover <ArrowRight size={16} className="group-hover:translate-x-0.5 transition-transform" />
                        </Button>
                    </div>
                </div>
            </div>
