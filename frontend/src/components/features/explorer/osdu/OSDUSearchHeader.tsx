  ...
  <div className="space-y-4 p-2">
    ...
    <div className="grid gap-3">
      <div className="space-y-1">
        ...
        <code className="text-[10px] block bg-muted/50 p-1.5 rounded-lg border border-border/20">
          data.WellName: "Well-01"
        </code>
      </div>
      <div className="space-y-1">
        <span className="text-[9px] font-bold text-muted-foreground/60 uppercase tracking-tighter">
      ...
      <div className="space-y-1">
        ...
        <code className="text-[10px] block bg-muted/50 p-1.5 rounded-lg border border-border/20">
          data.Status: "Active" AND data.Type: "Prod"
        </code>
      </div>
      <div className="space-y-1">
        <span className="text-[9px] font-bold text-muted-foreground/60 uppercase tracking-tighter">
      ...
      <div className="flex items-center gap-4">
        {activeRecordId && (
          <Button
            ...
            className="h-10 w-10 rounded-2xl bg-background border-border/40 shadow-sm transition-transform active:scale-90"
          >
            <ArrowLeft size={18} />
          </Button>
        )}
          ...
          <div className="flex-1 max-w-2xl relative group">
            ...
            <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-2">
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8 rounded-lg hover:bg-muted text-muted-foreground/60"
                  >
                    <HelpCircle size={16} />
                  </Button>
                </PopoverTrigger>
                <PopoverContent
                  className="w-80 glass-panel rounded-2xl border-border/40 shadow-2xl p-4"
                  align="end"
                  sideOffset={12}
                >
                  <SearchGuide />
              ...
              <Button
                ...
                className="h-9 px-5 rounded-xl font-black uppercase tracking-widest text-[10px] shadow-xl shadow-primary/20"
              >
          ...
          <div className="flex items-center ml-auto gap-6">
            ...
            <Tabs value={activeTab} onValueChange={onTabChange} className="h-10">
              <TabsList className="bg-muted/30 p-1.5 rounded-[1.25rem] border border-border/10">
                <TabsTrigger
                  value="records"
                  className="h-7 px-4 text-[10px] font-black uppercase tracking-widest gap-2 rounded-xl data-[state=active]:bg-background data-[state=active]:shadow-xl transition-all"
                >
