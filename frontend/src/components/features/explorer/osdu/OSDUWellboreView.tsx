...
import { 
    Database, Activity, Map as MapIcon, Layers, 
    TrendingUp, Globe, ChevronRight, Ruler
} from 'lucide-react';
    ...
    return (
        <div className="p-8 space-y-8 animate-in fade-in duration-500 max-w-8xl mx-auto flex flex-col h-full overflow-hidden">
            <div className="space-y-1 shrink-0 px-1">
                ...
                <p className="text-sm text-muted-foreground font-medium">Domain-specific specialized view for master-data--Well entities.</p>
            </div>
            ...
            <ScrollArea className="flex-1 -mx-4 px-4 overflow-y-auto custom-scrollbar">
                <div className="space-y-8 pb-20">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 shrink-0">
                        <StatsCard title="Total Wells" value={stats.count} subtext="Master Data Records" icon={Database} variant="primary" />
                        <StatsCard title="Mean Depth" value={`${stats.avgDepth}m`} subtext="Vertical Measurement" icon={Ruler} variant="success" />
                        <StatsCard title="Active Ops" value={stats.active} subtext="Recent Spud Events" icon={Activity} variant="warning" />
                        <StatsCard title="Operators" value={stats.operators.length} subtext="Service Providers" icon={Globe} variant="info" />
                    </div>
                            ...
                            >
                                <div className="h-full w-full p-6 pb-10">
                                    <ResponsiveContainer width="100%" height="100%" key={`well-op-${theme}`}>
                                        <LineChart data={stats.operators} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                                        ...
                                        <BarChart data={stats.depthData} layout="vertical">
                                            ...
                                            <Bar dataKey="depth" name="Depth (m)" radius={[0, 4, 4, 0]} barSize={16}>
                                                {stats.depthData.map((_, index) => (
                                                    <Cell key={`cell-${index}`} fill={colors.CHART_COLORS[index % colors.CHART_COLORS.length]} fillOpacity={0.8} />
                                                ))}
                                            </Bar>
                                        </BarChart>
                                    </ResponsiveContainer>
                    ...
                    <Card className="bg-background/40 backdrop-blur-md border-border/40 shadow-xl overflow-hidden">
                        <CardHeader className="border-b border-border/5 bg-muted/5 flex flex-row items-center justify-between">
                            <div>
                                ...
                                <CardDescription className="text-xs">Ranking of operators by successful asset registration</CardDescription>
                            </div>
                        </CardHeader>
                        <CardContent className="p-0">
                            <div className="divide-y divide-border/5">
                                {stats.operators.map((op, i) => (
                                    <div key={op.name} className="px-8 py-4 flex items-center justify-between hover:bg-muted/30 transition-colors group">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <span className="font-bold text-sm text-foreground/80 group-hover:text-primary transition-colors">{op.name}</span>
                                        </div>
                                        <div className="flex items-center gap-6">
                                            <div className="flex flex-col items-end gap-0.5">
                                                <span className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-widest leading-none">Registered Wells</span>
                                                <span className="font-mono text-sm font-bold text-foreground/60">{op.value}</span>
                                            </div>
