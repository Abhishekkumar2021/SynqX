      ...
      <div className="p-3 flex flex-col gap-1.5">
        ...
        {data.relationField && (
          <div className="mt-1 pt-1.5 border-t border-border/40 flex items-center gap-1.5">
            ...
            <span
              className="text-[9px] font-mono text-indigo-600 truncate"
              title={data.relationField}
            >
              {data.relationField}
            </span>
          </div>
...
export const InspectorGraph: React.FC<InspectorGraphProps> = ({ record, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    const shortRootKind =
      record.details.kind?.split(':').slice(-2, -1)[0]?.split('--').pop() || 'Entity'
    ...
    <div className="h-full w-full bg-muted/5 relative">
      ...
      >
        ...
        <div className="absolute top-4 right-4 z-10 flex flex-col gap-2">
          ...
          <Badge
            variant="outline"
            className="bg-background/80 backdrop-blur border-emerald-500/30 text-emerald-600 gap-2"
          >
