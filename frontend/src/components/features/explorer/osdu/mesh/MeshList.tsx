...
export const MeshList: React.FC<MeshListProps> = ({
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('kind', {
        header: 'Kind',
        cell: ({ row }) => {
             const shortKind = row.original.kind.split(':').slice(-2, -1)[0]?.split('--').pop()
             return (
  ...
  return (
    <div className="rounded-2xl border border-border/40 bg-background/50 overflow-hidden shadow-sm">
      <Table>
          ...
          {table.getRowModel().rows?.length ? (
            table.getRowModel().rows.map((row) => (
              <TableRow
                ...
                className="border-border/20 hover:bg-muted/10 transition-colors group"
