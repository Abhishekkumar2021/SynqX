...
const HELP_EXAMPLES = [
  ...
  { label: 'Prefix Wildcard', syntax: 'data.WellName: Well*', desc: 'Matches Well-01, Well-02, etc.' },
  { label: 'Fuzzy Search', syntax: 'data.Status: Actvie~', desc: 'Corrects small typos in your query.' },
  { label: 'Logical AND', syntax: 'kind: "*Well:*" AND data.Status: "Active"', desc: 'Filters for records matching both.' },
  { label: 'Logical OR', syntax: 'data.Type: "Seismic" OR data.Type: "Well"', desc: 'Returns results matching either.' },
]
    ...
    <div className="flex h-full gap-10">
      ...
      <div className="flex-1 flex flex-col gap-8 min-w-0">
        <div className="flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2">
             <Button
                ...
                className={cn("h-8 gap-2 text-[10px] font-bold uppercase tracking-widest", showHelp ? "bg-primary/10 text-primary" : "text-muted-foreground")}
             >
                  ...
                  <div className="flex justify-center items-center py-3 relative">
                     <div className="bg-background relative z-10">
                        ...
                        >
                            ...
                            <SelectContent>
                                <SelectItem value="AND" className="text-xs font-bold text-blue-600">AND</SelectItem>
                                <SelectItem value="OR" className="text-xs font-bold text-amber-600">OR</SelectItem>
                            </SelectContent>
                        </Select>
                     </div>
                  </div>
                ...
                <div className={cn(
                    ...
                    <div className="flex-1 grid grid-cols-12 gap-3">
                        <div className="col-span-4 space-y-1.5">
                            <Label className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground/60 ml-1">Field</Label>
                            <div className="relative">
                                <Select
                                    value={COMMON_FIELDS.find(f => f.value === rule.field) ? rule.field : 'custom'}
                                    onValueChange={(val) => {
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        <div className="px-2 py-1.5 text-[10px] font-black text-muted-foreground uppercase tracking-widest">Common Properties</div>
                                        {COMMON_FIELDS.map(f => (
                                            <SelectItem key={f.value} value={f.value} className="text-xs font-mono">{f.label} <span className="text-muted-foreground/40 ml-2">({f.value})</span></SelectItem>
                                        ))}
                                        <div className="h-px bg-border/40 my-1" />
                                        <SelectItem value="custom" className="text-xs font-bold text-primary">Custom Field Name...</SelectItem>
                                    </SelectContent>
                                </Select>
                                {(!COMMON_FIELDS.find(f => f.value === rule.field) || rule.field === '') && (
                                    <Input
                                        value={rule.field}
                        ...
                        <div className="col-span-3 space-y-1.5">
                            <Label className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground/60 ml-1">Match Type</Label>
                            <Select
                                value={rule.operator}
                                ...
                                <SelectTrigger className="h-9 text-xs bg-muted/30 border-border/40 hover:bg-muted/50 transition-colors">
                                    <SelectValue />
                                                            ...
                                                            <SelectContent>
                                                                ...
                                                                <SelectItem value="wildcard" className="text-xs">Starts With (*)</SelectItem>
                                                                <SelectItem value="fuzzy" className="text-xs">Fuzzy (~)</SelectItem>
                                                                <SelectItem value="custom" className="text-xs">Raw Value</SelectItem>
                                                            </SelectContent>                            </Select>
                        </div>

                        {/* Value */}
                        <div className="col-span-5 space-y-1.5">
                            <Label className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground/60 ml-1">Value</Label>
                            <Input
                                value={rule.value}
        ...
        <div className="p-4 rounded-xl bg-black/40 border border-border/20 shrink-0">
            <Label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground mb-2 block">Lucene Preview</Label>
                ...
                {rules.map((r, i) => (
                    <span key={r.id}>
                        ...
                        <span className="text-emerald-400">
                            {r.operator === 'exact' ? `"${r.value}"` : 
                             r.operator === 'wildcard' ? `${r.value}*` : 
                             r.operator === 'fuzzy' ? `${r.value}~` : 
                             r.value}
                        </span>
                    </span>
                ))}
            ...
            {hasLeadingWildcard && (
                <div className="mt-3 flex items-center gap-2 text-rose-500 animate-in fade-in slide-in-from-top-1">
                    <AlertCircle size={12} />
                    <span className="text-[10px] font-bold">Warning: Leading wildcards (e.g. *value) are performance-heavy and often blocked by search services.</span>
                </div>
            )}
        </div>
      </div>
      ...
      {showHelp && (
        <div className="w-80 border-l border-border/40 pl-8 py-2 flex flex-col gap-8 animate-in slide-in-from-right-4 duration-300">
            <div className="space-y-1">
                ...
                <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-medium">Reference for Lucene query parser</p>
            </div>
            
            <ScrollArea className="flex-1 -mr-2 pr-4">
                <div className="space-y-6 pb-8">
                    {HELP_EXAMPLES.map((ex, i) => (
                        <div key={i} className="space-y-2.5 p-4 rounded-xl bg-muted/10 border border-border/20 transition-colors hover:bg-muted/20">
                            <div className="flex items-center justify-between">
                                <span className="text-[10px] font-black uppercase tracking-[0.15em] text-primary">{ex.label}</span>
                            </div>
                            ...
                            <p className="text-[10px] text-muted-foreground leading-relaxed font-medium">{ex.desc}</p>
                        </div>
                    ))}
                    ...
                    <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 space-y-2">
                        ...
                        <p className="text-[10px] text-amber-600/80 leading-snug">
                            <b>id</b> is a root field. Do not use <code>data.id</code>. Use wildcard <code>id: "val*"</code> to match versioned IDs.
                        </p>
                    </div>
                </div>
            </ScrollArea>
