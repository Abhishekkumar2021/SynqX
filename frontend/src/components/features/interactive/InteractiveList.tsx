...
const JobCard = ({
  ...
  job: EphemeralJobResponse
  onInspect: (j: EphemeralJobResponse) => void
}) => {
  return (
    <motion.div
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="flex items-center gap-2">
                ...
                {job.agent_group && (
                  <Badge
                    variant="outline"
                    className="text-[9px] font-bold uppercase bg-primary/5 text-primary border-primary/20 px-2 py-0.5 rounded-lg flex items-center gap-1.5 shadow-xs"
                  >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-10 w-10 rounded-xl bg-muted/20 text-muted-foreground hover:text-foreground hover:bg-muted/40 transition-all"
                >
              ...
              <DropdownMenuContent
                align="end"
                className="w-56 rounded-2xl border-border/40 bg-background/80 backdrop-blur-xl shadow-2xl p-1.5"
              >
                <DropdownMenuItem
                  onClick={() => onInspect(job)}
                  className="rounded-xl font-bold text-[11px] uppercase tracking-widest py-3 cursor-pointer gap-3"
                >
        ...
        <div className="flex-1 flex flex-col gap-5 relative z-10">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1.5 p-4 rounded-2xl bg-muted/20 border border-border/10">
              ...
              <span className="text-sm font-bold tabular-nums text-foreground">
                {job.result_summary?.count || 0}{' '}
                <span className="text-[10px] opacity-40 uppercase ml-1">rows</span>
              </span>
            </div>
          </div>
          ...
          <div className="mt-auto pt-5 border-t border-border/20 flex items-center justify-between">
            <div className="flex flex-col">
              <span className="text-[9px] font-bold text-muted-foreground/40 uppercase tracking-widest leading-none mb-1">
                Captured At
              </span>
