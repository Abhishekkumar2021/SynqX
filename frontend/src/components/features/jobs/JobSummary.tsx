            ...
            <BarChart data={data}>
              <Bar dataKey="duration" radius={[4, 4, 0, 0]}>
                {data.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={entry.duration > 60 ? 'oklch(0.62 0.19 25)' : 'oklch(0.7 0.18 55)'}
                    fillOpacity={0.4}
                    stroke={entry.duration > 60 ? 'oklch(0.62 0.19 25)' : 'oklch(0.7 0.18 55)'}
                  />
                ))}
              </Bar>
            </BarChart>
...
export const JobSummary: React.FC<JobSummaryProps> = ({ job, run }) => {
  ...
  const completedSteps = steps.filter(
    (s: any) => s.status === 'success' || s.status === 'completed' || s.status === 'warning'
  ).length
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
              ...
              isSuccess
                ...
                : isFailed
                  ...
                  : isCancelled
                    ...
                    : isQueued
                      ? 'bg-purple-500/3 border-purple-500/20'
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  {run?.run_number && (
                    <Badge
                      variant="secondary"
                      className="px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-widest bg-muted/50 text-muted-foreground border-none"
                    >
                      #{run.run_number}
                    </Badge>
                  )}
                  {job.queue_name ? (
                    <Badge
                      variant="outline"
                      className="px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-widest bg-primary/5 border-primary/20 text-primary flex items-center gap-1.5"
                    >
                    ...
                    <Badge
                      variant="outline"
                      className="px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-widest bg-blue-500/5 border-blue-500/20 text-blue-600 flex items-center gap-1.5"
                    >
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs md:text-sm font-medium text-muted-foreground max-w-md leading-relaxed opacity-70">
                    {isSuccess
                      ? 'Orchestration finalized. All data packets processed and target states synchronized.'
                      : isFailed
                        ? `Orchestration halted due to a terminal error: ${run?.error_message || 'System failure.'}`
                        : isCancelled
                          ? 'Orchestration terminated by operator intervention.'
                          : isQueued
                            ? `Standing by. Awaiting connection from an agent in the '${job.queue_name}' group.`
          ...
          <div className="xl:col-span-4 flex flex-col gap-4">
            ...
            <div className="p-5 rounded-[1.5rem] border border-border/40 bg-card/30 backdrop-blur-md flex flex-col justify-between flex-1">
              ...
              <div className="flex items-end justify-between">
                <span className="text-2xl font-bold text-foreground">
                  {formatDurationMs(job.execution_time_ms)}
                </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="p-6 rounded-[2rem] border border-border/40 bg-card/40 backdrop-blur-xl flex items-center gap-6">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xl font-bold tracking-tighter text-foreground truncate">
                {formatNumber(run?.total_extracted || 0)}
              </p>
            </div>
          </div>
          <div className="p-6 rounded-[2rem] border border-border/40 bg-card/40 backdrop-blur-xl flex items-center gap-6">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xl font-bold tracking-tighter text-foreground truncate">
                {formatNumber(run?.total_loaded || 0)}
              </p>
            </div>
          </div>
          <div className="p-6 rounded-[2rem] border border-border/40 bg-card/40 backdrop-blur-xl flex items-center gap-6">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xl font-bold tracking-tighter text-amber-600 truncate">
                {formatNumber(
                  steps.reduce((sum: number, s: any) => sum + (s.records_error || 0), 0)
                )}
              </p>
            </div>
          </div>
          <div className="p-6 rounded-[2rem] border border-border/40 bg-card/40 backdrop-blur-xl flex items-center gap-6">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xl font-bold tracking-tighter text-destructive truncate">
                {formatNumber(run?.total_failed || 0)}
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center gap-4">
              ...
              {run?.total_failed > 0 && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold text-destructive/80 uppercase">
                    Fault detected
                  </span>
                </div>
              )}
                ...
                <div key={step.id} className="group relative flex items-stretch gap-4 md:gap-6">
                  ...
                  <div
                    ...
                    className={cn(
                      ...
                      sIsRunning
                        ? 'bg-primary/4 border-primary/30'
                        : hasViolations
                          ? 'bg-amber-500/2 border-amber-500/20 shadow-inner'
                  ...
                  >
                    <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
                      <div className="flex items-start gap-4">
                        ...
                        >
                          {step.operator_type === 'extract' ? (
                            <Database size={18} />
                          ) : step.operator_type === 'load' ? (
                        ...
                        <div className="space-y-1.5">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-sm tracking-tight uppercase text-foreground">
                              {step.node?.name || step.operator_type || 'Node'}
                            </span>
                            <Badge
                              variant="outline"
                              className="px-1.5 py-0 rounded text-[8px] font-mono opacity-40 border-border/40"
                            >
                              ID:{step.node_id}
                            </Badge>
                            {hasViolations && (
                              <Badge className="px-1.5 py-0 rounded text-[8px] font-bold uppercase tracking-widest bg-amber-500 text-white border-none shadow-sm">
                                Violations
                              </Badge>
                            )}
                            {step.retry_count > 0 && (
                              <Badge
                                variant="destructive"
                                className="px-1.5 py-0 rounded text-[8px] font-bold uppercase tracking-widest animate-pulse"
                              >
                          ...
                          <div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-[9px] font-bold text-muted-foreground/50 uppercase tracking-wider">
                            <div className="flex items-center gap-2">
                              <span className={cn(sIsRunning && 'text-primary')}>
                                {step.status}
                              </span>
                              ...
                              <div className="flex items-center gap-1.5 px-1.5 py-0.5 rounded-md bg-amber-500/5 border border-amber-500/10">
                                ...
                                <span className="text-amber-600/80">
                                  {formatDurationMs(step.duration_seconds * 1000)}
                                </span>
                              </div>
                            </div>
                            {step.records_out > 0 && (
                              <div className="flex items-center gap-1.5 px-1.5 py-0.5 rounded-md bg-emerald-500/5 border border-emerald-500/10">
                                ...
                                <span className="text-emerald-600/80">
                                  {((step.records_out || 0) / (step.duration_seconds || 1)).toFixed(
                                    1
                                  )}{' '}
                                  rps
                                </span>
                          ...
                          {(step.records_filtered > 0 || step.records_error > 0) && (
                            <>
                              ...
                              {step.records_filtered > 0 && (
                                <div className="space-y-1 min-w-[50px]">
                                  <div className="text-[8px] font-bold uppercase text-muted-foreground/40">
                                    Filtered
                                  </div>
                                  <div className="text-xs font-bold text-amber-500/80">
                                    {formatNumber(step.records_filtered || 0)}
                                  </div>
                                </div>
                              )}
                              {step.records_error > 0 && (
                                <div className="space-y-1 min-w-[50px]">
                                  ...
                                  <div className="text-xs font-bold text-destructive/80">
                                    {formatNumber(step.records_error || 0)}
                                  </div>
                                </div>
                              )}
                            </>
                      ...
                      <div className="mt-4 p-4 rounded-xl bg-destructive/5 border border-destructive/10 space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-bold uppercase tracking-widest text-destructive/80">
                            {step.error_type || 'Execution Fault'}
                          </span>
                        </div>
                        <p className="text-[11px] font-medium text-destructive/90 leading-relaxed pl-5">
                          {step.error_message}
                        </p>
                      </div>
                    )}
