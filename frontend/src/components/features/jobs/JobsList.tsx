...
import { Search, Timer, Calendar, MoreVertical, Ban, ArrowRight, History, Activity, ShieldAlert, CheckCircle2 } from 'lucide-react';
    ...
    const filteredJobs = React.useMemo(() => {
        ...
        if (filter) {
            ...
            result = result.filter(j => 
                ...
                pipelines.find(p => p.id === j.pipeline_id)?.name.toLowerCase().includes(q)
            );
        }
                ...
                <div className="p-5 flex flex-col gap-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-foreground/70">Audit Registry</h3>
                        </div>
                        <Badge variant="outline" className="h-6 px-3 rounded-xl border-border/40 text-[10px] font-bold uppercase tracking-tight text-muted-foreground/60 bg-muted/10 shadow-sm">
                                ...
                                <div className="flex items-start justify-between">
                                    <div className="flex items-center gap-2">
                                        <div className={cn(
                                            ...
                                            isSelected ? "bg-primary text-primary-foreground shadow-md" : "bg-muted text-muted-foreground group-hover:bg-muted/80"
                                        )}>
                                            RUN-{job.id}
                                        </div>
                                    ...
                                    <div onClick={(e) => e.stopPropagation()}>
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-48 rounded-xl border-border/60 shadow-lg p-1 backdrop-blur-xl">
                                                <DropdownMenuItem onClick={() => onSelect(job.id)} className="rounded-lg font-medium text-xs py-2 cursor-pointer">
