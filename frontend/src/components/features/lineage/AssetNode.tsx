...
const AssetIcon = ({ type, className }: { type: string; className?: string }) => {
  ...
  // Files
  if (t.includes('s3') || t.includes('aws'))
    return <HardDrive className={cn('text-orange-400', className)} />
  if (t.includes('gcs') || t.includes('google'))
  ...
  // APIs
  if (t.includes('api') || t.includes('http'))
    return <Globe className={cn('text-primary', className)} />

  return <Table className={cn('text-muted-foreground', className)} />
}

export const AssetNode = memo(({ data, selected }: NodeProps) => {
  const nodeData = data as unknown as LineageNode['data'] & {
    label: string
  ...
  const row_count =
    nodeData.row_count !== undefined && nodeData.row_count !== null
      ? Number(nodeData.row_count)
      : null
  const size_bytes =
    nodeData.size_bytes !== undefined && nodeData.size_bytes !== null
      ? Number(nodeData.size_bytes)
      : null
  ...
  return (
    <div
      className={cn(
        ...
        isHighlighted &&
          'ring-2 ring-primary border-primary z-50 scale-[1.02] shadow-xl shadow-primary/20',
                ...
                className={cn(
                  ...
                  last_run_status === 'success'
                    ...
                    : last_run_status === 'failed'
                      ? 'bg-destructive shadow-[0_0_4px_currentColor]'
                      : 'bg-muted-foreground'
                )}
              />
            </div>
      ...
      <div className="p-3.5 space-y-3">
        {fqn && (
          <div className="flex items-center gap-2 p-1.5 rounded-lg bg-muted/30 border border-border/20 text-xs text-muted-foreground/80 group-hover:bg-muted/50 transition-colors">
            ...
            <span className="truncate font-mono text-[10px]" title={fqn}>
              {fqn}
            </span>
          </div>
        )}
      ...
      <div
        className={cn(
          ...
          selected
            ...
            : 'opacity-0 -translate-y-2 h-0 overflow-hidden pb-0 group-hover:opacity-100 group-hover:translate-y-0 group-hover:h-auto group-hover:pb-3.5'
        )}
      >
        {last_updated ? (
          <Badge
            variant="outline"
            className="h-5 px-1.5 text-[9px] bg-background/50 border-border/40 text-muted-foreground"
          >
