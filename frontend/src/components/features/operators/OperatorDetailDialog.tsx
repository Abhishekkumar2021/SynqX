...
import { Sliders, Layers, Terminal } from 'lucide-react'
...
export const OperatorDetailDialog: React.FC<OperatorDetailDialogProps> = ({
  selectedOp,
  open,
  onOpenChange,
}) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl p-0 gap-0 overflow-hidden rounded-2xl border-border/60 bg-background/95 shadow-2xl backdrop-blur-3xl">
        <VisuallyHidden.Root>
          ...
          <DialogDescription>
            View parameters and definition for the {selectedOp.name} operator.
          </DialogDescription>
        </VisuallyHidden.Root>
        <div className="flex flex-col h-[70vh]">
            ...
            <div className="flex items-center gap-6 relative z-10">
              ...
              <div className="space-y-1.5 flex-1 min-w-0">
                <div className="flex items-center gap-2.5">
                  ...
                  <Badge
                    variant="secondary"
                    className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md bg-muted/50 border-border/40"
                  >
                    {selectedOp.category}
                  </Badge>
                </div>
          ...
          <Tabs defaultValue="config" className="flex-1 flex min-h-0">
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-background/50 overflow-hidden relative">
              <TabsContent
                value="config"
                className="flex-1 m-0 overflow-y-auto p-8 animate-in fade-in duration-300"
              >
                <div className="space-y-6">
                  <div className="flex items-center gap-4">
                    <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground shrink-0">
                  ...
                  <div className="grid gap-3">
                    {Object.entries(selectedOp.configSchema).map(([key, type]) => {
                      ...
                      return (
                        <div
                          key={key}
                          className="flex items-center justify-between p-4 rounded-xl border border-border/40 bg-background/50 hover:bg-background/80 hover:border-border/60 transition-all group"
                        >
                          ...
                          <Badge
                            variant="outline"
                            className="text-[10px] font-mono font-medium text-muted-foreground bg-muted/30 border-border/40"
                          >
                            {cleanType}
                          </Badge>
                        </div>
              ...
              <TabsContent
                value="example"
                className="flex-1 m-0 animate-in fade-in duration-300 overflow-hidden relative"
              >
                <CodeBlock
