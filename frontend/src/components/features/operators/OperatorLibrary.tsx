...
export const OperatorLibrary: React.FC = () => {
  ...
  const searchResults = useFuzzySearch(OPERATORS, search, {
  ...
  const categories = useMemo(
    () => ['all', ...Array.from(new Set(OPERATORS.map((t) => t.category)))],
    []
  )
            ...
            >
              {filtered.map((op, idx) => (
                ...
                >
                  ...
                  <div
                    className={cn(
                      'absolute -right-10 -top-10 h-32 w-32 blur-3xl rounded-full opacity-0 group-hover:opacity-20 transition-opacity duration-500',
                      op.color.split(' ')[0].replace('text-', 'bg-')
                    )}
                  />
                  ...
                  <div className="flex items-start justify-between mb-5 relative z-10">
                    ...
                    <Badge
                      variant="outline"
                      className="rounded-lg text-[9px] px-2 py-0.5 border-border/60 font-bold uppercase tracking-widest bg-background/50"
                    >
                      {op.category}
                    </Badge>
                  </div>
                ...
                >
                  ...
                  <div className="col-span-1 flex justify-end">
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-8 w-8 rounded-lg group-hover:bg-primary/10 group-hover:text-primary"
                    >
                      <Info size={16} />
                    </Button>
                  </div>
