...
export const PipelineCanvas: React.FC = () => {
  ...
  const isLoading =
    isLoadingPipeline || (!!versionIdParam && isLoadingVersion) || (isDiffMode && isLoadingDiff)
  ...
  const filteredNodes = useMemo(() => {
    ...
    return nodes.map((n) => ({
      ...n,
      selected:
        n.data.label?.toLowerCase().includes(canvasSearch.toLowerCase()) ||
        n.id.toLowerCase().includes(canvasSearch.toLowerCase()),
    }))
  }, [nodes, canvasSearch])
    ...
    if (isDiffMode && diffData) {
      ...
      const loadDiff = async () => {
        ...
        const flowNodes: AppNode[] = vData.nodes.map((n: ApiNode) => {
          ...
          if (diffData.nodes.modified.some((m: any) => m.node_id === n.node_id))
            diffStatus = 'modified'
      ...
      try {
        ...
        if (isNew) {
          ...
          const newVersion = await createPipelineVersion(parseInt(id!), {
            ...
            edges: apiEdges,
            version_notes:
              notes || (deploy ? `Deployed at ${new Date().toLocaleTimeString()}` : 'Auto-save'),
          })
              ...
              <span className="hidden sm:inline">
                {isDiffMode ? (
                  <span className="flex items-center gap-1 text-primary">
                    v{diffData?.base_version || '?'} <GitCompare size={10} /> v
                    {diffData?.target_version || '?'}
                  </span>
                ) : isNew ? (
                  'v1'
                ) : (
                  `v${versionIdParam || pipeline?.latest_version?.version || pipeline?.published_version?.version || '?'}`
                )}
              </span>
              {!isNew && !isDiffMode && (
                <Button
              ...
              <TooltipTrigger asChild>
                <Button
                  ...
                  className={cn(
                    ...
                    showLogicView && 'bg-primary/10 text-primary'
                  )}
          ...
          <div className="flex-1 relative overflow-hidden">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge className="h-4 px-1.5 bg-amber-500/20 text-amber-500 border-none text-[9px] font-bold">
                        v{diffData?.base_version} → v{diffData?.target_version}
                      </Badge>
                    </div>
            ...
            {versionIdParam && !isDiffMode && (
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-100 w-fit min-w-100 max-w-[90%] bg-background/60 backdrop-blur-2xl border border-primary/30 rounded-2xl px-6 py-4 flex items-center justify-between gap-8 shadow-2xl animate-in slide-in-from-bottom duration-500 ring-1 ring-white/5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge className="h-4 px-1.5 bg-primary/20 text-primary border-none text-[9px] font-bold">
                        v{versionIdParam}
                      </Badge>
                    </div>
                    <span className="text-xs font-medium text-muted-foreground">
                      You are inspecting a historical state. Edits are disabled.
                    </span>
                  </div>
                </div>
                <Button
                ...
                >
                  Exit View{' '}
                  <ExternalLink className="h-3.5 w-3.5 group-hover:rotate-45 transition-transform" />
                </Button>
              </div>
            )}
            ...
            >
              ...
              <Controls
                className="bg-background/40! backdrop-blur-xl! border-border/10! shadow-sm! rounded-xl! overflow-hidden m-4 border fill-foreground text-foreground transition-all hover:scale-105"
              ...
              <AnimatePresence>
                {!isDiffMode && !versionIdParam && isEditor && (
                  <Panel
                    position="top-center"
                    className="mt-6 pointer-events-none flex items-center gap-3"
                  >
                    {/* Group 1: History */}
                    <motion.div
                    ...
                    >
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button className="h-12 w-12 rounded-2xl bg-primary text-primary-foreground shadow-xl shadow-primary/30 hover:bg-primary/90 hover:scale-105 active:scale-95 transition-all p-0 border border-primary/20">
                                  ...
                                  <div className="grid gap-1">
                                    {category.items.map((item: OperatorDefinition, i) => (
                                      <DropdownMenuItem
                                        ...
                                        onClick={() =>
                                          onAddNode(item.type, item.opClass, item.label)
                                        }
                                      >
                                        <div
                                          className={cn(
                                            ...
                                            item.type === 'source'
                                              ? 'bg-chart-1/10 border-chart-1/20 text-chart-1 group-hover:bg-chart-1/20'
                                              : item.type === 'sink'
                                                ? 'bg-chart-2/10 border-chart-2/20 text-chart-2 group-hover:bg-chart-2/20'
                                                : item.type === 'validate'
                                                  ? 'bg-chart-4/10 border-chart-4/20 text-chart-4 group-hover:bg-chart-4/20'
                                                  : ['join', 'union', 'merge'].includes(item.type)
                                                    ? 'bg-chart-5/10 border-chart-5/20 text-chart-5 group-hover:bg-chart-5/20'
                                        ...
                                        <div className="flex flex-col gap-0.5 overflow-hidden">
                                          ...
                                          <span className="text-[10px] text-muted-foreground/60 truncate leading-none font-medium">
                                            {item.desc}
                                          </span>
                                        </div>
                                      </DropdownMenuItem>
              ...
              >
                <div className="p-6 border-b border-white/10 bg-muted/20 flex items-center justify-between shrink-0">
                  ...
                  <Button
                    ...
                    onClick={() => setShowLogicView(false)}
                  >
                    <X size={18} />
                  </Button>
                </div>
