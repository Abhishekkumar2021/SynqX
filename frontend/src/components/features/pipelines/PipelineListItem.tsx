...
export const PipelineListItem: React.FC<PipelineListItemProps> = ({
  pipeline,
  onRun,
  onOpenSettings,
  onViewVersions,
  ...
  const successRate =
    pipeline.stats && pipeline.stats.total_runs > 0
      ...
      >
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[10px] text-muted-foreground truncate max-w-[90%] font-medium">
              {pipeline.description || (
                <span className=" opacity-50 text-[9px]">No description</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="col-span-6 md:col-span-2 flex flex-col justify-center gap-1">
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 border-l border-border/20 pl-2">
              ...
              <span className="text-[9px] font-bold tabular-nums text-foreground/60">
                {pipeline.stats?.successful_runs ?? 0}
              </span>
              <XCircle className="h-2.5 w-2.5 text-destructive/60 ml-0.5" />
              <span className="text-[9px] font-bold tabular-nums text-foreground/60">
                {pipeline.stats?.failed_runs ?? 0}
              </span>
            </div>
          </div>
          <div className="flex items-center gap-1.5">
            ...
            <span
              className={cn(
                ...
                pipeline.stats?.total_quarantined === 0
                  ? 'text-emerald-500/60'
                  : 'text-amber-600/80'
              )}
            >
        ...
        <div className="col-span-6 md:col-span-2 flex flex-col justify-center gap-1">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[8px] text-muted-foreground font-bold uppercase tracking-tighter opacity-40">
              rows
            </span>
          </div>
          ...
          {pipeline.schedule_enabled && pipeline.stats?.next_scheduled_run && (
            <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-primary/5 border border-primary/10 w-fit">
              ...
              <span className="text-[8px] font-bold text-primary uppercase tracking-tighter">
                Next{' '}
                {formatDistanceToNow(new Date(pipeline.stats.next_scheduled_run), {
                  addSuffix: true,
                })}
              </span>
            </div>
        ...
        <div className="col-span-12 md:col-span-2 flex items-center justify-end gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-all pr-4">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="w-52 rounded-xl border-border/60 shadow-xl p-1"
              onClick={(e) => e.stopPropagation()}
            >
              <DropdownMenuItem
                className="cursor-pointer gap-2 rounded-lg font-medium text-xs py-2"
              ...
              <DropdownMenuItem
                className="cursor-pointer gap-2 rounded-lg font-medium text-xs py-2"
                onClick={() => onViewVersions(pipeline)}
              >
              ...
              <DropdownMenuItem
                className="cursor-pointer gap-2 rounded-lg font-medium text-xs py-2"
                onClick={() => onOpenSettings?.(pipeline)}
