...
export const PipelineSettingsDialog: React.FC<PipelineSettingsDialogProps> = ({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      if (pipeline)
        queryClient.invalidateQueries({ queryKey: ['pipeline', pipeline.id.toString()] })
    ...
    onError: (err: any) => {
      toast.error('Update Failed', {
        description:
          err.response?.data?.detail?.message || 'There was an error saving configurations.',
      })
    },
  })
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl p-0 gap-0 overflow-hidden rounded-2xl border-border/40 bg-background/95 shadow-2xl backdrop-blur-3xl">
        <VisuallyHidden.Root>
          ...
          <DialogDescription>
            Configure parameters, scheduling, and resources for the pipeline.
          </DialogDescription>
        </VisuallyHidden.Root>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-3">
                  ...
                  <Badge
                    variant="secondary"
                    className="bg-muted/50 border-border/40 text-[9px] font-bold tracking-widest px-2 py-0.5 rounded-md uppercase"
                  >
                    Instance Settings
                  </Badge>
                </div>
                <p className="text-sm font-medium text-muted-foreground mt-1 opacity-80">
                  Control logic, scheduling and resources for{' '}
                  <span className="text-foreground font-bold">{pipeline?.name}</span>
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-background/50 overflow-hidden relative">
              ...
              >
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                  <TabsContent
                    value="general"
                    className="m-0 p-8 animate-in fade-in duration-300 space-y-8"
                  >
                      ...
                      <div className="p-6 flex items-center justify-between border-b border-border/20">
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-muted-foreground font-medium opacity-70">
                            Enable automated sync cycles via cron.
                          </p>
                        </div>
                        <Switch
                    ...
                    <div className="grid gap-8 max-w-2xl">
                      ...
                      <div className="space-y-6 p-6 rounded-2xl border border-border/40 bg-muted/5">
                        <div className="flex items-center justify-between">
                          ...
                          <Badge className="bg-primary/10 text-primary border border-primary/20 font-bold px-2 py-0.5 rounded-md text-[10px]">
                            {priority} / 10
                          </Badge>
                        </div>
                        <Controller
                      ...
                      <div className="space-y-4 p-6 rounded-2xl border border-border/40 bg-muted/5">
                        <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground block mb-2">
                          Fault Recovery
                        </Label>
                        <Controller
                          control={control}
                          ...
                          render={({ field }) => (
                            <Select onValueChange={field.onChange} value={field.value}>
                              ...
                              <SelectContent className="rounded-xl border-border/40 backdrop-blur-xl bg-background/95">
                                ...
                                <SelectItem value="fixed" className="text-xs font-bold">
                                  Fixed Interval
                                </SelectItem>
                                <SelectItem value="linear_backoff" className="text-xs font-bold">
                                  Linear Scale
                                </SelectItem>
                                <SelectItem
                          ...
                          {watch('retry_strategy') !== 'none' && (
                            ...
                            >
                              <div className="space-y-1.5">
                                <Label className="text-[9px] font-bold uppercase text-muted-foreground opacity-60">
                                  Retries
                                </Label>
                                <Input
                                  type="number"
                    ...
                    <div className="grid gap-8 max-w-2xl">
                        ...
                        <div className="grid grid-cols-2 gap-6 p-6 rounded-2xl border border-border/40 bg-muted/5">
                          <div className="space-y-2">
                            ...
                            <p className="text-[10px] text-muted-foreground px-1">
                              Alert if execution takes longer than this.
                            </p>
                          </div>
                          <div className="space-y-2">
                            <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground ml-1">
                            ...
                            />
                            <p className="text-[10px] text-muted-foreground px-1">
                              Alert if not completed by this time daily.
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="p-6 rounded-2xl border border-border/40 bg-muted/5 space-y-4">
                          ...
                          <div className="grid gap-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                            {allPipelines
                              ...
                              .map((p) => (
                                <div
                                  key={p.id}
                                  className="flex items-center space-x-3 p-3 rounded-xl bg-background border border-border/40 hover:border-primary/30 transition-all"
                                >
                                  <Switch
                                    id={`dep-${p.id}`}
                                  ...
                                  >
                                    {p.name}
                                    <span className="ml-2 text-[10px] font-medium text-muted-foreground opacity-60">
                                      ID: {p.id}
                                    </span>
                                  </Label>
                                </div>
                          ...
                          <p className="text-[10px] text-muted-foreground px-1">
                            This pipeline will auto-trigger when any selected upstream pipeline
                  ...
                  <TabsContent
                    value="governance"
                    className="m-0 p-8 animate-in fade-in duration-300"
                  >
