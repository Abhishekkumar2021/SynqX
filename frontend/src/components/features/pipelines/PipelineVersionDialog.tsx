...
export const PipelineVersionDialog: React.FC<PipelineVersionDialogProps> = ({
  ...
  const publishMutation = useMutation({
    mutationFn: (versionId: number) =>
      publishPipelineVersion(pipelineId, versionId),
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["pipeline", pipelineId.toString()],
      });
    },
    ...
    onError: (err: any) => {
      toast.error("Publish Failed", {
        description:
          err.response?.data?.detail?.message || "Could not publish version.",
      });
    },
  });
  ...
  const runMutation = useMutation({
    ...
    onError: (err: any) => {
      toast.error("Run Failed", {
        description:
          err.response?.data?.detail?.message || "Could not trigger pipeline.",
      });
    },
  });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl h-150 flex flex-col p-0 overflow-hidden border-border/60 bg-background/95 backdrop-blur-xl shadow-2xl rounded-[2rem] ring-1 ring-white/5 dark:ring-white/10">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 custom-scrollbar relative bg-background">
          {isLoading ? (
            ...
            <div className="relative space-y-4">
              ...
              <div className="absolute left-6.75 top-6 bottom-6 w-px bg-border/40 hidden md:block" />
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 flex flex-col gap-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className="md:hidden font-black text-xl text-primary">
                          v{version.version}
                        </span>
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                onClick={() => {
                                  navigate(
                                    `/pipelines/${pipelineId}?version=${version.version}`
                                  );
                            ...
                            <TooltipContent className="text-[10px] font-black uppercase tracking-widest px-2 py-1">
                              Inspect
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                    ...
                    <div className="flex items-center justify-between mt-auto">
                      ...
                      <span className="text-[8px] font-black text-muted-foreground/30 uppercase tracking-[0.2em] italic">
                        Immutable Snapshot
                      </span>
                    </div>
                  </div>
