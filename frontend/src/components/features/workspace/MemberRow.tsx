...
export const MemberRow: React.FC<MemberRowProps> = ({
    ...
    const getRoleBadge = (role: string) => {
        switch (role) {
            case WorkspaceRole.ADMIN: return <Badge className="bg-primary/10 text-primary border-primary/20 hover:bg-primary/20">Admin</Badge>;
            case WorkspaceRole.EDITOR: return <Badge className="bg-blue-500/10 text-blue-500 border-blue-500/20 hover:bg-blue-500/20">Editor</Badge>;
            default: return <Badge variant="secondary" className="hover:bg-muted">Viewer</Badge>;
        }
    };
                ...
                {isAdmin && !isSelf ? (
                    ...
                    >
                        ...
                        <SelectContent className="rounded-xl border-border/60 shadow-2xl">
                            <SelectItem value={WorkspaceRole.ADMIN} className="text-xs font-bold py-2.5">Admin</SelectItem>
                            <SelectItem value={WorkspaceRole.EDITOR} className="text-xs font-bold py-2.5">Editor</SelectItem>
                            <SelectItem value={WorkspaceRole.VIEWER} className="text-xs font-bold py-2.5">Viewer</SelectItem>
                        </SelectContent>
                    </Select>
                ) : (
                ...
                <td className="px-6 py-3 text-right pr-8">
                    {!isSelf ? (
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="rounded-2xl glass-card border-border/60 shadow-2xl p-1.5 min-w-48">
                                <DropdownMenuItem 
