...
import { Laptop, ChevronDown, Check, Box, RefreshCw, Loader2, Cpu } from 'lucide-react'
...
export const AgentGroupSwitcher: React.FC = () => {
  ...
  const updateWsMutation = useMutation({
    ...
    onSuccess: (_data, variables) => {
      ...
      toast.success(
        variables === 'internal' ? 'Reverted to Internal Worker' : 'Routing Environment Updated'
      )
    },
  })
  ...
  return (
    <DropdownMenu>
          ...
          >
            {updateWsMutation.isPending ? (
              ...
              <Cpu
                className={cn(
                  'h-3 w-3',
                  currentGroup !== 'internal' ? 'text-emerald-500' : 'text-primary/60'
                )}
              />
            )}
          </div>
      ...
      <DropdownMenuContent
        align="end"
        className="w-64 rounded-2xl glass-card shadow-2xl p-2 border-border/40 bg-background/95 backdrop-blur-xl"
      >
            ...
            {agentGroups.map((group) => (
              ...
              >
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[9px] opacity-50 font-medium tracking-tight">
                    Remote Execution
                  </span>
                </div>
