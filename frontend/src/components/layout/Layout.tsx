...
export const Layout: React.FC<LayoutProps> = () => {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden font-sans antialiased bg-transparent">
      ...
      {/* --- Main Content Area --- */}
      <motion.div layout className="flex flex-1 flex-col overflow-hidden relative">
        {/* --- Top Header --- */}
        <TopHeader setIsMobileMenuOpen={setIsMobileMenuOpen} setIsPaletteOpen={setIsPaletteOpen} />
        ...
        <main className="flex-1 overflow-auto p-4 md:p-6 scrollbar-thin scrollbar-thumb-border/50">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className={cn('mx-auto h-full w-full', isZenMode ? 'max-w-none' : 'max-w-8xl')}
            >
              <Outlet />
            </motion.div>
          </AnimatePresence>
