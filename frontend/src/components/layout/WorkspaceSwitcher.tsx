    ...
    >
      <div
        className={cn(
          ...
          variant === 'header'
            ? 'h-6 w-6 bg-primary/10 text-primary group-hover:bg-primary group-hover:text-primary-foreground'
            : isCollapsed
              ? 'h-6 w-6'
              : 'h-8 w-8 bg-primary/10 text-primary group-hover:bg-primary group-hover:text-primary-foreground'
        )}
      >
        ...
        <div className="flex flex-1 flex-col items-start overflow-hidden whitespace-nowrap mr-1">
          <div className="flex items-center gap-2 mb-0.5 w-full">
            ...
            {activeWorkspace?.default_agent_group && (
              <span
                className="ml-auto text-[8px] font-semibold uppercase tracking-tighter text-emerald-500 bg-emerald-500/10 px-1.5 py-px rounded border border-emerald-500/20 truncate max-w-16"
                title={`Agent: ${activeWorkspace.default_agent_group}`}
              >
                {activeWorkspace.default_agent_group}
              </span>
            )}
      ...
      {(!isCollapsed || variant === 'header') && (
        <ChevronsUpDown className="h-3 w-3 ml-auto opacity-30 group-hover:opacity-100 transition-opacity" />
      )}
    </button>
  )
  ...
  return (
    <div
      className={cn(
        ...
        variant === 'sidebar' && !isCollapsed && 'px-2.5'
      )}
    >
      <DropdownMenu>
