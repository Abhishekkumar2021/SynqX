...
import { 
    Workflow, Network, Search, Zap, 
    Moon, Sun, Home, LogOut, Hash, Book, Layout as LayoutIcon,
...
export const GlobalCommandPalette: React.FC<GlobalCommandPaletteProps> = ({ isOpen, onClose }) => {
    ...
    // Queries
    const { data: pipelines } = useQuery({ queryKey: ['pipelines'], queryFn: getPipelines, enabled: isOpen });
    const { data: connections } = useQuery({ queryKey: ['connections'], queryFn: getConnections, enabled: isOpen });
    const { data: jobs } = useQuery({ queryKey: ['jobs'], queryFn: () => getJobs(5), enabled: isOpen });
                ...
                <CommandList className="max-h-[480px] custom-scrollbar pb-4 relative z-10 bg-transparent">
                    <CommandEmpty className="py-12 text-center flex flex-col items-center gap-4">
                        ...
                        <div className="space-y-1">
                            <p className="text-sm font-bold text-foreground/60 uppercase tracking-widest">No results discovered</p>
                            <p className="text-[10px] font-medium text-muted-foreground/40 uppercase tracking-widest">Try a different keyword or command</p>
                        </div>
                    </CommandEmpty>
                    ...
                    >
                        ...
                        />
                        <PaletteItem 
                            icon={theme === 'dark' ? <Sun className="h-4 w-4 text-amber-500" /> : <Moon className="h-4 w-4 text-blue-500" />} 
                ...
                <div className="border-t border-border/20 px-6 py-4 flex items-center justify-between bg-muted/10 shrink-0">
                    <div className="flex items-center gap-6">
                        <div className="flex items-center gap-2 text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest">
                            <kbd className="px-1.5 py-0.5 rounded-md border border-border/40 bg-background font-mono text-xs shadow-sm shadow-black/5">↵</kbd>
                            <span>Open</span>
                        </div>
                        <div className="flex items-center gap-2 text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest">
                            <kbd className="px-1.5 py-0.5 rounded-md border border-border/40 bg-background font-mono text-xs shadow-sm shadow-black/5">↑↓</kbd>
                    ...
                    <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-primary/60">Synqx Omni-Hub</span>
                    </div>
                </div>
            </div>
        </CommandDialog>
...
const PaletteItem = ({ icon, title, subtitle, badge, onSelect, className, shortcut }: any) => (
    ...
    >
        ...
        <div className="flex flex-col flex-1 min-w-0">
            <div className="flex items-center gap-2">
                ...
                {badge && (
                    <Badge variant="outline" className={cn(
                        ...
                        badge === 'success' ? "bg-emerald-500/10 text-emerald-500" : 
                        badge === 'failed' ? "bg-destructive/10 text-destructive" :
            ...
            <span className="text-[10px] text-muted-foreground line-clamp-1 opacity-60 font-medium">{subtitle}</span>
        </div>
        {shortcut && (
