...
import { 
    Plus, Trash2, Copy, Key, Shield, 
    Calendar as CalendarIcon, Clock, CheckCircle2, 
    AlertCircle, ArrowRight, Loader2, Search,
      ...
      <div className="p-4 md:p-5 border-b border-border/40 bg-muted/10 flex flex-col md:flex-row items-center justify-between shrink-0 gap-4 md:gap-6">
        <div className="space-y-0.5 relative z-10">
          ...
          <p className="text-[10px] text-muted-foreground font-bold tracking-tight pl-1">
            <span className="text-foreground">{apiKeys?.length || 0}</span> ACTIVE KEYS <span className="mx-2 opacity-30">•</span> <span className="text-primary">{isAdmin ? 'ADMIN' : 'VIEWER'}</span> ACCESS
          </p>
        </div>
                  ...
                  {!createdKey ? (
                    <div className="flex flex-col h-full">
                            ...
                            <Popover>
                              <PopoverTrigger asChild>
                                ...
                                >
                                  ...
                                  {expiryDate ? format(expiryDate, "PPP") : <span className="opacity-60">Never (No expiration)</span>}
                                </Button>
                              </PopoverTrigger>
                      ...
                      <DialogFooter className="p-8 pt-4 bg-muted/5 border-t border-border/20 gap-3">
                        <Button variant="ghost" onClick={closeDialog} className="flex-1 rounded-xl h-12 font-bold uppercase tracking-widest text-[10px]">
                            Cancel
                        </Button>
                        <Button 
                      ...
                      <div className="p-8 text-center space-y-6">
                        ...
                        <div className="p-4 rounded-2xl bg-amber-500/5 border border-amber-500/10 flex gap-3 items-start text-left">
                          ...
                          <p className="text-[10px] text-amber-700/80 leading-normal font-bold uppercase tracking-tight">
                            Warning: You will not be able to see this key again once you close this dialog.
                          </p>
                        </div>
                      </div>
                        ...
                        {key.expires_at ? (
                          <div className="flex flex-col">
                            <span className={cn(
                                ...
                                isBefore(new Date(key.expires_at), new Date()) ? "text-destructive" : "text-muted-foreground/60"
                            )}>
                  ...
                  >
                    ...
                    <div className="space-y-1 mb-6">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button 
                            ...
                            onClick={() => {
                                toast.info("Full key hidden", { description: "API keys cannot be viewed after generation for security." });
                            }}
                        >
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-[11px] font-bold text-foreground/70">
                                    {key.last_used_at ? format(new Date(key.last_used_at), 'MMM d, HH:mm') : 'Never used'}
                                </span>
                            </div>
                        </div>
                        {key.scopes && (
                    ...
                    <div className="mt-auto space-y-3 pt-4 border-t border-border/10">
                      <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-tight text-muted-foreground/50">
                        <span>Issued On</span>
                        <span className="text-foreground/70 tabular-nums">{format(new Date(key.created_at), 'MMM d, yyyy')}</span>
                      </div>
                      <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-tight text-muted-foreground/50">
                        <span>Expiration</span>
                        <span className={cn(
                            "text-foreground/70 tabular-nums",
                            key.expires_at && isBefore(new Date(key.expires_at), new Date()) && "text-destructive"
                        )}>
