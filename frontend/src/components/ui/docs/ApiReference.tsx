...
const SchemaNode = ({
  ...
  const hasChildren =
    resolved.properties || (resolved.type === 'array' && resolved.items?.properties)
  ...
  return (
    <div className="py-2 border-b border-border/5 last:border-0">
      ...
      {resolved.description && (
        <p className="text-[10px] text-muted-foreground ml-6 mb-2 leading-relaxed">
          {resolved.description}
        </p>
      )}
      ...
      {resolved.enum && (
        <div className="ml-6 flex flex-wrap gap-1 mb-2">
          {resolved.enum.map((val: any) => (
            <code
              key={val}
              className="text-[9px] px-2 py-0.5 rounded bg-muted/30 text-foreground/70 font-mono"
            >
              {String(val)}
            </code>
          ))}
...
export function ApiReference() {
  ...
  const searchResults = useFuzzySearch(operations, search, {
    keys: ['path', 'summary', 'operationId'],
    threshold: 0.3,
  })

  const groupedOps = useMemo(() => {
    const filtered = searchResults.filter((op) => {
    ...
    return groups
  }, [operations, searchResults, methodFilter])
      ...
      <div
        className={cn(
          ...
          isFullscreen
            ...
            : 'relative h-auto lg:h-[900px] max-w-[1600px] mx-auto rounded-3xl'
        )}
      >
        {/* Sidebar */}
              ...
              {Object.entries(groupedOps).map(([tag, ops]) => (
                <div key={tag}>
                  <div className="px-2 mb-1.5 flex items-center gap-1.5 sticky top-0 bg-white/5 dark:bg-black/40 backdrop-blur-md py-1.5 z-10 rounded-md">
                    ...
                    <h3 className="text-[9px] font-bold uppercase tracking-[0.2em] text-muted-foreground/70">
                      {tag}
                    </h3>
                  </div>
                  ...
                  <div className="p-6 lg:p-8 space-y-8">
                      ...
                      <section className="space-y-3">
                        ...
                        <div className="space-y-2">
                          {activeOp.parameters.map((p) => (
                            <div
                              key={p.name}
                              className="p-3 rounded-lg border border-white/5 space-y-1.5 glass-card bg-white/5 dark:bg-black/5 hover:bg-white/10 dark:hover:bg-black/10 transition-colors"
                            >
                              <div className="flex items-start justify-between gap-3">
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-2 flex-wrap mb-0.5">
                                    <span className="font-mono text-xs font-bold">{p.name}</span>
                                    <span className="px-1.5 py-0.5 rounded bg-muted/50 text-[9px] font-bold uppercase">
                                      {p.in}
                                    </span>
                                    {p.required && (
                                  ...
                                  {p.schema?.enum && (
                                    <div className="flex flex-wrap gap-1 mt-1.5">
                                      {p.schema.enum.map((val: string) => (
                                        <code
                                          key={val}
                                          className="text-[9px] px-1.5 py-0.5 rounded bg-muted/30 font-mono"
                                        >
                                          {val}
                                        </code>
                                      ))}
                                ...
                                <Input
                                  type="text"
                                  placeholder={
                                    p.schema?.default ? String(p.schema.default) : `Enter ${p.name}`
                                  }
                    ...
                    <section className="space-y-4">
                      ...
                      <Tabs
                        ...
                        onValueChange={(v) => setActiveSchemaTab(v as any)}
                        className="border border-white/5 rounded-sm overflow-hidden glass-card shadow-xl"
                      >
                        ...
                        <div className="p-6">
                          ...
                          <TabsContent value="response" className="mt-0">
                            <div className="space-y-6">
                              {Object.entries(activeOp.responses || {}).map(
                                ([code, res]: [string, any]) => (
                  ...
                  <div className="p-6 lg:p-8 space-y-6">
                    ...
                    <section className="space-y-3">
                      ...
                      <Tabs
                        ...
                        onValueChange={setActiveCodeLang}
                        className="rounded-sm border border-white/5 overflow-hidden glass-card shadow-lg"
                      >
                        {/* Language Tabs */}
