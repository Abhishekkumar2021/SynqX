...
export const AuditLogsPage: React.FC = () => {
    ...
    const getSortIcon = (field: string) => {
        ...
        return sortOrder === 'desc' ? <ArrowDown className="ml-2 h-3 w-3 text-primary" /> : <ArrowUp className="ml-2 h-3 w-3 text-primary" />;
    };
    ...
    const hasActiveFilters = filters.eventType !== 'all' || filters.userId !== 'all' || filters.status !== 'all' || !!dateRange || !!searchQuery;
        ...
        if (status === 403) {
            return (
                <div className="h-full flex flex-col items-center justify-center text-center p-4">
                    ...
                    <Button variant="outline" className="mt-8 rounded-xl font-bold" onClick={() => window.history.back()}>
                        Return to Safety
                    </Button>
                </div>
            );
            ...
            <div className="flex-1 min-h-0 flex flex-col rounded-2xl border border-border/40 bg-background/40 backdrop-blur-xl shadow-xl overflow-hidden relative group">
                    ...
                    <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
                        ...
                        <Select value={filters.userId} onValueChange={(v) => setFilters(f => ({...f, userId: v}))}>
                            ...
                            <SelectContent className="rounded-xl border-border/60 shadow-2xl">
                                <SelectItem value="all" className="text-xs font-bold">All Actors</SelectItem>
                                {users?.map(user => (
                                    <SelectItem key={user.id} value={user.id.toString()} className="text-xs font-medium">
                                        {user.email}
                                    </SelectItem>
                                ))}
                            </SelectContent>
                        ...
                        <Select value={filters.status} onValueChange={(v) => setFilters(f => ({...f, status: v}))}>
                            ...
                            <SelectContent className="rounded-xl border-border/60 shadow-2xl">
                                ...
                                <SelectItem value="success" className="text-xs font-bold text-emerald-500">Success</SelectItem>
                                <SelectItem value="failure" className="text-xs font-bold text-destructive">Failure</SelectItem>
                            </SelectContent>
                        </Select>
                        ...
                        <Select value={filters.eventType} onValueChange={(v) => setFilters(f => ({...f, eventType: v}))}>
                            ...
                            <SelectContent className="rounded-xl border-border/60 shadow-2xl">
                                ...
                                <SelectItem value="pipeline" className="text-xs font-bold">Pipeline</SelectItem>
                                <SelectItem value="connection" className="text-xs font-bold">Connection</SelectItem>
                                <SelectItem value="asset" className="text-xs font-bold">Asset</SelectItem>
                                <SelectItem value="user" className="text-xs font-bold">User</SelectItem>
                                <SelectItem value="workspace" className="text-xs font-bold">Workspace</SelectItem>
                                <SelectItem value="api_key" className="text-xs font-bold">API Key</SelectItem>
                            </SelectContent>
                        </Select>
                ...
                {viewMode === 'list' && (
                    <div className="grid grid-cols-12 gap-4 px-6 py-3 border-b border-border/20 bg-muted/30 text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/50 shrink-0 sticky top-0 z-20 shadow-sm">
                        ...
                        <div className="col-span-2 hidden md:flex items-center cursor-pointer select-none group/col" onClick={() => toggleSort('user_id')}>
                            Actor {getSortIcon('user_id')}
                        </div>
                        <div className="col-span-3 hidden md:block">Target Resource</div>
                        <div className="col-span-2 flex items-center justify-end cursor-pointer select-none group/col" onClick={() => toggleSort('created_at')}>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Select value={limit.toString()} onValueChange={(val) => { setLimit(Number(val)); setPage(0); }}>
                                ...
                                <SelectContent className="rounded-xl border-border/60">
                                    ...
                                    <SelectItem value="20" className="text-[10px] font-bold">20</SelectItem>
                    ...
                    <div className="flex items-center gap-1.5 p-1 bg-background/50 rounded-2xl border border-border/40 shadow-sm">
                        ...
                        <div className="flex items-center gap-1 px-2">
                            {Array.from({ length: Math.min(5, totalPages) }).map((_, i) => {
                                ...
                                const pageNum = totalPages > 5 && page > 2 
                                    ? page - 2 + i 
                                    : i;
