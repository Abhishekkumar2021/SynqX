...
export const DashboardPage: React.FC = () => {
  ...
  const throughputData = React.useMemo(() => {
    ...
    return stats.throughput.map((p) => ({
      name:
        timeRange === '24h' || (timeRange === 'custom' && stats.throughput.length <= 48)
  ...
  const qualityData = React.useMemo(() => {
    ...
    return stats.quality_trend.map((p) => ({
      name:
        timeRange === '24h' || (timeRange === 'custom' && stats.quality_trend.length <= 48)
          ? format(parseISO(p.timestamp), 'HH:mm')
          : format(parseISO(p.timestamp), 'MMM dd'),
      valid: p.valid_rows,
  ...
  const distributionData = React.useMemo(() => {
    ...
    return stats.pipeline_distribution
      .map((d) => ({
        ...
        fill:
          d.status === 'active'
            ...
            : d.status === 'paused'
              ? 'hsl(var(--chart-5))'
              : d.status === 'broken' || d.status === 'failed'
                ? 'hsl(var(--destructive))'
                : 'hsl(var(--muted))',
      }))
      .filter((i) => i.value > 0)
  ...
  return (
    <motion.div
      className={cn('flex flex-col gap-8 pb-20 relative px-1', isZenMode && 'pt-4')}
      initial="hidden"
      animate="show"
      variants={container}
    >
      ...
      <motion.div
        variants={item}
        className="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-8 px-1 relative z-10"
      >
          ...
          <div className="flex items-center gap-2 bg-muted/30 p-1.5 rounded-xl border border-border/40 shadow-inner">
            <Select value={timeRange} onValueChange={setTimeRange}>
              ...
              <SelectContent className="rounded-xl border-border/40 shadow-2xl">
                <SelectItem value="24h" className="rounded-xl font-medium">
                  Last 24 Hours
                </SelectItem>
                <SelectItem value="7d" className="rounded-xl font-medium">
                  Last 7 Days
                </SelectItem>
                <SelectItem value="30d" className="rounded-xl font-medium">
                  Last 30 Days
                </SelectItem>
                <SelectItem value="all" className="rounded-xl font-medium">
                  All Time
                </SelectItem>
                <SelectItem value="custom" className="rounded-xl font-medium">
                  Custom Range
                </SelectItem>
              </SelectContent>
            </Select>
            ...
            {timeRange === 'custom' && (
              <DateRangePicker date={customRange} setDate={setCustomRange} />
            )}
          </div>
      ...
      <motion.div variants={item} className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        {isLoading ? (
          Array.from({ length: 6 }).map((_, i) => (
            <Skeleton key={i} className="h-40 w-full rounded-2xl bg-muted/20" />
          ))
        ) : (
          <>
            <StatsCard
      ...
      <div className="flex flex-col gap-8 px-1">
        ...
        <div className="grid gap-8 lg:grid-cols-12">
          <div className="lg:col-span-7">
            <DashboardWidget
              ...
              headerActions={
                <Tabs
                  ...
                  onValueChange={(v) => setThroughputView(v as 'jobs' | 'rows' | 'bytes')}
                >
                  <TabsList className="bg-background/50 h-10">
                    <TabsTrigger value="jobs" className="text-[10px] px-3">
                      Jobs
                    </TabsTrigger>
                    <TabsTrigger value="rows" className="text-[10px] px-3">
                      Rows
                    </TabsTrigger>
                    <TabsTrigger value="bytes" className="text-[10px] px-3">
                      Bytes
                    </TabsTrigger>
                  </TabsList>
                </Tabs>
            ...
            >
              <ExecutionThroughputChart data={throughputData} view={throughputView} />
            </DashboardWidget>
          </div>
                  ...
                  <div className="text-right">
                    <div className="flex items-center gap-2 justify-end">
                      <span className="text-xl font-bold tracking-tighter text-foreground">
                        {(stats?.total_pipelines ?? 0) > 0
                          ? Math.round(
                              ((stats?.pipeline_distribution?.find(
                      ...
                      <div
                        className={cn(
                          ...
                          ((stats?.total_pipelines ?? 0) > 0
                            ? (stats?.pipeline_distribution?.find(
                                (d: { status: string }) => d.status === 'active'
                              )?.count || 0) / (stats?.total_pipelines || 1)
                            : 0) > 0.9
          ...
          <div className="lg:col-span-8">
            <DashboardWidget
              ...
              headerActions={
                <Badge
                  variant="outline"
                  className="font-mono text-[9px] uppercase tracking-widest text-emerald-500 border-emerald-500/20 bg-emerald-500/5 px-2 py-0.5 rounded-full animate-pulse"
                >
                  Live Quality
                </Badge>
              }
