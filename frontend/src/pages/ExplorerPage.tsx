...
import { 
    Layers, Database, HardDrive, Globe, 
...
const CATEGORY_MAP: Record<string, { label: string, icon: any, color: string, route: string | null }> = {
    ...
    'unsupported': { label: 'Platform Connectors', icon: Server, color: 'text-muted-foreground/40', route: null },
};
                ...
                <div className="space-y-1.5">
                    <h2 className="text-3xl md:text-4xl font-bold tracking-tighter text-foreground flex items-center gap-3">
                        <div className="p-2 bg-primary/10 rounded-2xl ring-1 ring-border/50 backdrop-blur-md shadow-sm">
                            <Globe size={24} className="text-primary animate-spin-slow" style={{ animationDuration: '12s' }} />
                        </div>
                        Source Mesh
                    </h2>
                                ...
                                {[1, 2].map(i => (
                                    <div key={i} className="space-y-6">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                            {[1, 2, 3].map(j => <Skeleton key={j} className="h-48 rounded-[2rem]" />)}
                                        </div>
                                    </div>
                                ))}
                            </div>
                                ...
                                return (
                                    <section key={key} className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                                        <div className="flex items-center gap-4 px-2">
                                            <div className={cn("h-10 w-10 rounded-[1.25rem] bg-background border border-border/40 shadow-sm flex items-center justify-center ring-4 ring-muted/5", cat.color)}>
                                                <Icon size={20} />
                                            </div>
                                            <div className="flex flex-col">
                                                <h3 className="text-xl font-black uppercase tracking-tight leading-none text-foreground/90">{cat.label}</h3>
                                                <span className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-[0.2em] mt-1.5">{items.length} ACTIVE ENDPOINTS</span>
                                            </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                                                ...
                                                <div className="col-span-full py-16 text-center border-2 border-dashed border-border/20 rounded-[3rem] opacity-30 bg-muted/5">
                                                    <span className="text-xs font-black uppercase tracking-widest">No matching partitions</span>
                                                </div>
                                            )}
                                        </div>
                                    </section>
...
const ConnectionCard = ({ connection, onClick, colorClass, isUnsupported }: { connection: any, onClick: () => void, colorClass: string, isUnsupported?: boolean }) => {
    ...
    return (
        ...
        >
            ...
            {!isUnsupported && <div className="absolute top-0 right-0 p-24 -mr-12 -mt-12 bg-linear-to-br from-primary/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity rounded-full blur-3xl" />}
            
            <div className="relative z-10 h-full flex flex-col justify-between">
                <div className="flex items-start justify-between">
                    <div className="space-y-3">
                        <div className="flex items-center gap-2">
                            <Badge variant="outline" className={cn("h-5 px-2 text-[9px] font-black uppercase tracking-widest border-none bg-muted/50", colorClass)}>
                                {connection.connector_type}
                            </Badge>
                            {!isUnsupported && (
                                <div className={cn(
                                    "h-1.5 w-1.5 rounded-full shadow-[0_0_8px]",
                                    isHealthy ? "bg-emerald-500 shadow-emerald-500/40" : "bg-amber-500 shadow-amber-500/40"
                                )} />
                            )}
                        </div>
                    ...
                    <div className={cn(
                        ...
                        !isUnsupported && "group-hover:text-primary group-hover:border-primary/20 group-hover:shadow-primary/10"
                    )}>
                        {isUnsupported ? <X size={20} className="text-muted-foreground/40" /> : <ArrowRight size={20} className="transition-transform group-hover:translate-x-1 text-muted-foreground" />}
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-6">
                        {isUnsupported ? (
                            <span className="text-[9px] font-black text-muted-foreground/40 uppercase tracking-[0.2em]">Specialized Explorer Unavailable</span>
                        ) : (
                            <>
                                <div className="flex flex-col">
                                    <span className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-widest leading-none">Status</span>
                                    <span className={cn("text-[11px] font-bold mt-1.5 uppercase", isHealthy ? "text-emerald-500" : "text-amber-500")}>
                                        {connection.health_status}
                                    </span>
                                </div>
                                <div className="w-px h-6 bg-border/20" />
                                <div className="flex flex-col">
                                    <span className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-widest leading-none">Entities</span>
                                    <span className="text-[11px] font-bold mt-1.5 text-foreground/80">{connection.asset_count || 0} discovered</span>
                                </div>
                            </>
                        )}
                    </div>
