...
import { RefreshCw, Filter, Terminal, History as HistoryIcon } from 'lucide-react'
...
export const JobsPage: React.FC = () => {
  ...
  const pipelineIdFilter = searchParams.get('pipeline')
    ? parseInt(searchParams.get('pipeline')!)
    : null
  ...
  const selectedJob = useMemo(
    () => jobs?.find((j: Job) => j.id === selectedJobId),
    [jobs, selectedJobId]
  )
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-sm font-medium text-muted-foreground/60 tracking-tight">
              A high-fidelity registry for real-time pipeline monitoring, performance analytics, and
              log inspection.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <SelectContent className="rounded-2xl border-border/40 bg-background/80 backdrop-blur-xl shadow-2xl p-1.5">
              <SelectItem
                value="all"
                className="rounded-xl font-bold text-[11px] uppercase tracking-widest py-3"
              >
                All Pipelines
              </SelectItem>
              {pipelines?.map((p) => (
      ...
      <div className="flex-1 min-h-0 relative rounded-3xl border border-border/40 bg-background/20 backdrop-blur-md overflow-hidden shadow-2xl">
        ...
        <ResizablePanelGroup direction="horizontal" className="h-full w-full">
          ...
          <ResizablePanel
            ...
            minSize={20}
            maxSize={45}
            className="bg-card/30 backdrop-blur-xl border-r border-white/5"
          >
            <JobsList
              jobs={filteredJobs}
              pipelines={pipelines || []}
                ...
                <div className="flex flex-col items-center justify-center h-full text-muted-foreground/60 space-y-8 animate-in fade-in zoom-in-95 duration-500">
                  ...
                  <div className="text-center space-y-3 px-6">
                    ...
                    <p className="max-w-xs mx-auto text-sm font-medium leading-relaxed">
                      Select an execution from the forensic history to inspect real-time logs,
