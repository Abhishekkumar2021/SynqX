...
const LineageGraphComponent = ({ graphData }: LineageGraphProps) => {
  ...
  const { data: colLineage, isLoading: isLoadingColLineage } = useQuery({
    ...
    queryFn: () =>
      getColumnLineage(selectedAsset?.data?.asset_id as number, selectedColumn as string),
  ...
  const { data: colImpact, isLoading: isLoadingColImpact } = useQuery({
    ...
    queryFn: () =>
      getColumnImpact(selectedAsset?.data?.asset_id as number, selectedColumn as string),
    enabled: !!selectedAsset?.data?.asset_id && !!selectedColumn,
  })

  // 3. Calculate highlights based on source data (prevents infinite loops with state)
  const { highlightedNodes, highlightedEdges } = useLineagePathHighlight(
    graphData?.edges || [],
    colLineage
  )
          ...
          >
            <div className="p-6 border-b border-border/40 bg-muted/20 flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  <span className="text-foreground font-bold">{selectedAsset.data.label}</span>
                </p>
              </div>
              <Button
                variant="ghost"
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
                ...
                <div className="p-4 rounded-xl bg-card/50 border border-border/40 space-y-2">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium capitalize">
                      {selectedAsset.data.connection_type}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Rows (Est.)</span>
                    <span className="font-mono">
                      {selectedAsset.data.row_count?.toLocaleString() ?? '—'}
                    </span>
                  </div>
                ...
                <div className="p-4 rounded-xl bg-primary/5 border border-primary/10 space-y-4">
                  <Select value={selectedColumn || ''} onValueChange={setSelectedColumn}>
                    ...
                    <SelectContent className="glass border-border/40 rounded-xl">
                      {(selectedAsset.data as AssetNodeData).schema_metadata?.columns?.map(
                        (col: { name: string; type: string }) => (
                          ...
                          >
                            {col.name}{' '}
                            <span className="opacity-40 text-[10px] ml-1">({col.type})</span>
                          </SelectItem>
                        )
                      ) || (
                      ...
                      <div className="space-y-6 animate-in fade-in slide-in-from-top-2 duration-500">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="space-y-3 relative before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-0.5 before:bg-primary/20">
                              ...
                              <div className="flex gap-3 relative z-10">
                                ...
                                <div>
                                  <div className="text-[10px] font-bold uppercase text-primary tracking-tighter">
                                    Source
                                  </div>
                                  <div className="text-xs font-bold">
                                    {colLineage.origin_column_name}
                                  </div>
                                </div>
                              </div>
                        ...
                        {colImpact && (
                          <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                              {colImpact.impacts.length > 0 ? (
                                colImpact.impacts.map((imp, idx) => (
                                  <div
                                    key={idx}
                                    className="p-2 rounded-lg bg-destructive/5 border border-destructive/10 text-[11px] space-y-1"
                                  >
                                    <div className="flex justify-between items-start">
                                      ...
                                      <Badge
                                        variant="outline"
                                        className="text-[8px] h-3 px-1 border-destructive/20 text-destructive"
                                      >
                                        {imp.transformation_type}
                                      </Badge>
                                    </div>
              ...
              <div className="space-y-3">
                ...
                ) : impactData?.downstream_pipelines &&
                  impactData.downstream_pipelines.length > 0 ? (
                  <div className="space-y-2">
                    {impactData.downstream_pipelines.map((p) => (
                      <div
                        key={p.id}
                        className="flex items-center justify-between p-3 rounded-xl bg-blue-500/5 border border-blue-500/10 hover:border-blue-500/30 transition-colors cursor-pointer group/item"
                      >
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <span className="text-sm font-bold text-foreground/80 group-hover/item:text-primary">
                            {p.name}
                          </span>
                        </div>
                ...
                {impactData?.downstream_assets && impactData.downstream_assets.length > 0 ? (
                  <div className="space-y-2">
                    {impactData.downstream_assets.map((a) => (
                      <div
                        key={a.id}
                        className="p-3 rounded-xl bg-muted/30 border border-border/40 text-sm font-medium flex items-center gap-2"
                      >
...
export const LineagePage = () => {
  ...
  return (
    ...
    >
      <AnimatePresence mode="popLayout">
        {!isZenMode && (
          ...
          >
            <PageMeta
              title="Data Lineage"
              description="Visualize and trace data dependencies across your ecosystem."
            />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <motion.div
        layout
        className="shrink-0 flex flex-col md:flex-row md:items-center justify-between gap-3 p-0.5"
      >
