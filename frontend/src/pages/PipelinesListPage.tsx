...
import { Plus, Workflow, Activity, Search, Laptop, FileUp, Loader2 } from 'lucide-react'
  ...
  const handleImportYAML = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success('Pipeline Synchronized', {
        description: 'Definition imported successfully via YAML.',
      })
  ...
  const enrichedPipelines = useMemo(() => {
    ...
    return pipelines
      ...
      .filter((p) => {
        ...
        const matchesTag =
          tagFilter === 'all' || (p.tags && Object.keys(p.tags).includes(tagFilter))
          ...
          {isEditor && (
            <>
              ...
              <Button
                size="sm"
                variant="outline"
                className="rounded-xl font-bold text-[10px] uppercase tracking-widest gap-2 bg-background/50 border-border/40 hover:bg-background transition-all"
              ...
              <Link to="/pipelines/new" className="w-full md:w-auto">
                <Button
                  size="sm"
                  className="w-full md:w-auto rounded-xl shadow-lg shadow-primary/20 hover:shadow-primary/40 transition-all hover:scale-105 active:scale-95 font-semibold"
                >
          ...
          <div className="flex items-center gap-3 w-full md:max-w-2xl group">
            ...
            <Select value={tagFilter} onValueChange={setTagFilter}>
              ...
              <SelectContent className="rounded-2xl border-border/40 bg-background/80 backdrop-blur-xl shadow-2xl p-1.5">
                <SelectItem
                  value="all"
                  className="rounded-xl font-bold text-[10px] uppercase tracking-widest py-2.5"
                >
                  All Categories
                </SelectItem>
                {allTags.map((tag) => (
        ...
        {viewMode === 'grid' && (
          <div className="flex-1 overflow-y-auto p-4 md:p-6 scrollbar-thin scrollbar-thumb-border/50 hover:scrollbar-thumb-border/80 scrollbar-track-transparent">
            {enrichedPipelines.length === 0 ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-6 pb-20 md:pb-0">
                {enrichedPipelines.map((pipeline) => (
                  <PipelineGridItem
                    ...
                    onRun={
                      isEditor
                        ? (id, versionId) => runMutation.mutate({ id, versionId })
                        : undefined
                    }
