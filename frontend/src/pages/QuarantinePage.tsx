          ...
          <div className="flex items-center gap-3.5">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <div
                ...
                onClick={(e) => {
                  e.stopPropagation()
                  navigate(`/pipelines/${item.pipeline_id}`)
                }}
              >
          ...
          <div className="flex flex-col gap-1.5 p-4 rounded-2xl bg-muted/30 border border-border/20 shadow-inner">
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                ...
                <span className="text-xl font-bold tabular-nums text-destructive tracking-tighter">
                  {percentage}%
                </span>
              </div>
  ...
  return (
    ...
    >
      ...
      <div className="col-span-2 hidden md:flex flex-col items-center border-l border-border/20 pl-4">
        <span className="text-sm font-bold tabular-nums text-destructive tracking-tight">
          {formatNumber(item.row_count)}
        </span>
      ...
      <div className="col-span-2 hidden md:flex flex-col items-end border-l border-border/20 pl-4">
        <span className="text-[10px] font-bold text-foreground/70 whitespace-nowrap">
...
export const QuarantinePage = () => {
  ...
  const { data: stepData, isLoading: isLoadingStepData } = useQuery({
    ...
    queryFn: () =>
      getStepData(inspectingItem.run_id, inspectingItem.step_id, 'quarantine', 1000, 0),
    enabled: !!inspectingItem,
  })

  const isLoading = isLoadingStats || isLoadingItems

  const filteredItems = (quarantineItems || []).filter(
    (item: any) =>
  ...
  return (
    ...
    >
      <PageMeta
        title="Quarantine"
        description="Manage and resolve data quality issues and rejected records."
      />
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between shrink-0 gap-6 px-1">
          ...
          <div className="flex items-center gap-8 flex-wrap pl-1">
            <div className="flex items-center gap-3 transition-all group">
              ...
              <div className="flex flex-col -space-y-0.5">
                ...
                <span className="text-base font-bold tabular-nums text-destructive tracking-tighter">
                  {formatNumber(stats?.total_rejected_rows || 0)}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 transition-all group">
              ...
              <div className="flex flex-col -space-y-0.5">
                ...
                <span className="text-base font-bold tabular-nums text-amber-500 tracking-tighter">
            ...
            <div className="flex items-center gap-3 transition-all group">
              ...
              <div className="flex flex-col -space-y-0.5">
                ...
                <span className="text-base font-bold tabular-nums text-emerald-500 tracking-tighter">
        ...
        <div className="flex items-center gap-2 pt-2">
          <Button
            variant="outline"
            size="sm"
            className="rounded-xl gap-2 shadow-sm h-10 border-border/40 hover:bg-muted/50"
                ...
                >
                  {Array.from({ length: 8 }).map((_, i) => (
                    <Skeleton
                      ...
                      className={cn(
                        'rounded-2xl',
                        viewMode === 'grid' ? 'h-48 w-full' : 'h-16 w-full'
                      )}
                    />
                  ))}
                </div>
                ...
                <div className="h-100 flex flex-col items-center justify-center space-y-4 text-center">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground max-w-xs">
                      Try adjusting your filters or check back later after a pipeline run.
                    </p>
                  </div>
                </div>
              ) : (
      ...
      >
        <DialogContent
          className={cn(
            ...
            isMaximized
              ? 'max-w-[100vw] h-screen sm:rounded-none'
              : 'max-w-7xl h-[90vh] sm:rounded-2xl'
          )}
        >
          <DialogHeader className="p-6 border-b border-border/40 bg-muted/30 shrink-0 relative pr-20">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <DialogDescription
                  asChild
                  className="flex items-center gap-3 text-xs font-bold uppercase tracking-widest text-muted-foreground/60"
                >
                  <div className="flex items-center gap-3">
                    <span
                      className="flex items-center gap-1.5 hover:text-primary cursor-pointer transition-colors"
                      onClick={() => navigate(`/pipelines/${inspectingItem?.pipeline_id}`)}
                    >
