...
export const SettingsPage: React.FC = () => {
  ...
  return (
    <motion.div
      ...
      className={cn('flex flex-col gap-6 md:gap-8', isZenMode ? 'h-[calc(100vh-3rem)]' : 'h-full')}
    >
      <PageMeta
        title="Settings"
        description="Manage your workspace preferences and security configurations."
      />
      ...
      <div className="flex-1 min-h-0 flex flex-col md:flex-row rounded-2xl border border-border/40 bg-background/40 backdrop-blur-xl shadow-xl relative overflow-hidden">
        ...
        <aside className="w-full md:w-72 border-b md:border-b-0 md:border-r border-border/40 bg-muted/10 shrink-0 flex flex-col overflow-hidden">
          <div className="p-6 border-b border-border/40 bg-background/20">
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60 leading-none">
              Management Console
            </p>
          </div>
                  ...
                  <div
                    className={cn(
                      ...
                      activeTab === tab.id
                        ...
                        : 'bg-muted/50 group-hover:bg-muted text-muted-foreground'
                    )}
                  >
                ...
                >
                        ...
                        <div className="flex flex-col md:flex-row md:items-end gap-6">
                          <div className="relative">
                            <Avatar className="h-32 w-32 rounded-[2rem] border-4 border-background shadow-2xl relative z-10">
                              <AvatarImage
                                src={`https://api.dicebear.com/7.x/notionists/svg?seed=${user?.email || 'synqx'}`}
                              />
                          ...
                          <div className="flex-1 space-y-1 mb-2">
                            <h3 className="text-3xl font-bold tracking-tight text-foreground leading-none">
                              {user?.full_name || 'User Identity'}
                            </h3>
                            <div className="flex items-center gap-2">
                              <Badge
                                variant="secondary"
                                className="rounded-lg px-2 py-0.5 text-[9px] font-bold uppercase tracking-widest bg-primary/5 text-primary border-primary/10"
                              >
                                ROOT ACCOUNT
                              </Badge>
                              <span className="text-[11px] text-muted-foreground font-bold opacity-60 ">
                                {user?.email}
                              </span>
                            </div>
                          </div>
                      ...
                      <div className="px-8 py-5 bg-muted/10 border-t border-border/20 flex items-center justify-between">
                        <div className="flex items-center gap-2 text-muted-foreground">
                          <RefreshCw
                            className={cn(
                              'h-3.5 w-3.5',
                              profileMutation.isPending && 'animate-spin'
                            )}
                          />
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center gap-3 px-1">
                      ...
                      <h4 className="text-[11px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60 leading-none">
                ...
                >
                  <div className="p-8 rounded-[2.5rem] bg-emerald-500/5 border border-emerald-500/20 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl">
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        <h3 className="font-bold text-xl leading-none text-foreground uppercase tracking-tight">
                          Workspace Governance
                        </h3>
                        <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mt-2 opacity-60">
                          Identity, routing and shared protocols.
                        </p>
                      </div>
                    </div>
                    {activeWorkspace?.default_agent_group &&
                      activeWorkspace.default_agent_group !== 'internal' && (
                        <Badge
                          variant="outline"
                          className="rounded-xl px-4 py-2 bg-emerald-500/10 text-emerald-600 border-emerald-500/20 font-bold uppercase text-[10px] gap-2 shadow-xl backdrop-blur-md"
                        >
                  ...
                  <Card className="border-destructive/30 bg-destructive/5 backdrop-blur-xl rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
                    ...
                    <CardHeader className="p-8">
                      ...
                      <CardDescription className="font-bold text-xs uppercase opacity-60">
                        Permanently terminate your SynqX deployment and associated data.
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="p-8 pt-0 flex flex-col md:flex-row md:items-center justify-between gap-8">
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground font-medium max-w-sm  leading-relaxed">
                          Initiating this protocol will purge all pipelines, history, and metadata.
                          This action is atomic and terminal.
                        </p>
                      </div>
                      <Button
                        variant="destructive"
                      ...
                      <AlertDialogContent className="rounded-[2.5rem] border-destructive/20 bg-background/95 backdrop-blur-3xl shadow-2xl">
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription className="text-muted-foreground font-medium ">
                            This will permanently destroy your SynqX identity and all associated
                            virtual assets. This procedure cannot be aborted once confirmed.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter className="mt-6 gap-3">
                          ...
                          <AlertDialogAction
                            ...
                            className={cn(
                              ...
                              'rounded-xl font-bold uppercase text-[10px] tracking-widest shadow-xl shadow-destructive/20'
                            )}
                          >
                  ...
                  <div className="p-8 border-b border-border/40 bg-muted/10 flex flex-col lg:flex-row items-center justify-between shrink-0 gap-6">
                    <div className="space-y-1 relative z-10">
                      ...
                      <p className="text-[9px] text-muted-foreground font-bold tracking-[0.2em] uppercase opacity-60 pl-1">
                        <span className="text-foreground">{alerts?.length || 0}</span> ACTIVE RULES
                    ...
                    {loadingAlerts && !alerts ? (
                      <div className="p-8 space-y-6">
                        {[1, 2, 3].map((i) => (
                          <div
                            key={i}
                            className="flex justify-between items-center py-6 border-b border-border/10"
                          >
                          ...
                          <Table className="rounded-none border-none shadow-none">
                            <TableHeader className="bg-muted/20 border-b border-border/20">
                              <TableRow className="hover:bg-transparent border-none">
                                <TableHead className="pl-8 font-bold text-[9px] uppercase tracking-[0.2em] text-muted-foreground/50 py-4">
                                  Rule Configuration
                                </TableHead>
                                <TableHead className="font-bold text-[9px] uppercase tracking-[0.2em] text-muted-foreground/50 py-4">
                                  Trigger
                                </TableHead>
                                <TableHead className="font-bold text-[9px] uppercase tracking-[0.2em] text-muted-foreground/50 py-4 text-center">
                                  State
                                </TableHead>
                                <TableHead className="text-right pr-8 font-bold text-[9px] uppercase tracking-[0.2em] text-muted-foreground/50 py-4">
                                  Control
                                </TableHead>
                              </TableRow>
                            </TableHeader>
                            <TableBody className="divide-y divide-border/10">
                              {filteredAlerts.map((alert: any) => (
                                <TableRow
                                  key={alert.id}
                                  className="group hover:bg-primary/2 transition-colors border-b border-border/5"
                                >
                                  <TableCell className="pl-8 py-5">
                                    <div className="flex items-center gap-4">
                                      ...
                                      <div className="flex flex-col gap-0.5">
                                        ...
                                        <span className="text-[9px] text-muted-foreground font-bold tracking-tight opacity-60 ">
                                          {alert.delivery_method} • {alert.recipient || 'INTERNAL'}
                                        </span>
                                      </div>
                                    </div>
                                  </TableCell>
                                  <TableCell>
                                    <Badge
                                      variant="outline"
                                      className="text-[8px] font-bold uppercase tracking-widest bg-muted/30 border-border/40 px-2 py-0.5 rounded-md"
                                    >
                                      {alert.alert_type}
                                    </Badge>
                                  </TableCell>
                                    ...
                                    <div className="flex justify-center">
                                      <Switch
                                        ...
                                        onCheckedChange={(enabled) =>
                                          toggleAlertMutation.mutate({ id: alert.id, enabled })
                                        }
                            ...
                            {filteredAlerts.map((alert: any) => (
                              ...
                              >
                                ...
                                <div className="space-y-1">
                                  <h4 className="font-bold text-base tracking-tight text-foreground uppercase truncate">
                                    {alert.name}
                                  </h4>
                                  <p className="text-[10px] font-bold text-muted-foreground/60 line-clamp-2  leading-relaxed uppercase tracking-tighter">
                                    {alert.alert_type} Protocol
                                  </p>
                                </div>
                                <div className="mt-6 pt-4 border-t border-border/20 flex items-center justify-between">
                                  ...
                                  <span className="text-[9px] font-bold text-emerald-500/60 font-mono ">
                                    {alert.recipient || 'INTERNAL'}
                                  </span>
                                </div>
                              </motion.div>
                            ))}
