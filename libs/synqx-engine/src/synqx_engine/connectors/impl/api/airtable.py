...
class AirtableConnector(BaseConnector):
    def __init__(self, config: dict[str, Any]):
        if Api is None:
            raise ConfigurationError(
                "pyairtable not installed. Run 'pip install pyairtable'."
            )

        self._config_model: AirtableConfig | None = None
    ...
    def validate_config(self) -> None:
        try:
            ...
            raise ConfigurationError(f"Invalid Airtable configuration: {e}")  # noqa: B904
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            raise ConnectionFailedError(f"Failed to connect to Airtable: {e}")  # noqa: B904
    ...
    def infer_schema(self, asset: str, **kwargs) -> dict[str, Any]:
        ...
        try:
            ...
            for name, val in fields.items():
                ...
                columns.append(
                    {"name": name, "type": synqx_type, "native_type": py_type}
                )
    ...
    def execute_query(self, query: str, **kwargs) -> list[dict[str, Any]]:
        raise NotImplementedError(
            "Airtable does not support raw SQL queries. Use table selection."
        )
