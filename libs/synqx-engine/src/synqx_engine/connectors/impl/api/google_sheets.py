...
class GoogleSheetsConnector(BaseConnector):
    ...
    def validate_config(self) -> None:
        if not GOOGLE_LIBS_INSTALLED:
            raise ConfigurationError(
                "Google API libraries not installed. Please install 'google-api-python-client' and 'google-auth'."  # noqa: E501
            )

        if not self.config.get("spreadsheet_id"):
            raise ConfigurationError(
                "spreadsheet_id is required for Google Sheets connector."
            )
        ...
        if auth_type == "service_account":
            if not self.config.get("service_account_json"):
                raise ConfigurationError(
                    "service_account_json is required for service_account auth_type."
                )
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            raise ConfigurationError(  # noqa: B904
                f"Failed to connect to Google Sheets API: {e!s}"
            )
    ...
    def infer_schema(
        ...
        for col in df.columns:
            ...
            schema["columns"].append({"name": col, "type": stype, "native_type": dtype})
    ...
    def write_batch(
        ...
        **kwargs,
        ...
        try:
            ...
            values = [df.columns.values.tolist()] + df.values.tolist()  # noqa: RUF005
            ...
            if mode == "overwrite":
                ...
                self.service.spreadsheets().values().clear(
                    spreadsheetId=self.spreadsheet_id, range=asset
                ).execute()
                ...
                sheet_data = (
                    self.service.spreadsheets()
                    .values()
                    .get(spreadsheetId=self.spreadsheet_id, range=f"{asset}!A1:A1")
                    .execute()
                    .get("values", [])
                )
    ...
    def execute_query(
        ...
        **kwargs,
        ...
        raise NotImplementedError(
            "SQL queries are not supported for Google Sheets connector."
        )
