...
class RestApiConfig(BaseSettings):
    ...
    auth_type: str = Field(
        "none", description="Auth Type: none, basic, bearer, api_key"
    )
    auth_username: str | None = Field(None, description="Username for Basic Auth")
    auth_password: str | None = Field(None, description="Password for Basic Auth")
    auth_token: str | None = Field(None, description="Bearer Token")
    api_key_name: str | None = Field("X-API-Key", description="API Key Name")
    api_key_value: str | None = Field(None, description="API Key Value")
    api_key_in: str = Field("header", description="API Key Location: header or query")

    # Advanced Config
    headers: dict[str, str] = Field(
        default_factory=dict, description="Custom headers for all requests"
    )
    ...
    endpoints: list[dict[str, str]] = Field(
        ...
        description="List of pre-configured endpoints. Format: [{'name': 'users', 'path': '/users', 'methods': ['GET', 'POST']}]",  # noqa: E501
    )

    # Data Extraction
    data_key: str | None = Field(
        None,
        description="Dot-notation path to data in response (e.g. 'data.items'). If None, looks for common keys.",  # noqa: E501
    )

    # Pagination
    pagination_type: str = Field(
        "none", description="Pagination: none, limit_offset, page_number, cursor"
    )
    ...
    # Write Config
    write_method: str = Field(
        "POST", description="HTTP method for writing data (POST, PUT, PATCH)"
    )
    batch_size: int = Field(
        1,
        description="Number of records to send in a single request (if API supports batching)",  # noqa: E501
    )
    batch_key: str | None = Field(
        None, description="Key to wrap batch data in (e.g. 'records')"
    )
...
class RestApiConnector(BaseConnector):
    ...
    def __init__(self, config: dict[str, Any]):
        self._config_model: RestApiConfig | None = None
    ...
    def validate_config(self) -> None:
        try:
            ...
            raise ConfigurationError(f"Invalid REST API configuration: {e}")  # noqa: B904
    ...
    def connect(self) -> None:
        ...
        if self._config_model.auth_type == "basic":
            if self._config_model.auth_username and self._config_model.auth_password:
                auth = (
                    self._config_model.auth_username,
                    self._config_model.auth_password,
                )
            ...
            if self._config_model.api_key_value:
                if self._config_model.api_key_in == "header":
                    headers[self._config_model.api_key_name] = (
                        self._config_model.api_key_value
                    )
                else:
                    params[self._config_model.api_key_name] = (
                        self._config_model.api_key_value
                    )
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if self._config_model.endpoints:
                test_paths.extend(
                    [ep["path"] for ep in self._config_model.endpoints[:2]]
                )
            test_paths.append("/")
            ...
            for path in test_paths:
                try:
                    ...
                    res = self.client.get(
                        path, params={self._config_model.limit_param: 1}
                    )
                    if res.status_code < 400:  # noqa: PLR2004
                        return True
                except Exception:
                    continue
    ...
    def discover_assets(
        ...
        if not assets and not pattern:
            assets.append(
                {"name": "root", "fully_qualified_name": "/", "asset_type": "endpoint"}
            )

        return assets

    def infer_schema(
        self, asset: str, sample_size: int = 10, **kwargs
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            pagination_type = (
                kwargs.get("pagination_type") or self._config_model.pagination_type
            )
            limit_param = kwargs.get("limit_param") or self._config_model.limit_param
            page_size_param = (
                kwargs.get("page_size_param") or self._config_model.page_size_param
            )
            ...
            raise DataTransferError(f"REST schema inference failed for {asset}: {e}")  # noqa: B904

    def fetch_sample(
        self, asset: str, limit: int = 10, **kwargs
    ) -> list[dict[str, Any]]:
        """
        ...
        **kwargs,
        ...
        while True:
            ...
            try:
                ...
                raise DataTransferError(f"REST API read failed at path {path}: {e}")  # noqa: B904
    ...
    def write_batch(
        ...
        **kwargs,
        ...
        logger.info(
            f"Executing REST load using method {method} (Strategy: {clean_mode})"
        )
            ...
            if batch_size > 1:
                ...
                for i in range(0, len(records), batch_size):
                    ...
                    try:
                        ...
                        raise DataTransferError(f"REST batch write failed: {e}")  # noqa: B904
            else:
                ...
                for record in records:
                    try:
                        ...
                        # In production, we might want to continue or fail depending on config  # noqa: E501
                        if kwargs.get("strict", False):
                            raise DataTransferError(f"REST record write failed: {e}")  # noqa: B904
    ...
    def _extract_records(
        self, data: Any, override_data_key: str | None = None
    ) -> list[dict[str, Any]]:
        """
