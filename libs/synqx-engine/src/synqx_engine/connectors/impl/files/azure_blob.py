...
class AzureBlobConfig(BaseSettings):
    ...
    account_name: str | None = Field(None, description="Storage Account Name")
...
class AzureBlobConnector(BaseConnector):
    def __init__(self, config: dict[str, Any]):
        if BlobServiceClient is None:
            raise ConfigurationError(
                "Azure Storage Blob client not installed. Run 'pip install azure-storage-blob'."  # noqa: E501
            )

        self._config_model: AzureBlobConfig | None = None
    ...
    def validate_config(self) -> None:
        try:
            ...
            if not self._config_model.connection_string and not (
                self._config_model.account_name and self._config_model.account_key
            ):
                raise ConfigurationError(
                    "Either connection_string OR (account_name and account_key) must be provided."  # noqa: E501
                )
        except Exception as e:
            raise ConfigurationError(f"Invalid Azure Blob configuration: {e}")  # noqa: B904
        ...
        try:
            if self._config_model.connection_string:
                self._service_client = BlobServiceClient.from_connection_string(
                    self._config_model.connection_string
                )
            else:
            ...
            self._container_client = self._service_client.get_container_client(
                self._config_model.container_name
            )
            if not self._container_client.exists():
                raise ConnectionFailedError(
                    f"Container '{self._config_model.container_name}' does not exist."
                )

        except Exception as e:
            raise ConnectionFailedError(f"Failed to connect to Azure Blob Storage: {e}")  # noqa: B904
    ...
    def discover_assets(
        ...
        for blob in blobs:
            if len(assets) >= max_assets:
                logger.warning(
                    f"Reached max discovery limit of {max_assets} assets for container {self._config_model.container_name}"  # noqa: E501
                )
                break
            ...
            if ext in valid_extensions:
                asset = {
                    "name": blob.name,
                    "fully_qualified_name": f"{self._config_model.container_name}/{blob.name}",  # noqa: E501
                    "type": "file",
                    "format": ext.replace(".", ""),
                }
                if include_metadata:
    ...
    def infer_schema(
        ...
        try:
            ...
            raise SchemaDiscoveryError(  # noqa: B904
                f"Azure Blob schema inference failed for {asset}: {e}"
            )
    ...
    def execute_query(
        ...
        **kwargs,
        ...
        raise NotImplementedError(
            "Query execution is not supported for Azure Blob connector."
        )
    ...
    def list_files(self, path: str = "") -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise DataTransferError(f"Failed to list Azure blobs: {e}")  # noqa: B904
    ...
    def download_file(self, path: str) -> bytes:
        ...
        try:
            ...
            raise DataTransferError(f"Failed to download Azure blob: {e}")  # noqa: B904
    ...
    def upload_file(self, path: str, content: bytes) -> bool:
        ...
        try:
            ...
            raise DataTransferError(f"Failed to upload Azure blob: {e}")  # noqa: B904
    ...
    def delete_file(self, path: str) -> bool:
        ...
        try:
            ...
            raise DataTransferError(f"Failed to delete Azure blob: {e}")  # noqa: B904
    ...
    def create_directory(self, path: str) -> bool:
        ...
        try:
            ...
            raise DataTransferError(f"Failed to create Azure placeholder: {e}")  # noqa: B904
