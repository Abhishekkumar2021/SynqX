...
class S3Connector(BaseConnector):
    ...
    def __init__(self, config: dict[str, Any]):
        self._config_model: S3Config | None = None
    ...
    def validate_config(self) -> None:
        try:
            ...
            raise ConfigurationError(f"Invalid S3 configuration: {e}")  # noqa: B904
        ...
        try:
            ...
            if not self._fs.exists(self._config_model.bucket):
                raise ConnectionFailedError(
                    f"Bucket '{self._config_model.bucket}' not found or inaccessible."
                )

        except Exception as e:
            raise ConnectionFailedError(f"Failed to connect to S3: {e}")  # noqa: B904
    ...
    def discover_assets(
        ...
        try:
            ...
            glob_pattern = (
                f"{self._config_model.bucket}/**"
                if is_recursive
                else f"{self._config_model.bucket}/*"
            )
    ...
    def infer_schema(
        ...
        try:
            ...
            raise SchemaDiscoveryError(f"S3 schema inference failed for {asset}: {e}")  # noqa: B904
        ...
        **kwargs,
        ...
        try:
            ...
            if fmt == "csv":
                ...
                df = pd.read_json(
                    path, storage_options=storage_options, lines=(fmt == "jsonl")
                )
                df_iter = iter([self.slice_dataframe(df, offset, None)])
            else:
            ...
            raise DataTransferError(f"S3 read failed for {asset}: {e}")  # noqa: B904
    ...
    def write_batch(
        ...
        **kwargs,
        ...
        try:
            ...
            raise DataTransferError(f"S3 write failed for {asset}: {e}")  # noqa: B904

    def _write_df(
        self, df: pd.DataFrame, path: str, fmt: str, options: dict, mode: str
    ):
        if fmt == "csv":
            df.to_csv(
                path,
            ...
            df.to_json(
                path, orient="records", lines=(fmt == "jsonl"), storage_options=options
            )
    ...
    def list_files(self, path: str = "") -> list[dict[str, Any]]:
        ...
        try:
                ...
                results.append(
                    {
                        ...
                        "modified_at": item.get("LastModified").timestamp()
            ...
            raise DataTransferError(f"Failed to list S3 files: {e}")  # noqa: B904
    ...
    def download_file(self, path: str) -> bytes:
        ...
        try:
            ...
            raise DataTransferError(f"Failed to download S3 file: {e}")  # noqa: B904
    ...
    def upload_file(self, path: str, content: bytes) -> bool:
        ...
        try:
            ...
            raise DataTransferError(f"Failed to upload S3 file: {e}")  # noqa: B904
    ...
    def delete_file(self, path: str) -> bool:
        ...
        try:
            ...
            raise DataTransferError(f"Failed to delete S3 item: {e}")  # noqa: B904
    ...
    def create_directory(self, path: str) -> bool:
        ...
        try:
            ...
            raise DataTransferError(f"Failed to create S3 placeholder: {e}")  # noqa: B904
