...
class CassandraConfig(BaseSettings):
    ...
    use_asyncio: bool = Field(
        False,
        description="Whether to use AsyncioConnection (recommended True only for asyncio apps, defaults to False for sync compatibility)",  # noqa: E501
    )
...
class CassandraConnector(BaseConnector):
    def __init__(self, config: dict[str, Any]):
        if Cluster is None:
            raise ConfigurationError(
                "Cassandra driver not installed. Run 'pip install cassandra-driver'."
            )
        ...
        if os.environ.get("CASS_DRIVER_NO_EXTENSIONS") == "1":
            logger.info(
                "Cassandra driver: C extensions disabled via environment variable."
            )

        self._config_model: CassandraConfig | None = None
        ...
        try:
            if isinstance(self.config.get("contact_points"), str):
                self.config["contact_points"] = [
                    cp.strip() for cp in self.config["contact_points"].split(",")
                ]
            self._config_model = CassandraConfig.model_validate(self.config)
        except Exception as e:
            raise ConfigurationError(f"Invalid Cassandra configuration: {e}")  # noqa: B904
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            # For Python 3.12/3.13, AsyncioConnection can be used but requires careful loop handling in sync apps.  # noqa: E501
            # We default to standard connection (Sync) which is safer for this Connector architecture.  # noqa: E501
            # However, for 3.12+, AsyncioConnection is required if C-extensions are missing.  # noqa: E501
            if (
                self._config_model.use_asyncio or sys.version_info >= (3, 12)
            ) and AsyncioConnection:
            ...
            # Fallback: if AsyncioConnection failed (e.g. no running loop) or driver issue  # noqa: E501
            if "connection_class" in cluster_kwargs:
                logger.warning(
                    f"Failed to connect with AsyncioConnection ({e}), retrying with default connection class"  # noqa: E501
                )
                try:
                    ...
                    raise ConnectionFailedError(  # noqa: B904
                        f"Failed to connect to Cassandra (Default Fallback): {e2}"
                    )

            # If standard connection failed, check for C-extension issues common on Windows  # noqa: E501
            if "No module named" in str(e) or "DLL load failed" in str(e):
                logger.warning(
                    "Potential C-extension issue detected. Try setting CASS_DRIVER_NO_EXTENSIONS=1"  # noqa: E501
                )

            raise ConnectionFailedError(f"Failed to connect to Cassandra: {e}")  # noqa: B904
    ...
    def discover_assets(
        ...
        keyspace_meta = self._cluster.metadata.keyspaces.get(
            self._config_model.keyspace
        )
    ...
    def infer_schema(self, asset: str, **kwargs) -> dict[str, Any]:
        ...
        for col_name, col_meta in table_meta.columns.items():
            ...
            columns.append(
                {"name": col_name, "type": synqx_type, "native_type": c_type}
            )

        return {"asset": asset, "columns": columns, "type": "table"}
    ...
    def read_batch(
        ...
        **kwargs,
        ...
        from cassandra.query import SimpleStatement  # noqa: PLC0415
