...
class KafkaConfig(BaseSettings):
    ...
    bootstrap_servers: list[str] = Field(
        ..., description="List of Kafka brokers (e.g. localhost:9092)"
    )
    security_protocol: str = Field(
        "PLAINTEXT", description="Protocol used to communicate with brokers"
    )
    sasl_mechanism: str | None = Field(
        None, description="SASL mechanism (e.g. PLAIN, SCRAM-SHA-256)"
    )
    sasl_plain_username: str | None = Field(None, description="SASL Username")
    sasl_plain_password: str | None = Field(None, description="SASL Password")
    group_id: str | None = Field(None, description="Consumer Group ID")
...
class KafkaConnector(BaseConnector):
    def __init__(self, config: dict[str, Any]):
        if KafkaConsumer is None:
            raise ConfigurationError(
                "Kafka client not installed. Run 'pip install kafka-python-ng'."
            )

        self._config_model: KafkaConfig | None = None
        ...
        try:
            ...
            if isinstance(self.config.get("bootstrap_servers"), str):
                self.config["bootstrap_servers"] = [
                    s.strip() for s in self.config["bootstrap_servers"].split(",")
                ]

            self._config_model = KafkaConfig.model_validate(self.config)
        except Exception as e:
            raise ConfigurationError(f"Invalid Kafka configuration: {e}")  # noqa: B904
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            raise ConnectionFailedError(f"No Kafka brokers available: {e}")  # noqa: B904
        except Exception as e:
            raise ConnectionFailedError(f"Failed to connect to Kafka: {e}")  # noqa: B904
    ...
    def discover_assets(
        ...
        try:
            ...
            for topic in topics:
                ...
                asset = {"name": topic, "fully_qualified_name": topic, "type": "stream"}
                assets.append(asset)
            admin.close()
            return assets
        except Exception as e:
    ...
    def read_batch(
        ...
        **kwargs,
        ...
        args["enable_auto_commit"] = (
            False  # Manual control if needed, but for explorer we iterate
        )
        args["consumer_timeout_ms"] = kwargs.get(
            "timeout_ms", 5000
        )  # Stop if no messages
                    ...
                    try:
                        ...
                        if isinstance(row["value"], dict):
                            row["value"] = json.dumps(
                                row["value"]
                            )  # keep as string for dataframe consistency
    ...
    def execute_query(self, query: str, **kwargs) -> list[dict[str, Any]]:
        raise NotImplementedError(
            "Kafka does not support direct queries. Use stream processing."
        )
