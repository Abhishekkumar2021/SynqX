...
class RabbitMQConnector(BaseConnector):
    def __init__(self, config: dict[str, Any]):
        if pika is None:
            raise ConfigurationError(
                "Pika client not installed. Run 'pip install pika'."
            )
    ...
    def validate_config(self) -> None:
        try:
            ...
            raise ConfigurationError(f"Invalid RabbitMQ configuration: {e}")  # noqa: B904
        ...
        try:
            ...
            if self._config_model.username:
                credentials = pika.PlainCredentials(
                    self._config_model.username, self._config_model.password
                )
            ...
            raise ConnectionFailedError(f"Failed to connect to RabbitMQ: {e}")  # noqa: B904
    ...
    def read_batch(
        ...
        **kwargs,
        ...
        while True:
            method_frame, header_frame, body = self._channel.basic_get(  # noqa: RUF059
                queue=asset, auto_ack=True
            )
