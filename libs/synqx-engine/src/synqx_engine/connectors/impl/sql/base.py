...
class SQLConnector(BaseConnector):
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            raise ConnectionFailedError(f"Connection failed: {e}")  # noqa: B904
        ...
        try:
            ...
            for asset in all_assets:
                ...
                results.append(
                    {
                        "name": asset,
                        "fully_qualified_name": f"{db_schema}.{asset}"
                        if db_schema
                        else asset,
    ...
    def _get_row_count(self, table: str, schema: str | None = None) -> int | None:
        try:
    ...
    def infer_schema(
        ...
        try:
            ...
            if mode == "sample" or mode == "auto":  # noqa: PLR1714
    ...
    def _schema_from_metadata(
        ...
        try:
            ...
            raise SchemaDiscoveryError(f"Metadata extraction failed: {e}")  # noqa: B904

    def _schema_from_sample(
        self, asset: str, schema: str | None, sample_size: int
    ) -> dict[str, Any]:
        try:
            ...
            raise SchemaDiscoveryError(f"Sample-based inference failed: {e}")  # noqa: B904
        ...
        **kwargs,
        ...
        if custom_query:
            ...
            if incremental_filter and isinstance(incremental_filter, dict):
                ...
                if where_clauses:
                    clean_query = f"SELECT * FROM ({clean_query}) AS inc_subq WHERE {' AND '.join(where_clauses)}"  # noqa: E501
        ...
        try:
            for chunk in pd.read_sql_query(  # noqa: UP028
                text(query),
            ...
            raise DataTransferError(  # noqa: B904
                f"Stream read failed for entity '{asset}'. Underlying fault: {e!s}"
            )
    ...
    def get_total_count(
        ...
        try:
            ...
            if is_query:
                ...
                count_query = (
                    f"SELECT COUNT(*) FROM ({clean_query}) AS total_count_subq"
                )
            else:
            ...
            logger.warning(
                f"Failed to calculate total record count for '{query_or_asset}': {e}"
            )
            return None
    ...
    def execute_query(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(
                f"Executing SQL: {clean_query} | Params: {bind_params} | Limit: {limit} | Offset: {offset}"  # noqa: E501
            )
            ...
            # PERFORMANCE: For non-SELECT statements (INSERT/UPDATE/TRUNCATE), use direct execution  # noqa: E501
            ...
            raise DataTransferError(f"Query execution failed. Detailed fault: {e!s}")  # noqa: B904

    @retry(exceptions=(Exception,), max_attempts=3)
    def write_batch(  # noqa: PLR0912, PLR0915
        self,
        ...
        **kwargs,
        ...
        evolution_policy = kwargs.get(
            "schema_evolution_policy", SchemaEvolutionPolicy.STRICT
        )
        ...
        try:
            ...
            for df in data_iter:
                ...
                new_cols = (
                    [c for c in df.columns if c not in target_columns]
                    if table_exists
                    else []
                )
                ...
                if new_cols:
                    if evolution_policy == SchemaEvolutionPolicy.STRICT:
                        msg = f"Schema Breach: New columns {new_cols} detected but policy is STRICT for '{asset}'."  # noqa: E501
                        logger.error(msg)
                        raise DataTransferError(msg)

                    elif evolution_policy == SchemaEvolutionPolicy.EVOLVE:
                        logger.info(
                            f"Schema Evolution: Automatically adding {len(new_cols)} columns to '{asset}'"  # noqa: E501
                        )
                        for col in new_cols:
                            ...
                            if "int" in dtype:
                                ...
                                sql_type = (
                                    "JSONB"
                                    if "postgres" in str(self._engine.url).lower()
                                    else "TEXT"
                                )

                            table_ref = (
                                f'"{schema}"."{name}"' if schema else f'"{name}"'
                            )
                            alter_stmt = (
                                f'ALTER TABLE {table_ref} ADD COLUMN "{col}" {sql_type}'
                            )
                            ...
                            try:
                                ...
                                raise DataTransferError(  # noqa: B904
                                    f"Failed to evolve schema for '{asset}': {alter_err}"  # noqa: E501
                                )

                    elif evolution_policy == SchemaEvolutionPolicy.IGNORE:
                        logger.warning(
                            f"Schema Evolution (IGNORE): Dropping new columns {new_cols} from stream for '{asset}'"  # noqa: E501
                        )
                        df = df[target_columns]  # noqa: PLW2901

                # Case: Safety Filter for missing columns in source (Nullable by default in target is fine)  # noqa: E501
                if table_exists and evolution_policy != SchemaEvolutionPolicy.EVOLVE:
                    # Only include columns that actually exist in target
                    df = df[[c for c in df.columns if c in target_columns]]  # noqa: PLW2901
            ...
            raise DataTransferError(  # noqa: B904
                f"Target commit failed for entity '{asset}'. Detailed fault: {e!s}"
            )
