...
class BigQueryConnector(BaseConnector):
    ...
    def __init__(self, config: dict[str, Any]):
        self._config_model: BigQueryConfig | None = None
    ...
    def validate_config(self) -> None:
        try:
            ...
            raise ConfigurationError(f"Invalid BigQuery config: {e}")  # noqa: B904
        ...
        try:
            if self._config_model.credentials_json:
                ...
                self._client = bigquery.Client(
                    project=self._config_model.project_id, credentials=creds
                )
            ...
            raise ConnectionFailedError(f"BigQuery connection failed: {e}")  # noqa: B904
    ...
    def infer_schema(self, asset: str, **kwargs) -> dict[str, Any]:
        ...
        columns = [
            {"name": s.name, "type": s.field_type, "mode": s.mode} for s in table.schema
        ]
        return {
            "asset": asset,
            "columns": columns,
            "schema": self._config_model.dataset_id,
        }
    ...
    def read_batch(
        ...
        **kwargs,
        ...
        self.connect()
        query = f"SELECT * FROM `{self._config_model.project_id}.{self._config_model.dataset_id}.{asset}`"  # noqa: E501
        ...
        try:
            ...
            raise DataTransferError(f"BigQuery read failed: {e}")  # noqa: B904
    ...
    def write_batch(
        ...
        **kwargs,
        ...
        self.connect()
        table_id = (
            f"{self._config_model.project_id}.{self._config_model.dataset_id}.{asset}"
        )

        job_config = bigquery.LoadJobConfig()
        if mode == "replace" or mode == "overwrite":  # noqa: PLR1714
    ...
    def write_staged(
        ...
        **kwargs,
        ...
        """
        from synqx_engine.connectors.impl.files.gcs import GCSConnector  # noqa: PLC0415

        if not isinstance(stage_connector, GCSConnector):
            logger.warning(
                "BigQuery staged write requested with non-GCS connector. Falling back to direct load."  # noqa: E501
            )
        ...
        try:
            ...
            rows_written = stage_connector.write_batch(
                data, stage_filename, mode="replace"
            )
            ...
            raise DataTransferError(f"BigQuery staged load failed: {e}")  # noqa: B904
        finally:
            try:
    ...
    def execute_query(
        ...
        **kwargs,
        ...
        try:
            ...
            raise DataTransferError(f"BigQuery query execution failed: {e}")  # noqa: B904
