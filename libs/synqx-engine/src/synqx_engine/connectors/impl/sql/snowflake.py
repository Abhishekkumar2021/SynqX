    ...
    def write_staged(
        ...
        **kwargs,
        ...
        if not isinstance(stage_connector, S3Connector):
            logger.warning(f"Snowflake staged write requested with non-S3 connector ({type(stage_connector)}). Falling back to direct insert.")
        ...
        try:
            ...
            raise DataTransferError(f"Snowflake staged load failed: {e}")
        finally:
