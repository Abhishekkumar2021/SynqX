...
class DataCache:
    ...
    def __init__(self, max_memory_mb: int = 2048, spill_dir: str | None = None):
        ...
        logger.info(
            f"DataCache initialized. Memory Limit: {max_memory_mb}MB | Spill Dir: {self.spill_dir}"  # noqa: E501
        )
    ...
    def store(self, node_id: str, chunks: list[Any]):
        ...
        with self._lock:
            ...
            memory_mb = sum(self._get_df_size(df) for df in chunks) / (1024 * 1024)

            # Memory pressure handling: Spill instead of just warning
            if self._current_memory_mb + memory_mb > self.max_memory_mb:
                logger.warning(
                    f"Memory pressure detected: Current={self._current_memory_mb:.2f}MB, "  # noqa: E501
                    f"Incoming={memory_mb:.2f}MB, Limit={self.max_memory_mb}MB. Triggering Spill-to-Disk."  # noqa: E501
                )
    ...
    def clear_node(self, node_id: str):
        ...
        with self._lock:
            ...
            if node_id in self._cache:
                ...
                memory_freed = sum(self._get_df_size(df) for df in chunks) / (
                    1024 * 1024
                )
    ...
    def _spill_to_disk(self, node_id: str) -> float:
        ...
        try:
            ...
            logger.info(
                f"Spilled node '{node_id}' to disk. Freed {memory_to_free:.2f}MB RAM."
            )
