...
class OSDUGovernanceService(OSDUBaseClient):
    ...
    def get_groups(self, limit: int = 100, offset: int = 0, **kwargs) -> List[Dict[str, Any]]:
        params = {"limit": limit, "offset": offset, **kwargs}
        return self._get("api/entitlements/v2/groups", params=params).json().get("groups", [])

    def get_group_members(self, group_email: str) -> List[Dict[str, Any]]:
        return self._get(f"api/entitlements/v2/groups/{group_email}/members").json().get("members", [])

    def add_member(self, group_email: str, email: str, role: str = "MEMBER"):
        payload = {"email": email, "role": role}
        return self._post(f"api/entitlements/v2/groups/{group_email}/members", json=payload).json()

    # --- Legal (Compliance) ---
    def get_legal_tags(self, limit: int = 100, offset: int = 0, **kwargs) -> List[Dict[str, Any]]:
        params = {"limit": limit, "offset": offset, **kwargs}
        return self._get("api/legal/v1/legaltags", params=params).json().get("legalTags", [])
