...
class AggregateTransform(PolarsTransform):
    ...
    def transform(self, data: Iterator[pl.DataFrame]) -> Iterator[pl.DataFrame]:  # noqa: PLR0912
        ...
        try:
            ...
            for col, op in agg_map.items():
                if op == "sum":
                ...
                elif op == "mean" or op == "avg":  # noqa: PLR1714
                    ...
                    logger.warning(
                        f"Unsupported aggregate operation '{op}' for column '{col}'. Skipping."  # noqa: E501
                    )
